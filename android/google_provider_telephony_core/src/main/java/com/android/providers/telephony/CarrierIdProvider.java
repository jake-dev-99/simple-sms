 ...
 *
 * Each row in carrier identification db consists of matching rule (e.g.,
 * MCCMNC, GID1, GID2, PLMN)
 * and its matched carrier id & carrier name. Each carrier either MNO or MVNO
 * could be
 * identified by multiple matching rules but is assigned with a unique ID (cid).
 *
 *
 * This class provides the ability to retrieve the cid of the current
 * subscription.
 * This is done atomically through a query.
 *
 * This class also provides a way to update carrier identifying attributes of an
 * existing entry.
...
public class CarrierIdProvider extends ContentProvider {
    ...
    // The version number is offset by SDK level, the MSB 8 bits is reserved for
    // SDK.
     ...
     * Key is the active subId and entryValue is carrier id(int), mno carrier id
     * (int) and
     * carrier name(String).
     */
    ...
    /**
     * These methods can be overridden in a subclass for testing CarrierIdProvider
     * using an
     * in-memory database.
     */
     ...
     * Get the latest version from the last applied, assets and ota file. if the
     * latest version
     * is newer than the last applied, update is required. Otherwise no update is
     * required and
    ...
    private int updateCarrierIdForCurrentSubscription(Uri uri, ContentValues cv) {
        ...
        if (!sm.isActiveSubscriptionId(subId)) {
            ...
            final List<SubscriptionInfo> subscriptionInfoList = sm.getCompleteActiveSubscriptionInfoList();
