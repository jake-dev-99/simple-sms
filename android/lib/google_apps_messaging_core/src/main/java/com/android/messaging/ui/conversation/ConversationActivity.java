...
public class ConversationActivity extends BugleActionBarActivity
    ...
    // Fragment transactions cannot be performed after onSaveInstanceState() has
    // been called since
    // it will cause state loss. We don't want to call commitAllowingStateLoss()
    // since it's
    // dangerous. Therefore, we note when instance state is saved and avoid
    // performing UI state
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        // See if we're getting called from a widget to directly display an image or
        // video
        final String extraToDisplay = intent.getStringExtra(UIIntents.UI_INTENT_EXTRA_ATTACHMENT_URI);
        if (!TextUtils.isEmpty(extraToDisplay)) {
            final String contentType = intent.getStringExtra(UIIntents.UI_INTENT_EXTRA_ATTACHMENT_TYPE);
    ...
    protected void onSaveInstanceState(final Bundle outState) {
        ...
        // After onSaveInstanceState() is called, future changes to mUiState won't
        // update the UI
        // anymore, because fragment transactions are not allowed past this point.
        // For an activity recreation due to orientation change, the saved instance
        // state keeps
        // using the in-memory copy of the UI state instead of writing it to parcel as
        // an
        // optimization, so the UI state values may still change in response to, for
        // example,
        // focus change from the framework, making mUiState and actual UI inconsistent.
        // Therefore, save an exact "snapshot" (clone) of the UI state object to make
        // sure the
    ...
    protected void onResume() {
        ...
        // we need to reset the mInstanceStateSaved flag since we may have just been
        // restored from
    ...
    public void onWindowFocusChanged(final boolean hasFocus) {
        ...
        // When the screen is turned on, the last used activity gets resumed, but it
        // gets
    ...
    public void onParticipantPickerUiStateChanged(final int oldState, final int newState,
