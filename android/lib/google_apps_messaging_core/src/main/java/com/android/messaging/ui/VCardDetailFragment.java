...
/**
 * A fragment that shows the content of a VCard that contains one or more
 * contacts.
 */
public class VCardDetailFragment extends Fragment implements PersonItemDataListener {
    ...
    /**
     * We need to persist the VCard in the scratch directory before letting the user
     * view it.
     * We save this Uri locally, so that if the user cancels the action and
     * re-perform the add
    ...
    public boolean onOptionsItemSelected(final MenuItem item) {
        if (item.getItemId() == R.id.action_add_contact) {
            ...
            // We have to do things in the background in case we need to copy the vcard
            // data.
            new SafeAsyncTask<Void, Void, Uri>() {
                ...
                protected Uri doInBackgroundTimed(final Void... params) {
                    ...
                    return mScratchSpaceUri != null ? mScratchSpaceUri : UriUtil.persistContentToScratchSpace(vCardUri);
                }
