 ...
 * This widget is used
 * by both the media picker and the conversation message view to show audio
 * attachments.
 */
public class AudioAttachmentView extends LinearLayout {
    ...
    /**
     * The compact layout mode with only the play button and the timer beneath it.
     * Suitable
    ...
    // The corner radius for drawing rounded corners. The default value is zero (no
    // rounded corners)
    ...
    // should the MediaPlayer be prepared lazily when the user chooses to play the
    // audio (as
    ...
    public AudioAttachmentView(final Context context, final AttributeSet attrs) {
        super(context, attrs);
        final TypedArray typedAttributes = context.obtainStyledAttributes(attrs, R.styleable.AudioAttachmentView);
    ...
    private void updateVisualStyle() {
        if (mMode == LAYOUT_MODE_SUB_COMPACT) {
            // Sub-compact mode has static visual appearance already set up during
            // initialization.
            return;
        }
