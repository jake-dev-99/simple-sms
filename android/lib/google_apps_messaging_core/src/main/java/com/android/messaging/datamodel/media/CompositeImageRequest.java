...
/**
 * Requests a composite image resource. The composite image resource is
 * constructed by first
 * sequentially requesting a number of sub image resources specified by
 * {@link CompositeImageRequestDescriptor#getChildRequestDescriptors()}. After
 * this, the
 * individual sub images are composed into the final image onto their respective
 * target rects
 * returned by
 * {@link CompositeImageRequestDescriptor#getChildRequestTargetRects()}.
 */
public class CompositeImageRequest<D extends CompositeImageRequestDescriptor>
    ...
    protected ImageResource loadMediaInternal(List<MediaRequest<ImageResource>> chainedTask) {
        final List<? extends ImageRequestDescriptor> descriptors = mDescriptor.getChildRequestDescriptors();
        ...
        for (int i = 0; i < descriptors.size(); i++) {
            final MediaRequest<ImageResource> request = descriptors.get(i).buildSyncMediaRequest(mContext);
            // Synchronously request the child image.
            final ImageResource resource = MediaResourceManager.get().requestMediaResourceSync(request);
            if (resource != null) {
                try {
