...
/**
 * Action used to convert a draft message to an outgoing message. Its writes SMS
 * messages to
 * the telephony db, but {@link SendMessageAction} is responsible for inserting
 * MMS message into
 * the telephony DB. The latter also does the actual sending of the message in
 * the background.
...
public class InsertNewMessageAction extends Action implements Parcelable {
    ...
    protected Object executeAction() {
        ...
        // If the user taps the Send button before the conversation draft is
        // created/loaded by
        // ReadDraftDataAction (maybe the action service thread was busy), the
        // MessageData may not
        // have the participant id set. It should be equal to the self id, so we'll use
        // that.
        ...
        // TODO: Work out whether to send with SMS or MMS (taking into account
        // recipients)?
        final boolean isSms = (message.getProtocol() == MessageData.PROTOCOL_SMS);
        if (isSms) {
            String sendingConversationId = conversationId;
            if (recipients.size() > 1) {
                // Broadcast SMS - put message in "fake conversation" before farming out to real
                // 1:1
    ...
    private ParticipantData getSelf(
        ...
        // Check if we are asked to bind to a non-default subId. This is directly passed
        // in from
        // the UI thread so that the sub id may be locked as soon as the user clicks on
        // the Send
        // button.
        ...
        if (requestedSubId != ParticipantData.DEFAULT_SELF_SUB_ID) {
            ...
            if (selfId == null) {
                ...
                // self id was not loaded AND 2) the user didn't pick a SIM from the SIM
                // selector.
                // In this case, use the conversation's self id.
                final ConversationListItemData conversation = ConversationListItemData.getExistingConversation(db,
                        conversationId);
            ...
            // We do not use SubscriptionManager.DEFAULT_SUB_ID for sending a message, so we
            // need
    ...
    private MessageData createMessage() {
        ...
        final ArrayList<String> recipients = BugleDatabaseOperations
                .getRecipientsFromConversationParticipants(participants);
    ...
    private MessageData insertSendingSmsMessage(final MessageData content, final int subId,
        ...
        if (sendingConversationId == null) {
            // For 1:1 message generated sending broadcast need to look up
    ...
    public static final Parcelable.Creator<InsertNewMessageAction> CREATOR = new Parcelable.Creator<InsertNewMessageAction>() {
        @Override
