...
/**
 * An extension for {@link RecipientEditTextView} which shows a list of
 * Materialized contact chips.
 * It uses Bugle's ContactUtil to perform contact lookup, and is able to return
 * the list of
...
public class ContactRecipientAutoCompleteView extends RecipientEditTextView {
    ...
    private class ContactChipsWatcher implements TextWatcher {
        /**
         * Tracks the old chips count before text changes. Note that we currently don't
         * compare
         * the entire chip sets but just the cheaper-to-do before and after counts,
         * because
        ...
        public void beforeTextChanged(final CharSequence s, final int start, final int count,
                final int after) {
            // We don't take mLastChipsCount from here but from the last afterTextChanged()
            // run.
            // The reason is because at this point, any chip spans to be removed is already
            // removed
    ...
    /**
     * A tuple of chips which AsyncContactChipSanitizeTask reports as progress to
     * have the
    ...
    /**
     * An AsyncTask that cleans up contact chips on every chips commit (i.e. get or
     * create a new
    ...
    /**
     * We don't use SafeAsyncTask but instead use a single threaded executor to
     * ensure that
     * all sanitization tasks are serially executed so as not to interfere with each
     * other.
     */
    ...
    /**
     * Whenever the caller wants to start a new conversation with the list of chips
     * we have,
    ...
    public ArrayList<ParticipantData> getRecipientParticipantDataForConversationCreation() {
        final DrawableRecipientChip[] recips = getText()
                .getSpans(0, getText().length(), DrawableRecipientChip.class);
        final ArrayList<ParticipantData> contacts = new ArrayList<ParticipantData>(recips.length);
        for (final DrawableRecipientChip recipient : recips) {
            final RecipientEntry entry = recipient.getEntry();
    ...
    /**
     * c
     * Gets a set of currently selected chips' emails/phone numbers. This will
     * facilitate the
