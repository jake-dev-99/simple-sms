...
public class ImageUtils {
     ...
     *
     * @param quality Value between 0 and 100 that the compressor uses to discern
     *                what quality the
    ...
    /**
     * Based on the input bitmap bounds given by BitmapFactory.Options, compute the
     * required
     * sub-sampling size for loading a scaled down version of the bitmap to the
     * required size
     *
     * @param options   a BitmapFactory.Options instance containing the bounds info
     *                  of the bitmap
     ...
     * TODO: This uses memory very inefficiently as it processes the whole image as
     * a unit
     * (rather than slice by slice) but system JPEG functions do not support slicing
     * and dicing.
     */
    public static class ImageResizer {
        ...
        private byte[] resizeGifImage() {
            ...
            if (GifTranscoder.canBeTranscoded(mWidth, mHeight)) {
                ...
                final boolean success = GifTranscoder.transcode(mContext, inputFilePath, outputFilePath);
                if (success) {
                    try {
        ...
        /**
         * Choose an initial subsamplesize that ensures the decoded image is no more
         * than
         * MAX_TARGET_SCALE_FACTOR bigger than largest supported image and that it is
         * likely to
         * compress to smaller than the target size (assuming compression down to 1 bit
         * per pixel).
         *
        ...
        private boolean canBeCompressed() {
            ...
            // Assume can use half working memory to decode the initial image (4 bytes per
            // pixel)
            ...
            // When choosing to halve the resolution - only do so the image will still be
            // too big
            ...
            while (!fits) {
                ...
                // Note that recodeImage may try using mSampleSize * 2. Hence we use the factor
                // of 4
                if (sampleSize >= (Integer.MAX_VALUE / 4)) {
                    LogUtil.w(LogUtil.BUGLE_IMAGE_TAG, String.format(
                            ...
                            mWidthLimit, mHeightLimit, mByteLimit,
                            mWidth, mHeight));
                ...
                if (mScaled == null) {
                    ...
                    if (decodedWidth > mWidthLimit || decodedHeight > mHeightLimit) {
                        final float minScaleFactor = Math.max(
                                mWidthLimit == 0 ? 1.0f : (float) decodedWidth / (float) mWidthLimit,
                                mHeightLimit == 0 ? 1.0f : (float) decodedHeight / (float) mHeightLimit);
        ...
        /**
         * When image recode fails this method updates compression parameters for the
         * next attempt
         *
        ...
        private void updateRecodeParameters(final int currentSize) {
            ...
            if (currentSize > 0 &&
                ...
                // First try scaling by a little (< factor of 2) just so long resulting scale
                // down
    ...
    /**
     * Scales and center-crops a bitmap to the size passed in and returns the new
     * bitmap.
     *
    ...
    public static Bitmap scaleCenterCrop(final Bitmap source, final int newWidth,
        ...
        // The target rectangle for the new, scaled version of the source bitmap will
        // now
        // be
     ...
     * instance for each
     * drawable of different sizes, then the drawable sizes would interfere with
     * each other. The
     * solution here is to create a new drawable instance for every time with the
     * SAME
     * ConstantState (i.e. sharing the same common state such as the bitmap, so that
     * we don't have
     * to recreate the bitmap resource), and apply the different properties on top
     * (nine-patch
     * size and color tint).
     *
     * TODO: we are creating new drawable instances here, but there are
     * optimizations that
     * can be made. For example, message bubbles shouldn't need the mutate() call
     * and the
     * play/pause buttons shouldn't need to create new drawable from the constant
     * state.
     */
