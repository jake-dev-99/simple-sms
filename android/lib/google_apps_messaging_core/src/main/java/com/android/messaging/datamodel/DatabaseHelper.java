...
public class DatabaseHelper extends SQLiteOpenHelper {
     ...
     * TODO(rtenneti): Fix the following special SQLLite table name when SQLLite
     * changes.
     */
    ...
    public static class ConversationColumns implements BaseColumns {
        ...
        /*
         * Latest text subject for display in conversation list, empty string if none
         * exists
         */
        ...
        /*
         * If we should display the current draft snippet/preview pair or
         * snippet/preview pair
         */
        public static final String SHOW_DRAFT = "show_draft";

        /*
         * Latest draft text subject for display in conversation list, empty string if
         * none exists
         */
        ...
        /*
         * Participant contact ID if this conversation has a single participant. -1
         * otherwise
         */
        public static final String PARTICIPANT_CONTACT_ID = "participant_contact_id";

        /*
         * Participant lookup key if this conversation has a single participant. null
         * otherwise
         */
        public static final String PARTICIPANT_LOOKUP_KEY = "participant_lookup_key";

        /*
         * Participant's normalized destination if this conversation has a single
         * participant.
         * null otherwise.
         */
        ...
        /*
         * Participant count not including self (so will be 1 for 1:1 or bigger for
         * group)
         */
        ...
        // Record the last received sms's service center info if it indicates that the
        // reply path
        // is present (TP-Reply-Path), so that we could use it for the subsequent
        // message to send.
        ...
        // A conversation is enterprise if one of the participant is a enterprise
        // contact.
        public static final String IS_ENTERPRISE = "IS_ENTERPRISE";
    }

    // Conversation table SQL
    private static final String CREATE_CONVERSATIONS_TABLE_SQL = "CREATE TABLE " + CONVERSATIONS_TABLE + "("
    ...
    private static final String CONVERSATIONS_TABLE_SMS_THREAD_ID_INDEX_SQL = "CREATE INDEX index_"
            + CONVERSATIONS_TABLE + "_" + ConversationColumns.SMS_THREAD_ID
            + " ON " + CONVERSATIONS_TABLE
            + "(" + ConversationColumns.SMS_THREAD_ID + ")";

    private static final String CONVERSATIONS_TABLE_ARCHIVE_STATUS_INDEX_SQL = "CREATE INDEX index_"
            + CONVERSATIONS_TABLE + "_" + ConversationColumns.ARCHIVE_STATUS
            + " ON " + CONVERSATIONS_TABLE
            + "(" + ConversationColumns.ARCHIVE_STATUS + ")";

    private static final String CONVERSATIONS_TABLE_SORT_TIMESTAMP_INDEX_SQL = "CREATE INDEX index_"
            + CONVERSATIONS_TABLE + "_" + ConversationColumns.SORT_TIMESTAMP
            + " ON " + CONVERSATIONS_TABLE
    ...
    // Messages table SQL
    private static final String CREATE_MESSAGES_TABLE_SQL = "CREATE TABLE " + MESSAGES_TABLE + " ("
    ...
    // Primary sort index for messages table : by conversation id, status, received
    // timestamp.
    private static final String MESSAGES_TABLE_SORT_INDEX_SQL = "CREATE INDEX index_" + MESSAGES_TABLE + "_sort ON "
            + MESSAGES_TABLE + "("
            + MessageColumns.CONVERSATION_ID + ", "
            + MessageColumns.STATUS + ", "
            + MessageColumns.RECEIVED_TIMESTAMP + ")";

    private static final String MESSAGES_TABLE_STATUS_SEEN_INDEX_SQL = "CREATE INDEX index_" + MESSAGES_TABLE
            + "_status_seen ON " + MESSAGES_TABLE + "("
            + MessageColumns.STATUS + ", "
            + MessageColumns.SEEN + ")";
    ...
    private static final String CREATE_PARTS_TABLE_SQL = "CREATE TABLE " + PARTS_TABLE + "("
    ...
    public static final String CREATE_PARTS_TRIGGER_SQL = "CREATE TRIGGER " + PARTS_TABLE + "_TRIGGER"
            + " AFTER INSERT ON " + PARTS_TABLE
            + " FOR EACH ROW "
    ...
    public static final String CREATE_MESSAGES_TRIGGER_SQL = "CREATE TRIGGER " + MESSAGES_TABLE + "_TRIGGER"
            + " AFTER UPDATE OF "
    ...
    private static final String PARTS_TABLE_MESSAGE_INDEX_SQL = "CREATE INDEX index_" + PARTS_TABLE + "_message_id ON "
            + PARTS_TABLE + "("
    ...
    public static class ParticipantColumns implements BaseColumns {
        ...
        /*
         * The slot of the active SIM (inserted in the device) for this
         * self-participant. If the
        ...
        /*
         * The phone number stored in a standard E164 format if possible. This is unique
         * for a
         * given participant. We can't handle multiple participants with the same phone
         * number
         * since we don't know which of them a message comes from. This can also be an
         * email
        ...
        /*
         * The phone number as originally supplied and used for dialing. Not necessarily
         * in E164
         * format or unique
         */
        ...
        /*
         * The user-friendly formatting of the phone number according to the region
         * setting of
        ...
        /*
         * String that contains hints on how to find contact information in a contact
         * lookup
         */
    ...
    private static final String CREATE_PARTICIPANTS_TABLE_SQL = "CREATE TABLE " + PARTICIPANTS_TABLE + "("
    ...
    private static final String CREATE_SELF_PARTICIPANT_SQL = "INSERT INTO " + PARTICIPANTS_TABLE
    ...
    // Conversation Participants table schema - contains a list of participants
    // excluding the user
    ...
    private static final String CREATE_CONVERSATION_PARTICIPANTS_TABLE_SQL = "CREATE TABLE "
    ...
    // Primary access pattern for conversation participants is to look them up for a
    // specific
    // conversation.
    private static final String CONVERSATION_PARTICIPANTS_TABLE_CONVERSATION_ID_INDEX_SQL = "CREATE INDEX index_"
            + CONVERSATION_PARTICIPANTS_TABLE + "_"
    ...
    /**
     * Get a (singleton) instance of {@link DatabaseHelper}, creating one if there
     * isn't one yet.
     * This is the only public method for getting a new instance of the class.
     *
     * @param context Should be the application context (or something that will live
     *                for the
    ...
    /**
     * Test method that always instantiates a new DatabaseHelper instance. This
     * should
     ...
     * <p>
     * The database is always opened as a writeable database.
     *
     * @return The current (or a new) DatabaseWrapper instance.
     */
    @DoesNotRunOnMainThread
    DatabaseWrapper getDatabase() {
        // We prevent the main UI thread from accessing the database here since we have
        // to allow
    ...
    private static void dropAllTables(final SQLiteDatabase db) {
        final Cursor tableCursor = db.query(PRIMARY_TABLE, PRIMARY_COLUMNS, "type='table'", null, null, null, null);
        if (tableCursor != null) {
            try {
    ...
    private static void dropAllTriggers(final SQLiteDatabase db) {
        final Cursor triggerCursor = db.query(PRIMARY_TABLE, PRIMARY_COLUMNS, "type='trigger'", null, null, null, null);
    ...
    public static void dropAllViews(final SQLiteDatabase db) {
        final Cursor viewCursor = db.query(PRIMARY_TABLE, PRIMARY_COLUMNS, "type='view'", null, null, null, null);
        if (viewCursor != null) {
            try {
    ...
    private static void dropAllIndexes(final SQLiteDatabase db) {
        final Cursor indexCursor = db.query(PRIMARY_TABLE, PRIMARY_COLUMNS, "type='index'", null, null, null, null);
        if (indexCursor != null) {
            try {
    ...
    private static void createDatabase(final SQLiteDatabase db) {
        ...
        // Add the default self participant. The default self will be assigned a proper
        // slot id
