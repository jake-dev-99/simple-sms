...
public class SnackBar {
    ...
    /**
     * Defines an action to be performed when the user clicks on the action button
     * on the snack bar
     */
    ...
    public static class Builder {
        private static final List<SnackBarInteraction> NO_INTERACTIONS = new ArrayList<SnackBarInteraction>();
        ...
        // The parent view is only used to get a window token and doesn't affect the
        // layout
        ...
        /**
         * Sets the components that this toast's animation will interact with. These
         * components may
    ...
    private void setUpButton() {
        if (mAction == null || mAction.getActionRunnable() == null) {
            ...
            // In the XML layout we add left/right padding to the button to add space
            // between
            // the message text and the button and on the right side we add padding to put
            // space
            // between the button and the edge of the snack bar. This is so the button can
            // use the
            // padding area as part of it's click target. Since we have no button, we need
            // to put
            // some margin on the right side. While the left margin is already set on the
            // wrapper,
