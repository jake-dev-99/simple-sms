...
/**
 * Wraps around the functionalities to allow the user to pick an
 * image/video/audio from the document
 * picker. Instances of this class must be tied to a Fragment which is able to
 * delegate activity
 * result callbacks.
 */
public class DocumentImagePicker {
    ...
    public interface SelectionListener {
        /**
         * Called when an document is selected from picker. At this point, the file
         * hasn't been
         * actually loaded and staged in the temp directory, so we are passing in a
         * pending
         * MessagePartData, which the consumer should use to display a placeholder
         * image.
         * 
         * @param pendingItem a temporary attachment data for showing the placeholder
         *                    state.
         */
    ...
    public void onActivityResult(final int requestCode, final int resultCode, final Intent data) {
        ...
        // Guard against null uri cases for when the activity returns a null/invalid
        // intent.
        if (url != null) {
    ...
    private void prepareDocumentForAttachment(final Uri documentUri) {
        ...
        new SafeAsyncTask<Void, Void, String>() {
            ...
            protected void onPostExecute(final String contentType) {
                ...
                final PendingAttachmentData pendingItem = PendingAttachmentData.createPendingAttachmentData(contentType,
                        documentUri);
