...
/**
 * Produces and holds a list view and its tab header to be displayed in a
 * ViewPager.
 */
public abstract class CustomHeaderPagerListViewHolder extends BasePagerViewHolder
    ...
    public void onContactsCursorUpdated(final Cursor data) {
        ...
        if (!mListCursorInitialized) {
            // We set the emptyView here instead of in create so that the initial load won't
            // show
     ...
     * Due to timing when data is loaded, the view may not be ready (and vice
     * versa). So we
     ...
     * individual list
     * view items so that they can be properly tracked by the scene transition
     * manager.
     * 
