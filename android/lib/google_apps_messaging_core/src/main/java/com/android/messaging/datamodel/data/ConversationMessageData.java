...
/**
 * Class representing a message within a conversation sequence. The message
 * parts
 * are available via the getParts() method.
 *
 * TODO: See if we can delegate to MessageData for the logic that this class
 * duplicates
 * (e.g. getIsMms).
 */
public class ConversationMessageData {
    ...
    /**
     * Takes a group-concated and quoted string and decomposes it into its
     * constituent
     * parts. A quoted string starts and ends with a single quote. Actual single
     * quotes
     * within the string are escaped using a second single quote. So, for example,
     * an
     ...
     * ignoring the outer quotes and the divider and replacing any pair of
     * consecutive
                ...
                while (++characterPos < length) {
                    ...
                    if (currentChar == QUOTE_CHAR) {
                        final char peekAhead = characterPos < length - 1
                                ? inputString.charAt(characterPos + 1)
                                : 0;
    ...
    public String getText() {
        ...
        // For single text part, we just return the part without creating the
        // StringBuilder
        String firstTextPart = null;
        boolean foundText = false;
        // For multiple text parts, we need the StringBuilder and the separator for
        // concatenation
    ...
    public boolean getCanForwardMessage() {
        // Even for outgoing messages, we only allow forwarding if the message has
        // finished sending
    ...
    /*
     * Generate a sqlite snippet to call the quote function on the columnName
     * argument.
    ...
    private static final String CONVERSATION_MESSAGE_VIEW_PARTS_COUNT = "count(" + DatabaseHelper.PARTS_TABLE + '.'
            + PartColumns._ID + ")";

    private static final String EMPTY_STRING = "";

    private static final String CONVERSATION_MESSAGES_QUERY_PROJECTION_SQL = DatabaseHelper.MESSAGES_TABLE + '.'
            + MessageColumns._ID
    ...
    private static final String CONVERSATION_MESSAGES_QUERY_FROM_WHERE_SQL = " FROM " + DatabaseHelper.MESSAGES_TABLE
    ...
    // This query is mostly static, except for the injection of conversation id.
    // This is for
    // performance reasons, to ensure that the query uses indices and does not
    // trigger full scans
    ...
    private static final String CONVERSATION_MESSAGE_IDS_PROJECTION_SQL = DatabaseHelper.MESSAGES_TABLE + '.'
            + MessageColumns._ID
            + " as " + ConversationMessageViewColumns._ID + " ";
    ...
    // Note that we sort DESC and ConversationData reverses the cursor. This is a
    // performance
    // issue (improvement) for large cursors.
    private static final String CONVERSATION_MESSAGES_QUERY_SQL_GROUP_BY = " GROUP BY " + DatabaseHelper.PARTS_TABLE
            + '.' + PartColumns.MESSAGE_ID
            + " ORDER BY "
            + DatabaseHelper.MESSAGES_TABLE + '.' + MessageColumns.RECEIVED_TIMESTAMP + " DESC";

    private static final String NOTIFICATION_QUERY_SQL_GROUP_BY = " GROUP BY " + DatabaseHelper.PARTS_TABLE + '.'
            + PartColumns.MESSAGE_ID
            + " ORDER BY "
            + DatabaseHelper.MESSAGES_TABLE + '.' + MessageColumns.RECEIVED_TIMESTAMP + " DESC";
