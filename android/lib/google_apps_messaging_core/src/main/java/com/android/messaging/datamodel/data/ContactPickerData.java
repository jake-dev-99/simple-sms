 ...
 * The caller is responsible for ensuring that the app has READ_CONTACTS
 * permission (see
 * {@link ContactUtil#hasReadContactsPermission()}) before instantiating this
 * class.
 */
public class ContactPickerData extends BindableData implements
    ...
    public void onLoadFinished(final Loader<Cursor> loader, final Cursor data) {
        ...
        if (isBound(cursorLoader.getBindingId())) {
            ...
            if (loader.getId() != PARTICIPANT_LOADER) {
                // The frequent contacts cursor to be used in the UI depends on results from
                // both
                // all contacts and frequent contacts loader, and we don't know which will
                // finish
    ...
    public static boolean isTooManyParticipants(final int participantCount) {
        // When creating a conversation, the conversation will be created using the
        // system's
