...
/**
 * A immutable wrapper around a Binding object. Callers can only access readonly
 * methods like
 * getData(), isBound() and ensureBound() but not bind() and unbind(). This is
 * used for MVC pattern
 * where both the View and the Controller needs access to a centrally bound
 * Model object. The View
 * is the one that owns the bind/unbind logic of the Binding, whereas controller
 * only serves as a
 * consumer.
 */
public class ImmutableBindingRef<T extends BindableData> extends BindingBase<T> {
    ...
    /**
     * Resolve the source binding to the real BindingImpl it's referencing. This
     * avoids the
     * redundancy of multiple wrapper calls when creating a binding reference from
     * an existing
     * binding reference.
     */
