...
/**
 * Class for creating / loading / reusing bitmaps. This class allow the user to
 * create a new bitmap,
 * reuse an bitmap from the pool and to return a bitmap for future reuse. The
 * pool of bitmaps
 * allows for faster decode and more efficient memory usage.
 * Note: consumers should not create BitmapPool directly, but instead get the
 * pool they want from
...
public class BitmapPool implements MemoryCache {
    ...
    /**
     * Overall pool data structure which currently only supports rectangular
     * bitmaps. The size of
    ...
    /**
     * Inner structure which holds a pool of bitmaps all the same size (i.e. all
     * have the same
    ...
    /**
     * Creates a pool of reused bitmaps with helper decode methods which will
     * attempt to use the
     * reclaimed bitmaps. This will help speed up the creation of bitmaps by using
     * already allocated
     * bitmaps.
     * 
     * @param maxSize The overall max size of the pool. When the pool exceeds this
     *                size, all calls
     *                to reclaimBitmap(Bitmap) will result in recycling the bitmap.
     * @param name    Name of the bitmap pool and only used for logging. Can not be
     *                null.
     */
    ...
    /**
     * @return The pool key for the provided image dimensions or 0 if either width
     *         or height is
    ...
    /**
     *
     * @return A bitmap in the pool with the specified dimensions or null if no
     *         bitmap with the
    ...
    /**
     * Internal function to try and find a bitmap in the pool which matches the
     * desired width and
     ...
     * 
     * @param optionsTmp The BitmapFactory.Options to update with the bitmap for the
     *                   system to try
     *                   to reuse.
    ...
    /**
     * Load a resource into a bitmap. Uses a bitmap from the pool if possible to
     * reduce memory
     * turnover.
     * 
     ...
     *                   getBitmapOptionsForPool(). Cannot
     *                   be null.
    ...
    /**
     * Load an input stream into a bitmap. Uses a bitmap from the pool if possible
     * to reduce memory
     * turnover.
     * 
    ...
    /**
     * Turn encoded bytes into a bitmap. Uses a bitmap from the pool if possible to
     * reduce memory
     * turnover.
     * 
     * @param bytes      Encoded bytes to draw on the bitmap. Cannot be null.
     * @param optionsTmp The bitmap will set here and the input should be generated
     *                   from
    ...
    /**
     * Creates a bitmap with the given size, this will reuse a bitmap in the pool,
     * if one is
     ...
     * @return A bitmap with the desired width and height, this maybe a reused
     *         bitmap from the pool.
     */
    ...
    /**
     * Called when a bitmap is finished being used so that it can be used for
     * another bitmap in the
     * future or recycled. Any bitmaps returned should not be used by the caller
     * again.
     * 
     * @param b The bitmap to return to the pool for future usage or recycled. This
     *          cannot be null.
     */
