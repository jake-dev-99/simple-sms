...
public class PerSubscriptionSettingsActivity extends BugleActionBarActivity {
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        if (!TextUtils.isEmpty(title)) {
            ...
            // This will fall back to the default title, i.e. "Messaging settings," so
            // No-op.
        }
    ...
    public static class PerSubscriptionSettingsFragment extends PreferenceFragment
        ...
        public void onCreate(final Bundle savedInstanceState) {
            ...
            final PreferenceCategory advancedCategory = (PreferenceCategory) findPreference(
                    getString(R.string.advanced_category_pref_key));
            final PreferenceCategory mmsCategory = (PreferenceCategory) findPreference(
                    getString(R.string.mms_messaging_category_pref_key));
            ...
            if (!MmsManager.shouldUseLegacyMms()
                ...
                // 1) Remove the ability to edit the local APN prefs if it doesn't use legacy
                // APIs.
                // 2) Don't remove the ability to edit the local APN prefs if this device lets
                // us
            ...
            // We want to disable preferences if we are not the default app, but we do all
            // of the
            // above first so that the user sees the correct information on the screen
            if (!PhoneUtils.getDefault().isDefaultSmsApp()) {
                mGroupMmsPreference.setEnabled(false);
                final Preference autoRetrieveMmsPreference = findPreference(
                        getString(R.string.auto_retrieve_mms_pref_key));
                autoRetrieveMmsPreference.setEnabled(false);
                final Preference deliveryReportsPreference = findPreference(
                        getString(R.string.delivery_reports_pref_key));
        ...
        private boolean isCellBroadcastAppLinkEnabled() {
            ...
            try {
                ...
                return pm.getApplicationEnabledSetting(
                        UIIntents.CMAS_COMPONENT) != PackageManager.COMPONENT_ENABLED_STATE_DISABLED;
        ...
        private void updateGroupMmsPrefSummary() {
            ...
            mGroupMmsPreference.setSummary(groupMmsEnabled ? R.string.enable_group_mms : R.string.disable_group_mms);
        }
