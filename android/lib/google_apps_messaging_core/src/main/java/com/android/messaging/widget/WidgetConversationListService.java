    ...
    private static class WidgetConversationListFactory extends BaseWidgetFactory {
        ...
        public RemoteViews getViewAt(int position) {
            ...
            synchronized (sWidgetLock) {
                ...
                remoteViews.setImageViewBitmap(R.id.avatarView, includeAvatar ? getAvatarBitmap(iconUri) : null);

                // Error
                // Only show the fail icon if it is not a group conversation.
                // And also require that we be the default sms app.
                final boolean showError = conv.getIsFailedStatus() && !conv.getIsGroup() && isDefaultSmsApp;
                ...
                remoteViews.setViewVisibility(R.id.conversation_failed_status_icon,
                        showError && includeAvatar ? View.VISIBLE : View.GONE);
        ...
        private String getSnippetText(final ConversationListItemData conv) {
            String snippetText = conv.getShowDraft() ? conv.getDraftSnippetText() : conv.getSnippetText();
            final String previewContentType = conv.getShowDraft() ? conv.getDraftPreviewContentType()
                    : conv.getPreviewContentType();
        ...
        /**
         * @return the "View more conversations" view. When the user taps this item,
         *         they're
        ...
        protected RemoteViews getViewMoreItemsView() {
            ...
            // Tapping this "More conversations" item should take us to the
            // ConversationList.
            // However, the list view is primed with an intent to go to the Conversation
            // activity.
            // Each normal conversation list item sets the fill-in intent with the
            // ConversationId for that particular conversation. In other words, the only
            // place
