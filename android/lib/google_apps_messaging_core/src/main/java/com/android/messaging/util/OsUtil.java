...
public class OsUtil {
    ...
    /**
     * @return True if the version of Android that we're running on is at least Ice
     *         Cream Sandwich
     *         MR1 (API level 15).
     */
    ...
    /**
     * @return True if the version of Android that we're running on is at least
     *         Jelly Bean
     *         (API level 16).
     */
    ...
    /**
     * Check if the app has the specified permission. If it does not, the app needs
     * to use
     * . Note that if it
     * returns true, it cannot return false in the same process as the OS kills the
     * process when
    ...
    public static boolean hasPermission(final String permission) {
        if (OsUtil.isAtLeastM()) {
            // It is safe to cache the PERMISSION_GRANTED result as the process gets killed
            // if the
            // user revokes the permission setting. However, PERMISSION_DENIED should not be
            // cached as the process does not get killed if the user enables the permission
            // setting.
    ...
    /**
     * Returns array with the set of permissions that have not been granted from the
     * given set.
     * The array will be empty if the app has all of the specified permissions. Note
     * that calling
     * for an already granted permission can prompt the user
