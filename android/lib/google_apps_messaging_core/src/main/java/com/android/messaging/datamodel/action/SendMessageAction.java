...
/**
 * Action used to send an outgoing message. It writes MMS messages to the
 * telephony db
 * ({@link InsertNewMessageAction}) writes SMS messages to the telephony db). It
 * also
 * initiates the actual sending. It will all be used for re-sending a failed
 * message.
 * <p>
 * This class is public (not package-private) because the SMS/MMS (e.g.
 * MmsUtils) classes need to
...
public class SendMessageAction extends Action implements Parcelable {
    ...
    /**
     * Queue sending of existing message (can only be called during execute of
     * action)
     */
    ...
    /**
     * Constructor used for retrying sending in the background (only message id
     * available)
     */
    ...
    private boolean queueAction(final String messageId, final Action processingAction) {
        ...
        if (message != null && message.canSendMessage()) {
            ...
            final ArrayList<String> recipients = BugleDatabaseOperations.getRecipientsForConversation(db,
                    conversationId);
    ...
    protected Bundle doBackgroundWork() {
        ...
        if (isSms) {
            ...
            final ArrayList<String> recipients = actionParameters.getStringArrayList(KEY_RECIPIENTS);
            if (messageUri == null) {
    ...
    protected Object processBackgroundFailure() {
        ...
        final int httpStatusCode = actionParameters.getInt(ProcessSentMessageAction.KEY_HTTP_STATUS_CODE);
     ...
     * @param updatedMessageUri message uri for newly-inserted messages; null
     *                          otherwise
     * @param clearSeen         whether the message 'seen' status should be reset if
     *                          error occurs
     */
    public static boolean updateMessageAndStatus(final boolean isSms, final MessageData message,
        ...
        // 'SENT' even before they actually are. We should technically be using QUEUED
        // or OUTBOX
        // instead, but if we do that, it's possible that the Messaging app will try to
        // send them
        ...
        // InsertNewMessageAction.insertBroadcastSmsMessage and insertSendingSmsMessage
        // (should be
    ...
    public static final Parcelable.Creator<SendMessageAction> CREATOR = new Parcelable.Creator<SendMessageAction>() {
        @Override
