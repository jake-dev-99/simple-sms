...
/**
 * A cursor builder that takes the frequent contacts cursor and aggregate it
 * with the all contacts
 * cursor to fill in contact details such as phone numbers and strip away
 * invalid contacts.
 *
 * Because the frequent contact list depends on the loading of two cursors, it
 * needs to temporarily
 * store the cursor that it receives with setFrequents() and setAllContacts()
 * calls. Because it
 * doesn't know which one will be finished first, it always checks whether both
 * cursors are ready
 * to pull data from and construct the aggregate cursor when it's ready to do
 * so. Note that
 * this cursor builder doesn't assume ownership of the cursors passed in - it
 * merely references
 * them and always does a isClosed() check before consuming them. The ownership
 * still belongs to
...
public class FrequentContactsCursorBuilder {
    ...
    /**
     * Sets the frequent contacts cursor as soon as it is loaded, or null if it's
     * reset.
     * 
    ...
    /**
     * Attempt to build the cursor records from the frequent and all contacts cursor
     * if they
     * are both ready to be consumed.
     * 
     * @return the frequent contact cursor if built successfully, or null if it
     *         can't be built yet.
     */
    public Cursor build() {
        if (mFrequentContactsCursor != null && mAllContactsCursor != null) {
            ...
            // Frequent contacts cursor has one record per contact, plus it doesn't contain
            // info
            // such as phone number and type. In order for the records to be usable by
            // Bugle, we
            ...
            // First, go through the frequents cursor and take note of all lookup keys and
            // their
            ...
            // contacts list only contains phone contacts, this step will ensure that we
            // filter
            ...
            while (mAllContactsCursor.moveToNext()) {
                ...
                if (lookupKeyToRankMap.containsKey(lookupKey)) {
                    ...
                    row[ContactUtil.INDEX_DISPLAY_NAME] = mAllContactsCursor.getString(ContactUtil.INDEX_DISPLAY_NAME);
                    row[ContactUtil.INDEX_PHOTO_URI] = mAllContactsCursor.getString(ContactUtil.INDEX_PHOTO_URI);
                    row[ContactUtil.INDEX_PHONE_EMAIL] = mAllContactsCursor.getString(ContactUtil.INDEX_PHONE_EMAIL);
            ...
            // Now we have a list of rows containing frequent contacts in alphabetical
            // order.
            // Therefore, sort all the rows according to their actual ranks in the frequents
            // list.
