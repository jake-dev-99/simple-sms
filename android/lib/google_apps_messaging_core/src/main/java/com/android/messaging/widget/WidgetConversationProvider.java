...
public class WidgetConversationProvider extends BaseWidgetProvider {
    ...
    public static void rebuildWidget(final Context context, final int appWidgetId) {
        ...
        if (!isWidgetConfigured(appWidgetId)) {
            ...
            final String conversationId = WidgetPickConversationActivity.getConversationIdPref(appWidgetId);
            final boolean isMainThread = Looper.myLooper() == Looper.getMainLooper();
            // If we're running on the UI thread, we can't do the DB access needed to get
            // the
            // conversation data. We'll do excute this again off of the UI thread.
            final ConversationListItemData convData = isMainThread ? null
                    : getConversationData(context, conversationId);
            ...
            remoteViews.setTextViewText(R.id.widget_label,
                    convData != null ? convData.getName() : context.getString(R.string.app_name));
            ...
            // Note: the template intent has to be a "naked" intent without any extras. It
            // turns out
            // that if the template intent does have extras, those particular extras won't
            // get
    ...
    /*
     * notifyConversationDeleted is called when a conversation is deleted. Look
     * through all the
     * widgets and if they're displaying that conversation, force the widget into
     * its
     * configuration state.
     */
    public static void notifyConversationDeleted(final Context context,
        ...
        for (final int appWidgetId : appWidgetManager.getAppWidgetIds(new ComponentName(context,
            ...
            final String widgetConvId = WidgetPickConversationActivity.getConversationIdPref(appWidgetId);
    ...
    /*
     * notifyConversationRenamed is called when a conversation is renamed. Look
     * through all the
     * widgets and if they're displaying that conversation, force the widget to
     * rebuild itself
     */
    ...
    public void onReceive(final Context context, final Intent intent) {
        ...
        // The base class AppWidgetProvider's onReceive handles the normal widget
        // intents. Here
        // we're looking for an intent sent by our app when it knows a message has
        // been sent or received (or a conversation has been read) and is telling the
        // widget it
        // needs to update.
        if (getAction().equals(action)) {
            ...
            // Normally the conversation id points to a specific conversation and we only
            // update
            // widgets looking at that conversation. When the conversation id is null, that
            // means
            // there's been a massive change (such as the initial import) and we need to
            // update
            ...
            for (final int widgetId : appWidgetIds) {
                // Retrieve the persisted information for this widget from preferences.
                final String widgetConvId = WidgetPickConversationActivity.getConversationIdPref(widgetId);
    ...
    /**
     * When this widget is created, it's created for a particular conversation and
     * that
     * ConversationId is stored in shared prefs. If the associated conversation is
     * deleted,
     * the widget doesn't get deleted. Instead, it goes into a "tap to configure"
     * state. This
     * function determines whether the widget has been configured and has an
     * associated
     * ConversationId.
     */
