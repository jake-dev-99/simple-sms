 ...
 * <ol>
 * <li>Threading policy choice - Users of this class should use the explicit API
 * instead of
 * {@link #execute} which uses different threading policy on different OS
 * versions.
...
public abstract class SafeAsyncTask<Params, Progress, Result>
    ...
    /**
     * @param maxTimeMillis            maximum expected time for the background
     *                                 operation. This is just
     *                                 a diagnostic tool to catch unexpectedly long
     *                                 operations. If an operation does take
     *                                 longer than expected, it is fine to increase
     *                                 this argument. If the value is larger
     *                                 than a minute, you should consider using a
     *                                 dedicated thread so as not to interfere
     *                                 with other AsyncTasks.
     *
     *                                 <p>
     *                                 Use {@link #UNBOUNDED_TIME} if you do not
     *                                 know the maximum expected time. This
     *                                 is strongly discouraged as it can block other
     *                                 AsyncTasks indefinitely.
     *
     * @param cancelExecutionOnTimeout whether to attempt to cancel the task
     *                                 execution on timeout.
     *                                 If this is set, at execution timeout we will
     *                                 call cancel(), so doInBackgroundTimed()
     *                                 should periodically check if the task is to
     *                                 be cancelled and finish promptly if
     *                                 possible, and handle the cancel event in
     *                                 onCancelled(). Also, at the end of execution
     *                                 we will not crash the execution if it went
     *                                 over limit since we explicitly canceled it.
     */
    ...
    protected final Result doInBackground(final Params... params) {
        // This enforces that executeOnThreadPool was called, not execute. Ideally, we
        // would
        // make execute throw an exception, but since it is final, we cannot override
        // it.
     ...
     *
     * To enable wakelock during the execution, see
     * {@link #executeOnThreadPool(Runnable, boolean)}
     *
     ...
     *
     * @param runnable     The Runnable to execute asynchronously
     * @param withWakeLock when set, a wake lock will be held for the duration of
     *                     the runnable
     *                     execution
     */
