...
public class DownloadMmsAction extends Action implements Parcelable {
    ...
    /**
     * Queue download of an mms notification message (can only be called during
     * execute of action)
     */
    ...
    // Values we attach to the pending intent that's fired when the message is
    // downloaded.
    ...
    protected boolean queueAction(final String messageId, final Action processingAction) {
        ...
        if (message != null && message.canDownloadMessage()) {
            ...
            if (message.getInDownloadWindow(now)) {
                ...
                // Pre-compute the next status when failed so we don't have to load from db
                // again
                ...
                // Retries depleted and we failed. Update the message status so we won't retry
                // again
                updateMessageStatus(notificationUri, messageId, conversationId,
    ...
    /**
     * Get the corresponding downloading status based on the starting status of the
     * message
     *
     * @param status The starting status of the message.
    ...
    protected Bundle doBackgroundWork() {
        ...
        final long receivedTimestampRoundedToSecond = 1000 * ((System.currentTimeMillis() + 500) / 1000);

        LogUtil.i(TAG, "DownloadMmsAction: Downloading MMS message " + messageId
                + " (" + (autoDownload ? "auto" : "manual") + ")");

        // Bundle some values we'll need after the message is downloaded (via platform
        // APIs)
    ...
    protected Object processBackgroundResponse(final Bundle response) {
        // Nothing to do here; post-download actions handled by
        // ProcessDownloadedMmsAction
        return null;
    }
    ...
    static void updateMessageStatus(final Uri messageUri, final String messageId,
        ...
        final Context context = Factory.get().getApplicationContext();
        // Downloading status just kept in local DB but need to fix up telephony DB
        // first
    ...
    public static final Parcelable.Creator<DownloadMmsAction> CREATOR = new Parcelable.Creator<DownloadMmsAction>() {
        @Override
