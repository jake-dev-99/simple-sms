...
public class AvatarRequest extends UriImageRequest<AvatarRequestDescriptor> {
    ...
    /**
     * We can load multiple types of images for avatars depending on the uri. The
     * uri should be
     * built by {@link AvatarUriUtil} which will decide on
     * what uri to build based on the available profile photo and name. Here we will
     * check if the
     * image is a local resource (ie profile photo uri), if the resource isn't a
     * local one we will
    ...
    protected ImageResource loadMediaInternal(List<MediaRequest<ImageResource>> chainedTasks)
        ...
        if (bitmap == null) {
            ...
            if (isLocalResourceUri) {
                // If we are here, we just failed to load the local resource. Use the fallback
                // Uri
                // if possible.
    ...
    private Bitmap renderDefaultAvatar(final int width, final int height) {
        ...
        if (mDescriptor.isWearBackground) {
            ...
            defaultPerson = isLargeDefault ? sDefaultPersonBitmapLarge : sDefaultPersonBitmap;
        }
