...
/**
 * View that encapsulates a video preview (either as a thumbnail image, or video
 * player), and the
 * a play button to overlay it. Ensures that the video preview maintains the
 * aspect ratio of the
 * original video while trying to respect minimum width/height and constraining
 * to the available
 * bounds
 */
public class VideoThumbnailView extends FrameLayout {
    /**
     * When in this mode the VideoThumbnailView is a lightweight AsyncImageView with
     * an ImageButton
    ...
    /**
     * When in this mode the VideoThumbnailVideo will include a VideoView, and the
     * play button will
     * play the video inline. When in this mode, the loop and playOnLoad attributes
     * can be applied
    ...
    public VideoThumbnailView(final Context context, final AttributeSet attrs) {
        super(context, attrs);
        final TypedArray typedAttributes = context.obtainStyledAttributes(attrs, R.styleable.VideoThumbnailView);
        ...
        final boolean loop = typedAttributes.getBoolean(R.styleable.VideoThumbnailView_loop, false);
        ...
        if (mMode == MODE_PLAYABLE_VIDEO) {
            ...
            // Video view tries to request focus on start which pulls focus from the user's
            // intended
    ...
    // TODO: The check could be added to MessagePartData itself so that all users of
    // MessagePartData
    // get the right behavior, instead of requiring all the users to do similar
    // checks.
