...
/**
 * Custom layout panel which makes the MediaPicker animations seamless and
 * synchronized
 * Designed to be very specific to the MediaPicker's usage
 */
public class MediaPickerPanel extends ViewGroup {
    /**
     * The window of time in which we might to decide to reinterpret the intent of a
     * gesture.
     */
    ...
    /**
     * The current desired height of the MediaPicker. This property may be animated
     * and the
    ...
    /**
     * The action bar height used to compute the padding on the view pager when it's
     * full screen.
     */
    ...
    protected void onFinishInflate() {
        ...
        // Make sure full screen mode is updated in landscape mode change when the panel
        // is open.
    ...
    protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
        ...
        if (mExpanded && desiredHeight == 0) {
            // If we want to be shown, we have to have a non-0 height. Returning a height of
            // 0 will
            // cause the framework to abort the animation from 0, so we must always have
            // some
        ...
        // If we are animating and have an interim desired height, use the default
        // height. We can't
        // take the max here as on some devices the mDefaultViewPagerHeight may be too
        // big in
        // landscape mode after animation.
        final int tabAdjustedDesiredHeight = desiredHeight - tabStripHeight;
        final int viewPagerHeight = tabAdjustedDesiredHeight <= 1 ? mDefaultViewPagerHeight : tabAdjustedDesiredHeight;
    ...
    /**
     * Get the desired height of the media picker panel for when the panel is not in
     * motion (i.e.
    ...
    private int getDesiredHeight() {
        if (mFullScreen) {
            ...
            if (OsUtil.isAtLeastKLP() && isAttachedToWindow()) {
                // When we're attached to the window, we can get an accurate height, not
                // necessary
                // on older API level devices because they don't include the action bar height
                View composeContainer = getRootView().findViewById(R.id.conversation_and_compose_container);
    ...
    /**
     * Expand the media picker panel. Since we always set the pager adapter to null
     * when the panel
    ...
    private void setExpanded(final boolean expanded, final boolean animate, final int startingPage,
        ...
        // Call setFullScreenView() when we are in landscape mode so it can go full
        // screen as
    ...
    private class TouchHandler implements OnTouchListener {
        ...
        // The system defined pixel size to determine when a movement is considered a
        // drag.
        ...
        /**
         * Whether we are currently moving down. We may not be able to move down in full
         * screen
        ...
        /**
         * Indicates whether the child view contained in the panel can swipe down at the
         * beginning
         * of the drag event (i.e. the initial down). The MediaPanel can contain
         * scrollable children such as a list view / grid view. If the child view can
         * swipe down,
         * We want to let the child view handle the scroll first instead of handling it
         * ourselves.
         */
        private boolean mCanChildViewSwipeDown = false;

        /**
         * Necessary direction ratio for a fling to be considered in one direction this
         * prevents
         * horizontal swipes with small vertical components from triggering vertical
         * swipe actions
         */
        ...
        /**
         * The media picker panel may contain scrollable children such as a GridView,
         * which eats
         * all touch events before we get to it. Therefore, we'd like to intercept these
         * events
         * before the children to determine if we should handle swiping down in full
         * screen mode.
         * In non-full screen mode, we should handle all vertical scrolling events and
         * leave
        ...
        /**
         * Determine whether we think the user is actually trying to expand or slide
         * despite the
        ...
        private boolean shouldAllowRecaptureTouch(MotionEvent ev) {
            ...
            if (mDownEvent == null || elapsedMs == 0 || elapsedMs > TOUCH_RECAPTURE_WINDOW_MS) {
                // Either we don't have info to decide or it's been long enough that we no
                // longer
        ...
        public boolean onTouch(final View view, final MotionEvent motionEvent) {
            switch (motionEvent.getAction()) {
                case MotionEvent.ACTION_UP: {
                    ...
                    final float dt = (motionEvent.getEventTime() - mDownEvent.getEventTime()) / 1000.0f;
