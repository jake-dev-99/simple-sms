...
/**
 * Holds and displays multiple attachments in a 4x2 grid. Each preview image
 * "tile" can take
 * one of three sizes - small (1x1), wide (2x1) and large (2x2). We have a
 * number of predefined
 * layout settings designed for holding 2, 3, 4+ attachments (these layout
 * settings are
 * tweakable by design request to allow for max flexibility). For a visual
 ...
 * +---------------+-------+--------+
 *
 * In the above example, the layout consists of four tiles, A-D. A is a large
 * tile, B is a
 * wide tile and C & D are both small tiles. A starts at (0,0) and ends at
 * (1,1), B starts at
 * (2,0) and ends at (3,0), and so on. In our layout class we'd have these tiles
 * in the order
 * of A-D, so that we make sure the last tile is always the one where we can put
 * the overflow
 * indicator (e.g. "+2").
 */
public class MultiAttachmentLayout extends FrameLayout {
    ...
    /**
     * A layout simply contains a list of tiles, in the order of top-left ->
     * bottom-right.
     */
    ...
    private void buildViews(final Iterable<MessagePartData> attachments,
        ...
        while (iterator.hasNext() && i < count) {
            ...
            // The first view will animate in using PopupTransitionAnimation, but the
            // remaining
            // views will slide from their previous position to their new position within
            // the
            // layout
            if (i == 0) {
        ...
        // Build the plus text view (e.g. "+2") for when there are more attachments than
        // what
    ...
    protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (view instanceof AsyncImageView) {
                final ImageRequestDescriptor imageRequest = AttachmentPreviewFactory
