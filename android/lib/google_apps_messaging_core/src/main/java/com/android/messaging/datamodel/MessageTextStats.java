...
public class MessageTextStats {
    ...
    public void updateMessageTextStats(final int selfSubId, final String messageText) {
        ...
        if (smsToMmsLengthThreshold > 0) {
             ...
             * Korean or Chinese, the max length is SmsMessage.MAX_USER_DATA_BYTES (140)
             * bytes
             * (70 code units).
             * Here we check if the total code units we can use is smaller than 140. If so,
             * we know we should count threshold in double-byte, so divide the threshold by
             * 2.
             * In this way, we will count Korean text correctly with regard to the length
             * threshold.
             */
            if (usedInCurrentMessage + mCodePointsRemainingInCurrentMessage < SmsMessage.MAX_USER_DATA_BYTES) {
