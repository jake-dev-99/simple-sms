...
/**
 * Container class for holding a bitmap resource used by the
 * MediaResourceManager. This resource
 * can both be cached (albeit not very storage-efficiently) and directly used by
 * the UI.
 */
public class DecodedImageResource extends ImageResource {
    ...
    /**
     * Attempt to reuse the bitmap in the image resource and repurpose it for
     * something else.
     * After this, the image resource will relinquish ownership on the bitmap
     * resource so that
    ...
    /**
     * Gets the orientation of the image as one of the ExifInterface.ORIENTATION_*
     * constants
     */
    @Override
        ...
        public ImageResource loadMediaBlocking(List<MediaRequest<ImageResource>> chainedRequests)
            ...
            try {
                ...
                if (bitmapWidth > 0 && bitmapHeight > 0 &&
                    ...
                    final ImageRequestDescriptor descriptor = ((ImageRequest<?>) mOriginalImageRequest).getDescriptor();
                    ...
                    if (targetScale < 1.0f && targetWidth > 0 && targetHeight > 0 &&
                        ...
                        scaledBitmap = bitmap = Bitmap.createScaledBitmap(bitmap, targetWidth, targetHeight, false);
                    }
                }
