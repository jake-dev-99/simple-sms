...
/**
 * Keeps track of the different UI states that the ConversationActivity may be
 * in. This acts as
 * a state machine which, based on different actions (e.g.
 * onAddMoreParticipants), notifies the
 * ConversationActivity about any state UI change so it can update the visuals.
 * This class
 * implements Parcelable and it's persisted across activity tear down and
 * relaunch.
 */
public class ConversationActivityUiState implements Parcelable, Cloneable {
    interface ConversationActivityUiStateHost {
        void onParticipantPickerUiStateChanged(int oldState, int newState, boolean animate);
    }
    ...
    /**
     * Only a full screen contact picker is showing asking user to pick the initial
     * contact.
     */
    public static final int STATE_CONTACT_PICKER_ONLY_INITIAL_CONTACT = 2;
    /**
     * Only a full screen contact picker is showing asking user to pick more
     * participants. This
     * happens after the user picked the initial contact, and then decide to go back
     * and add more.
     */
    ...
    /**
     * A hybrid mode where the conversation view + contact chips view are showing.
     * This happens
     * right after the user picked the initial contact for which a 1-1 conversation
     * is fetched or
     * created.
     */
    ...
    private int mParticipantUiState;
    ...
    // ConversationFragment is attached. This is a transient state that's not
    // persisted as
    // part of the parcelable.
    private boolean mPendingResumeComposeMessage = false;

    // The owner ConversationActivity. This is not parceled since the instance
    // always change upon
    // object reuse.
    private ConversationActivityUiStateHost mHost;

    // Indicates the owning ConverastionActivity is in the process of updating its
    // UI presentation
    // to be in sync with the UI states. Outside of the UI updates, the UI states
    // here should
    ...
    ConversationActivityUiState(final String conversationId) {
        ...
        mParticipantUiState = conversationId == null ? STATE_CONTACT_PICKER_ONLY_INITIAL_CONTACT
                : STATE_CONVERSATION_ONLY;
    }
    ...
    public boolean shouldShowConversationFragment() {
        return mParticipantUiState == STATE_HYBRID_WITH_CONVERSATION_AND_CHIPS_VIEW ||
                mParticipantUiState == STATE_CONVERSATION_ONLY;
    }

    public boolean shouldShowContactPickerFragment() {
        return mParticipantUiState == STATE_CONTACT_PICKER_ONLY_ADD_MORE_CONTACTS ||
                mParticipantUiState == STATE_CONTACT_PICKER_ONLY_MAX_PARTICIPANTS ||
                mParticipantUiState == STATE_CONTACT_PICKER_ONLY_INITIAL_CONTACT ||
                mParticipantUiState == STATE_HYBRID_WITH_CONVERSATION_AND_CHIPS_VIEW;
    }

    /**
     * Returns whether there's a pending request to resume message compose (i.e. set
     * focus to
     * the compose message view and show the soft keyboard). If so, this request
     * will be served
     * when the conversation fragment get created and resumed. This happens when the
     * user commits
     * participant selection for a group conversation and goes back to the
     * conversation fragment.
     * Since conversation fragment creation happens asynchronously, we issue and
     * track this
    ...
    public boolean shouldResumeComposeMessage() {
        if (mPendingResumeComposeMessage) {
            // This is a one-shot operation that just keeps track of the pending resume
            // compose
            // state. This is also a non-critical operation so we don't care about failure
            // case.
    ...
    /**
     * Called whenever the contact picker fragment successfully fetched or created a
     * conversation.
     */
    public void onGetOrCreateConversation(final String conversationId) {
        int newState = STATE_CONVERSATION_ONLY;
        if (mParticipantUiState == STATE_CONTACT_PICKER_ONLY_INITIAL_CONTACT) {
            newState = STATE_HYBRID_WITH_CONVERSATION_AND_CHIPS_VIEW;
        } else if (mParticipantUiState == STATE_CONTACT_PICKER_ONLY_ADD_MORE_CONTACTS ||
                mParticipantUiState == STATE_CONTACT_PICKER_ONLY_MAX_PARTICIPANTS) {
            newState = STATE_CONVERSATION_ONLY;
        } else {
            // New conversation should only be created when we are in one of the contact
            // picking
            // modes.
    ...
    /**
     * Called when the user started composing message. If we are in the hybrid chips
     * state, we
     * should commit to enter the conversation only state.
     */
    public void onStartMessageCompose() {
        // This cannot happen when we are in one of the full-screen contact picking
        // states.
        Assert.isTrue(mParticipantUiState != STATE_CONTACT_PICKER_ONLY_INITIAL_CONTACT &&
                mParticipantUiState != STATE_CONTACT_PICKER_ONLY_ADD_MORE_CONTACTS &&
                mParticipantUiState != STATE_CONTACT_PICKER_ONLY_MAX_PARTICIPANTS);
        if (mParticipantUiState == STATE_HYBRID_WITH_CONVERSATION_AND_CHIPS_VIEW) {
            performUiStateUpdate(STATE_CONVERSATION_ONLY, true);
        }
    }

    /**
     * Called when the user initiated an action to add more participants in the
     * hybrid state,
     * namely clicking on the "add more participants" button or entered a new
     * contact chip via
     * auto-complete.
     */
    ...
    /**
     * Called each time the number of participants is updated to check against the
     * limit and
     * update the ui state accordingly.
     */
    public void onParticipantCountUpdated(final boolean canAddMoreParticipants) {
        if (mParticipantUiState == STATE_CONTACT_PICKER_ONLY_ADD_MORE_CONTACTS
                && !canAddMoreParticipants) {
            performUiStateUpdate(STATE_CONTACT_PICKER_ONLY_MAX_PARTICIPANTS, false);
        } else if (mParticipantUiState == STATE_CONTACT_PICKER_ONLY_MAX_PARTICIPANTS
                && canAddMoreParticipants) {
    ...
    private void performUiStateUpdate(final int conversationContactState, final boolean animate) {
        // This starts one UI update cycle, during which we allow the conversation
        // activity's
        // UI presentation to be temporarily out of sync with the states here.
        beginUiUpdate();

        if (conversationContactState != mParticipantUiState) {
            final int oldState = mParticipantUiState;
            mParticipantUiState = conversationContactState;
            notifyOnOverallUiStateChanged(oldState, mParticipantUiState, animate);
        }
        endUiUpdate();
    }
    ...
    private void notifyOnOverallUiStateChanged(
        ...
        // Only do this if we are still attached to the host. mHost can be null if the
        // host
        // activity is already destroyed, but due to timing the contained UI components
        // may still
        // receive events such as focus change and trigger a callback to the Ui state.
        // We'd like
        // to guard against those cases.
        if (mHost != null) {
            mHost.onParticipantPickerUiStateChanged(oldState, newState, animate);
        }
    }

    private void assertValidState() {
        // Conversation id may be null IF AND ONLY IF the user is picking the initial
        // contact to
        // start a conversation.
        Assert.isTrue(
                (mParticipantUiState == STATE_CONTACT_PICKER_ONLY_INITIAL_CONTACT) == (mConversationId == null));
    }
    ...
    public void writeToParcel(final Parcel dest, final int flags) {
        dest.writeInt(mParticipantUiState);
        dest.writeString(mConversationId);
    }

    private ConversationActivityUiState(final Parcel in) {
        mParticipantUiState = in.readInt();
    ...
    public static final Parcelable.Creator<ConversationActivityUiState> CREATOR = new Parcelable.Creator<ConversationActivityUiState>() {
        @Override
    ...
    /**
     * allows for overridding the internal UI state. Should never be called except
     * by test code.
     */
    @VisibleForTesting
    void testSetUiState(final int uiState) {
        mParticipantUiState = uiState;
    }
}
