 ...
 * media resources.
 * Right now we don't utilize any disk cache as all media urls are expected to
 * be resolved to
 * local content.
 * <p/>
 *
 ...
 * MediaRequest, which may be a
 * regular request if the caller doesn't want to listen for events
 * (fire-and-forget),
 * or an async request wrapper if event callback is needed.</li>
 * <li>{@link #requestMediaResourceSync(MediaRequest)} which takes a
 * MediaRequest and synchronously
 ...
 * runs on a
 * dedicated thread, which calls MediaRequest.loadMediaBlocking() to perform the
 * actual media
 * loading work. As the media resources are loaded, MediaResourceManager
 * notifies the callers
 * (which must implement the MediaResourceLoadListener interface) via
 * onMediaResourceLoaded()
 * callback. Meanwhile, MediaResourceManager also pushes the loaded resource
 * onto its dedicated
 * cache.
 * </p>
 *
 ...
 * caches. They are
 * created on demand by the incoming MediaRequest's getCacheId() method. The
 * implementations of
 * MediaRequest (such as {@link ImageRequest}) get to determine the desired
 * cache id. For Bugle,
 ...
 * to be executed
 * after the completion of the main media loading task, by adding new tasks to
 * the chained
 * task list in {@link MediaRequest#loadMediaBlocking(List)}. One possible type
 * of chained task is
 * media encoding task. Loaded media will be encoded on a dedicated single
 * threaded executor
 * *after* the UI is notified of the loaded media. In this case, the encoded
 * media resource will
 * be eventually pushed to the cache, which will later be decoded before posting
 * to the UI thread
 * on cache hit.
 * </p>
 *
 ...
 * {@link ImageRequest}). Perform the
 * media loading work in loadMediaBlocking() and return a cache id in
 * getCacheId().</li>
 *
 ...
 * callback. Let the
 * UI component call MediaResourceManager.requestMediaResourceAsync() to request
 * a media source.
 * (example: {@link ContactIconView}</li>
 * </ol>
 */
public class MediaResourceManager {
    ...
    // We use a fixed thread pool for handling media loading tasks. Using a cached
    // thread pool
    // allows for unlimited thread creation which can lead to OOMs so we limit the
    // threads here.
    ...
    // media to be cached.
    // These tasks are run on a single worker thread with low priority so as not to
    // contend with the
    // media loading tasks.
     ...
     * loading task,
     * the listener will be notified of success/failure iff it's still bound. A
     * refcount on the
     ...
     *         caller must call
     *         release() on the resource once it's done using it (like with
     *         Cursors).
     */
    public <T extends RefCountedMediaResource> T requestMediaResourceSync(
        ...
        try {
            ...
            // The loaded resource should have at least one refcount by now reserved for the
            // caller.
     ...
     *
     * @param mediaRequest the media request to be processed asynchronously. May be
     *                     either an
     *                     {@link AsyncMediaRequestWrapper} for listening for event
     *                     callbacks, or a regular media
     *                     request for fire-and-forget type of behavior.
    ...
    private <T extends RefCountedMediaResource> void scheduleAsyncMediaRequest(
        ...
        final BindableMediaRequest<T> bindableRequest = (mediaRequest instanceof BindableMediaRequest<?>)
                ? (BindableMediaRequest<T>) mediaRequest
                : null;
        if (bindableRequest != null && !bindableRequest.isBound()) {
            return; // Request is obsolete
        }
        // We don't use SafeAsyncTask here since it enforces the shared thread pool
        // executor
