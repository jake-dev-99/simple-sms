...
public class AttachmentPreview extends ScrollView implements OnAttachmentClickListener {
    ...
    protected void onFinishInflate() {
        ...
        // The attachment preview is a scroll view so that it can show the bottom
        // portion of the
        // attachment whenever the space is tight (e.g. when in landscape mode). Per
        // design
    ...
    public void hideAttachmentPreview() {
        if (getVisibility() != GONE) {
            ...
            if (mAttachmentView.getChildCount() > 0) {
                ...
                final View viewToHide = mAttachmentView.getChildCount() > 1 ? mAttachmentView
                        : mAttachmentView.getChildAt(0);
    ...
    public boolean onAttachmentsChanged(final DraftMessageData draftMessageData) {
        ...
        final List<PendingAttachmentData> pendingAttachments = draftMessageData.getReadOnlyPendingAttachments();
        ...
        if (getVisibility() != VISIBLE) {
            ...
            // Don't animate in the close button if this is the first update after view
            // creation.
        ...
        // Merge the pending attachment list with real attachment. Design would prefer
        // these be
        // in LIFO order user can see added images past the 5th one but we also want
        // them to be in
    ...
    /**
     * Kicks off an animation to animate the layout change for closing the gap
     * between the
