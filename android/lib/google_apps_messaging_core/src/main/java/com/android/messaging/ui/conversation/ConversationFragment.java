...
public class ConversationFragment extends Fragment implements ConversationDataListener,
    ...
    // We animate the message from draft to message list, if we the message doesn't
    // show up in the
    ...
    // Saved Instance State Data - only for temporal data which is nice to maintain
    // but not
    ...
    // The minimum scrolling distance between RecyclerView's scroll change event
    // beyong which
    // a fling motion is considered fast, in which case we'll delay load image
    // attachments for
    // perf optimization.
    ...
    // Attachment data for the attachment within the selected message that was long
    // pressed
    private MessagePartData mSelectedAttachment;

    // Normally, as soon as draft message is loaded, we trust the UI state held in
    // ComposeMessageView to be the only source of truth (incl. the conversation
    // self id). However,
    // there can be external events that forces the UI state to change, such as SIM
    // state changes
    // or SIM auto-switching on receiving a message. This receiver is used to
    // receive such
    ...
    private final BroadcastReceiver mConversationSelfIdChangeReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(final Context context, final Intent intent) {
            ...
            final String selfId = intent.getStringExtra(UIIntents.UI_INTENT_EXTRA_CONVERSATION_SELF_ID);
    ...
    // Indicates whether local draft should be cleared due to external draft changes
    // that must
    // be reloaded from db
    ...
    private final RecyclerView.OnScrollListener mListScrollListener = new RecyclerView.OnScrollListener() {
        // Keeps track of cumulative scroll delta during a scroll event, which we may
        // use to
    ...
    private final ActionMode.Callback mMessageActionModeCallback = new ActionMode.Callback() {
        ...
        public boolean onCreateActionMode(final ActionMode actionMode, final Menu menu) {
            ...
            // ShareActionProvider does not work with ActionMode. So we use a normal menu
            // item.
            ...
            // unclear which attachment to pick when we make this context menu at the
            // message level
        ...
        public boolean onActionItemClicked(final ActionMode actionMode, final MenuItem menuItem) {
            ...
            if (itemId == R.id.save_attachment) {
                ...
                mHost.dismissActionMode();
                return true;
            } else if (itemId == R.id.forward_message_menu) {// TODO: Currently we are forwarding one part at a time,
                                                             // instead of
    ...
    /**
     * setConversationInfo() may be called before or after onCreate(). When a user
     * initiate a
     * conversation from compose, the ConversationActivity creates this fragment and
     * calls
     * setConversationInfo(), so it happens before onCreate(). However, when the
     * activity is
     * restored from saved instance state, the ConversationFragment is created
     * automatically by
     * the fragment, before ConversationActivity has a chance to call
     * setConversationInfo(). Since
     * the ability to start loading data depends on both methods being called, we
     * need to start
     * loading when onActivityCreated() is called, which is guaranteed to happen
     * after both.
     */
    @Override
    public void onActivityCreated(final Bundle savedInstanceState) {
        ...
        // Build the input manager with all its required dependencies and pass it along
        // to the
        ...
        mDraftMessageDataModel = BindingBase.createBindingReference(mComposeMessageView.getDraftDataModel());
        mDraftMessageDataModel.getData().addListener(this);
    }

    public void onAttachmentChoosen() {
        // Attachment has been choosen in the AttachmentChooserActivity, so clear local
        // draft
        ...
        mRecyclerView.setItemAnimator(new DefaultItemAnimator() {
            ...
            public boolean animateAdd(final ViewHolder holder) {
                ...
                if (data.getReceivedTimeStamp() == InsertNewMessageAction.getLastSentMessageTimestamp() &&
                    ...
                    final AttachmentPreview attachmentView = (AttachmentPreview) mComposeMessageView.findViewById(
    ...
    public static void displayPhoto(final Uri photoUri, final Rect imageBounds,
        ...
        final Uri imagesUri = isDraft ? MessagingContentProvider.buildDraftImagesUri(conversationId)
    ...
    public void onResume() {
        ...
        // On resume, check if there's a pending request for resuming message compose.
        // This
        // may happen when the user commits the contact selection for a group
        // conversation and
    ...
    public void onConversationMessagesCursorUpdated(final ConversationData data,
        ...
        // This needs to be determined before swapping cursor, which may change the
        // scroll state.
        ...
        final boolean oneOnOne = !data.getParticipantsLoaded() || data.getOtherParticipant() != null;
        ...
        if (cursor != null && oldCursor == null) {
            if (mListState != null) {
                ...
                // RecyclerView restores scroll states without triggering scroll change events,
                // so
        ...
        if (isSync) {
            ...
            // would
            // implicitly change RV's scroll position. We'd like the RV to keep scrolled to
            // the same
            // relative position from the bottom (because RV is stacked from bottom), so
            // that it
            ...
            // Show a snack bar notification if we are not scrolled to the bottom and the
            // new
        ...
        if (cursor != null) {
            ...
            // Are we coming from a widget click where we're told to scroll to a particular
            // item?
    ...
    public void onConversationMetadataUpdated(final ConversationData conversationData) {
        ...
        if (mSelectedMessage != null && mSelectedAttachment != null) {
            // We may have just sent a message and the temp attachment we selected is now
            // gone.
            // and it was replaced with some new attachment. Since we don't know which one
            // it
            // is we shouldn't reselect it (unless there is just one) In the
            // multi-attachment
            // case we would just deselect the message and allow the user to reselect,
            // otherwise we
    ...
    public void setConversationInfo(final Context context, final String conversationId,
        ...
        // Factory.get().bindConversationData(mBinding, getActivity(), this,
        // conversationId));
        if (!mBinding.isBound()) {
    ...
    public void onAttachmentsCleared() {
        // When attachments are removed, reset transient media picker state such as
        // image selection.
        mComposeMessageView.resetMediaPickerState();
    }

    /**
     * Called to check if all conditions are nominal and a "go" for some action,
     * such as deleting
    ...
    /**
     * When there's some condition that prevents an operation, such as sending a
     * message,
     * call warnOfMissingActionConditions to put up a snackbar and allow the user to
     * repair
     * that condition.
     *
     * @param sending                                  - true if we're called during
     *                                                 a sending operation
     * @param commandToRunAfterActionConditionResolved - a runnable to run after the
     *                                                 user responds
     *                                                 positively to the condition
     *                                                 prompt and resolves the
     *                                                 condition. If null,
     *                                                 the user will be shown a
     *                                                 toast to tap the send button
     *                                                 again.
     */
    @Override
    ...
    public void onConversationParticipantDataLoaded(final ConversationData data) {
        ...
        if (mBinding.getData().getParticipantsLoaded()) {
            ...
            mHost.onConversationParticipantDataLoaded(mBinding.getData().getNumberOfParticipantsExcludingSelf());
        }
    }
    ...
    public void promptForSelfPhoneNumber() {
        if (mComposeMessageView != null) {
            // Avoid bug in system which puts soft keyboard over dialog after orientation
            // change
        ...
        protected void onPostExecute(final Void result) {
            ...
            for (final AttachmentToSave attachment : mAttachmentsToSave) {
                ...
                if (ContentType.isImageType(attachment.contentType)) {
                    ...
                    final DownloadManager downloadManager = (DownloadManager) mContext.getSystemService(
    ...
    public int getConversationSelfSubId() {
        ...
        // If the self id or the self participant data hasn't been loaded yet, fallback
        // to
        // the default setting.
    ...
    public void updateActionBar(final ActionBar actionBar) {
        if (mComposeMessageView == null || !mComposeMessageView.updateActionBar(actionBar)) {
            ...
            // We update this regardless of whether or not the action bar is showing so that
            // we
            ...
            if (!TextUtils.isEmpty(conversationName)) {
                ...
                // In case phone numbers are mixed in the conversation name, we need to vocalize
                // it.
            ...
            // When conversation is showing and media picker is not showing, then hide the
            // action
    ...
    public void onDraftChanged(final DraftMessageData data, final int changeFlags) {
        ...
        // We're specifically only interested in ATTACHMENTS_CHANGED from the widget.
        // Ignore
        // other changes. When the widget changes an attachment, we need to reload the
        // draft.
        if (changeFlags == (DraftMessageData.WIDGET_CHANGED | DraftMessageData.ATTACHMENTS_CHANGED)) {
