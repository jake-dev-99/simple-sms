...
import android.support.mms.CarrierConfigValuesLoader;
 ...
 * the actual loading and stores the values in a Bundle. This class provides
 * getter
 * methods for values used in the app, which is easier to use than the raw
 * loader
 * class.
 */
public class MmsConfig {
    ...
    // A map that stores all MmsConfigs, one per active subscription. For pre-LMSim,
    // this will
    // contain just one entry with the default self sub id; for LMSim and above,
    // this will contain
    // all active sub ids but the default subscription id - the default subscription
    // id will be
    ...
    // The fallback values
    private static final MmsConfig sFallback = new MmsConfig(ParticipantData.DEFAULT_SELF_SUB_ID, new Bundle());
    ...
    /**
     * Same as load() but doing it using an async thread from SafeAsyncTask thread
     * pool.
     */
    ...
    private static void addMmsConfig(MmsConfig mmsConfig) {
        Assert.isTrue(OsUtil.isAtLeastL_MR1() != (mmsConfig.mSubId == ParticipantData.DEFAULT_SELF_SUB_ID));
