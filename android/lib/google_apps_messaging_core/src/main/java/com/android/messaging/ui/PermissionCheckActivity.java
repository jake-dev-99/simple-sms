 ...
 * to prompt the user
 * to grant permissions. However, the OS may not actually prompt the user if the
 * user had
 * previously checked the "Never ask again" checkbox while denying the required
 * permissions.
 */
public class PermissionCheckActivity extends Activity {
    ...
    public void onRequestPermissionsResult(
        ...
        if (requestCode == REQUIRED_PERMISSIONS_REQUEST_CODE) {
            ...
            // revoked while the permissions dialog box was being shown for the missing
            // permissions.
            if (!redirectIfNeeded()) {
                final long currentTimeMillis = SystemClock.elapsedRealtime();
                // If the permission request completes very quickly, it must be because the
                // system
