...
/**
 * Animates viewToAnimate from startRect to the place where it is in the layout,
 * viewToAnimate
 * should be in its final destination location before startAfterLayoutComplete
 * is called.
 * viewToAnimate will be drawn scaled and offset in a popupWindow.
 * This class handles the case where the viewToAnimate moves during the
 * animation
 */
public class PopupTransitionAnimation extends Animation {
    ...
    /**
     * The bounds of the popup in window coordinates. Does not include notification
     * bar
     */
    private final Rect mPopupRect;

    /**
     * The bounds of the action bar in window coordinates. We clip the popup to
     * below this
     */
    ...
    public void startAfterLayoutComplete() {
        // We want layout to occur, and then we immediately animate it in, so hide it
        // initially to
        ...
        final Runnable startAnimation = new Runnable() {
            ...
            public void run() {
                ...
                // size of 1x1 because anything with a size of 0x0 is considered hidden. We
                // can't
    ...
    protected void applyTransformation(final float interpolatedTime, final Transformation t) {
        ...
        // Update currentRect to the new animated coordinates, and request mPopupRoot to
        // redraw
    ...
    private void dismiss() {
        ...
        mViewToAnimate.setAlpha(1);
        mViewToAnimate.setVisibility(View.VISIBLE);
        // Delay dismissing the popup window to let mViewToAnimate draw under it and
        // reduce the
        // flash
    ...
    /**
     * Computes mDestRect (the position in window space of the placeholder view that
     * we should
     * animate to). Some frames during the animation fail to compute
     * getGlobalVisibleRect, so use
    ...
    /**
     * Sets up the PopupWindow that the view will animate in. Animating the size and
     * position of a
     * popup can be choppy, so instead we make the popup fill the entire space of
     * the screen, and
    ...
    private void initPopupWindow() {
        mPopupRoot = new View(mViewToAnimate.getContext()) {
            ...
            protected void onDraw(final Canvas canvas) {
                ...
                // the popup window may or may not cover the notification bar (depending on
                // screen
        ...
        // We must pass a non-zero value for the y offset, or else the system resets the
        // status bar
        // color to black (M only) during the animation. The actual position of the
        // window (and
        // the animated view inside it) are still correct, regardless of what we pass
        // for the y
