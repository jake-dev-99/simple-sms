...
/**
 * <p>
 * Shows a vertical "explode" animation for any view inside a view group (e.g.
 * views inside a
 * ListView). During the animation, a snapshot is taken for the view to the
 * animated and
 * presented in a popup window or view overlay on top of the original view
 * group. The background
 * of the view (a highlight) vertically expands (explodes) during the animation.
 * </p>
 * <p>
 * The exact implementation of the animation depends on platform API level. For
 * JB_MR2 and later,
 * the implementation utilizes ViewOverlay to perform highly performant overlay
 * animations; for
 * older API levels, the implementation falls back to using a full screen popup
 * window to stage
 * the animation.
 * </p>
 * <p>
 * To start this animation, call
 * {@link #startAnimationForView(ViewGroup, View, View, boolean, int)}
 * </p>
 */
public class ViewGroupItemVerticalExplodeAnimation {
    /**
     * Starts a vertical explode animation for a given view situated in a given
     * container.
     *
     * @param container            the container of the view which determines the
     *                             explode animation's final
     *                             size
     * @param viewToAnimate        the view to be animated. The view will be
     *                             highlighted by the explode
     *                             highlight, which expands from the size of the
     *                             view to the size of the container.
     * @param animationStagingView the view that stages the animation. Since
     *                             viewToAnimate may be
     *                             removed from the view tree during the animation,
     *                             we need a view that'll be alive
     *                             for the duration of the animation so that the
     *                             animation won't get cancelled.
     * @param snapshotView         whether a snapshot of the view to animate is
     *                             needed.
     */
    public static void startAnimationForView(final ViewGroup container, final View viewToAnimate,
        ...
        if (OsUtil.isAtLeastJB_MR2() && (viewToAnimate.getContext() instanceof Activity)) {
            ...
            // Pre JB_MR2, this animation can cause rendering failures which causes the
            // framework
            // to fall back to software rendering where camera preview isn't supported
            // (b/18264647)
    ...
    /**
     * Take a snapshot of the given review, return a Bitmap object that's owned by
     * the caller.
     */
    static Bitmap snapshotView(final View view) {
        ...
        // Strip the view of its background when taking a snapshot so that things like
        // touch
