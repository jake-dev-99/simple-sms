...
/**
 * Adds a "fast-scroll" bar to the conversation RecyclerView that shows the
 * current position within
 * the conversation and allows quickly moving to another position by dragging
 * the scrollbar thumb
 * up or down. As the thumb is dragged, we show a floating bubble alongside it
 * that shows the
...
public class ConversationFastScroller extends RecyclerView.OnScrollListener implements
     ...
     *                 {@code POSITION_RIGHT_SIDE} or
     *                 {@code POSITION_LEFT_SIDE})
     * @return a new ConversationFastScroller, or {@code null} if fast-scrolling is
     *         not supported
    ...
    // Whether to render the scrollbar on the right side (otherwise it'll be on the
    // left).
    ...
    public void refreshConversationThemeColor() {
        ...
        if (OsUtil.isAtLeastL()) {
            ...
            // drawable (it's rendered in the filter base color, which is red), so fall back
            // to
    ...
    private boolean isEnabled() {
        ...
        // Only enable scrollbars for conversations long enough that they would require
        // several
    ...
    /**
     * Returns the current position in the conversation, as a value between 0 and 1,
     * inclusive.
     * The top of the conversation is 0, the bottom is 1, the exact middle is 0.5,
     * and so on.
     */
    ...
    public boolean onInterceptTouchEvent(RecyclerView rv, MotionEvent e) {
        ...
        // If the user presses down on the scroll thumb, we'll start intercepting events
        // from the
        // RecyclerView so we can handle the move events while they're dragging it
        // up/down.
    ...
    public void onLayoutChange(View v, int left, int top, int right, int bottom,
        ...
        // The container is the size of the RecyclerView that's visible on screen. We
        // have to
        // exclude the top padding, because it's usually hidden behind the conversation
        // action bar.
