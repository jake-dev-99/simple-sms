...
/**
 * Shows one-time, transient notifications in response to action failures (i.e.
 * permanent failures
...
public class BugleActionToasts {
    ...
    static void onSendMessageOrManualDownloadActionCompleted(
        ...
        // We only show notifications for two cases, i.e. when mobile data is off or
        // when we are
        ...
        if (AccessibilityUtil.isTouchExplorationEnabled(Factory.get().getApplicationContext())) {
            ...
            // {@link MessageNotificationState#checkFailedMessages} does not post a
            // notification for
            // failures in observable conversations. For accessibility, we provide an
            // indication
            // here.
        ...
        if (AccessibilityUtil.isTouchExplorationEnabled(context)) {
            ...
            if (isFocusedConversation) {
                ...
                final String senderDisplayName = (sender == null)
                        ? res.getString(R.string.unknown_sender)
                        : sender.getDisplayName(false);
    ...
    public static UpdateDestinationBlockedAction.UpdateDestinationBlockedActionListener makeUpdateDestinationBlockedActionListener(
