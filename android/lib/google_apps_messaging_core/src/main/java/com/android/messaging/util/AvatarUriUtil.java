...
/**
 * A helper utility for creating {@link android.net.Uri}s to describe what
 * avatar to fetch or
 * generate and will help verify and extract information from avatar
 * {@link android.net.Uri}s.
 *
 * There are three types of avatar {@link android.net.Uri}.
 *
 * 1) Group Avatars - These are avatars which are used to represent a group
 * conversation. Group
 * avatars uris are basically multiple avatar uri which can be any of the below
 * types but not
 * another group avatar. The group avatars can hold anywhere from two to four
 * avatars uri and can
 ...
 *
 * 2) Local Resource - A local resource avatar is use when there is a profile
 * photo for the
 * participant. This can be any local resource.
 *
 * 3) Letter Tile - A letter tile is used when a participant has a name but no
 * profile photo. A
 * letter tile will contain the first code point of the participant's name and a
 * background color
 * based on the hash of the participant's full name. Letter tiles will be in the
 * following format.
 * messaging://avatar/l?n=<fullName>
 *
 * 4) Default Avatars - These are avatars are used when the participant has no
 * profile photo or
 * name. In these cases we use the default person icon with a color background.
 * The color
 * background is based on a hash of the normalized phone number.
 *
 * 5) Default Background Avatars - This is a special case for Default Avatars
 * where we use the
 * default background color for the default avatar.
 *
 * 6) SIM Selector Avatars - These are avatars used in the SIM selector. This
 * may either be a
 * regular local resource avatar (2) or an avatar with a SIM identifier (i.e.
 * SIM background with
...
public class AvatarUriUtil {
    ...
    /**
     * Creates an avatar uri based on a list of ParticipantData. The list of
     * participants may not
     * be null or empty. Depending on the size of the list either a group avatar uri
     * will be create
    ...
    /**
     * Joins together a list of valid avatar uri into a group uri.The list of
     * participants may not
     * be null or empty. If a lit of one is given then the first element will be
     * return back
     * instead of a group avatar uri. All uris in the list must be a valid avatar
     * uri. This will
    ...
    /**
     * Creates an avatar uri based on ParticipantData which may not be null and
     * expected to have
     * profilePhotoUri, fullName and normalizedDestination populated. This will
     * never return a null
     * uri.
     */
     ...
     * @param slotIdentifier  when null, this will simply use a regular avatar;
     *                        otherwise, the
     *                        first letter of slotIdentifier will be used for the
     *                        icon.
    ...
    /**
     * Creates an avatar uri from the given local resource Uri, followed by a
     * fallback Uri in case
    ...
    private static Uri fromName(@NonNull final CharSequence name, final String contactLookupKey) {
        ...
        final String identifier = TextUtils.isEmpty(contactLookupKey) ? nameString : contactLookupKey;
