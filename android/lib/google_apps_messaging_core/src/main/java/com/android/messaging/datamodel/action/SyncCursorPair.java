...
/**
 * Class holding a pair of cursors - one for local db and one for telephony
 * provider - allowing
...
class SyncCursorPair {
     ...
     *
     * Note that this may not be the most reliable way to tell if messages are in
     * sync.
     * For example, the local misses one message and has one obsolete message.
     * However, we have background sms sync once a while, also some other events
     * might
     * trigger a full sync. So we will eventually catch up. And this should be rare
     * to
     * happen.
     *
    ...
    long scan(final int maxMessagesToScan,
        ...
        // Import messages in remote not in local, delete messages in local not in
        // remote
    ...
    private static final String ORDER_BY_TIMESTAMP_DESC = MessageColumns.RECEIVED_TIMESTAMP + " DESC";
    ...
    /**
     * This class provides the same DatabaseMessage interface over a local SMS db
     * message
     */
    ...
    private static class RemoteCursorsIterator implements CursorIterator {
        ...
        @Override
        // Return approximate cursor position allowing for read ahead on two cursors
        // (hence -1)
        public int getPosition() {
    ...
    /**
     * Type selection for importing sms messages. Only SENT and INBOX messages are
     * imported.
     *
     ...
     * MESSAGE_TYPE is SEND_REQ (sent), RETRIEVE_CONF (received) or NOTIFICATION_IND
     * (download)
     *
    ...
    private void saveMessageToAdd(final List<SmsMessage> smsToAdd,
        ...
        // Cache the lookup and canonicalization of the phone number outside of the
        // transaction...
