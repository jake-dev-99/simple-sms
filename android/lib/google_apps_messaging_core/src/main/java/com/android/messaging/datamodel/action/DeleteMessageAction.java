...
public class DeleteMessageAction extends Action implements Parcelable {
    ...
    // which could bring the deleted message back to life between the time we
    // deleted
    // it locally and deleted it in telephony (sync is also done on
    // doBackgroundWork).
    //
    // Previously this block of code deleted from telephony first but that can be
    // very
    // slow (on the order of seconds) so this was modified to first delete locally,
    // trigger
