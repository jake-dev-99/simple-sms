 ...
 * Once image size is determined during loading this descriptor will update the
 * db if necessary.
 */
public class MessagePartImageRequestDescriptor extends UriImageRequestDescriptor {
    ...
    public void updateSourceDimensions(final int updatedWidth, final int updatedHeight) {
        // If the dimensions of the image do not match then queue a DB update with new
        // size.
        // Don't update if we don't have a part id, which happens if this part is loaded
        // as
