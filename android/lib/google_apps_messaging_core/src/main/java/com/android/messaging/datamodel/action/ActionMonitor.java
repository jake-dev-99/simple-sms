 ...
 * o) Immediate local processing only - will trigger ActionCompletedListener
 * when done
 * o) Background worker processing only - will trigger ActionCompletedListener
 * when done
 * o) Immediate local processing followed by background work followed by more
 * local processing
 * - will trigger ActionExecutedListener once local processing complete and
 * ActionCompletedListener when second set of local process (dealing with
 * background
...
public class ActionMonitor {
    ...
    public interface ActionCompletedListener {
        /**
         * @param result object returned from processing the action. This is the value
         *               returned by
    ...
    /**
     * Interface for being notified of action state changes - used for profiling,
     * testing only
     */
    ...
    // The background work has completed, either returning a success response or
    // resulting in a
    // failure
    ...
    /**
     * The action key is used to determine equivalence of operations and their
     * requests
     */
    ...
    /**
     * Unregister listeners so that they will not be called back - override this
     * method if needed
     */
    ...
    static void setState(final Action action, final int expectedOldState,
        ...
        final ActionMonitor monitor = ActionMonitor.lookupActionMonitor(action.actionKey);
        if (monitor != null) {
     ...
     * @param action           - action whose state is updating
     * @param expectedOldState - expected existing state of action (can be UNKNOWN)
     * @param result           - object returned from processing the action. This is
     *                         the value returned by
     *                         {@link Action#executeAction} if there is no
     *                         background work, or
     *                         else the value returned by
     *                         {@link Action#processBackgroundResponse}
     ...
     *
     * @param action               - action whose state is updating
     * @param expectedOldState     - expected existing state of action (can be
     *                             UNKNOWN)
     * @param hasBackgroundActions - has the completing action requested background
     *                             work
     * @param result               - the return value of
     *                             {@link Action#executeAction}
     */
     ...
     */
    @VisibleForTesting
    static SimpleArrayMap<String, ActionMonitor> sActionMonitors = new SimpleArrayMap<String, ActionMonitor>();
