...
/**
 * A ref-counted class that holds loaded media resource, be it bitmaps or media
 * bytes.
 * Subclasses must implement the close() method to release any resources (such
 * as bitmaps)
 ...
 *
 * Note: all synchronized methods in this class (e.g. addRef()) should not
 * attempt to make outgoing
 * calls that could potentially acquire media cache locks due to the potential
 * deadlock this can
 * cause. To synchronize read/write access to shared resource,
 * {@link #acquireLock()} and
...
public abstract class RefCountedMediaResource {
    ...
    // Set DEBUG to true to enable detailed stack trace for each addRef() and
    // release() operation
    ...
    // A lock that guards access to shared members in this class (and all its
    // subclasses).
