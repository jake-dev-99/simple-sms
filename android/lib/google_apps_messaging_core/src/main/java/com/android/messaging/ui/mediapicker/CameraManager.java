...
/**
 * Class which manages interactions with the camera, but does not do any UI.
 * This class is
 * designed to be a singleton to ensure there is one component managing the
 * camera and releasing
 ...
 * </ul>
 * Callers should call onPause and onResume to ensure that the camera is release
 * while the activity
 * is not active.
 * This class is not thread safe. It should only be called from one thread (the
 * UI thread or test
 * thread)
 */
class CameraManager implements FocusOverlayManager.Listener {
    /**
     * Wrapper around the framework camera API to allow mocking different hardware
     * scenarios while
     * unit testing
     */
    ...
    /**
     * The index of the selected camera or NO_CAMERA_SELECTED if a camera hasn't
     * been selected yet
     */
     ...
     * Using AsyncTask rather than SafeAsyncTask because the tasks need to be
     * serialized, but don't
     * need to be on the UI thread
     * TODO: If we have other AyncTasks (not SafeAsyncTasks) this may contend and we
     * may
     * need to create a dedicated thread, or synchronize the threads in the thread
     * pool
     */
    private AsyncTask<Integer, Void, Camera> mOpenCameraTask;

    /**
     * The camera index that is queued to be opened, but not completed yet, or
     * NO_CAMERA_SELECTED if
    ...
    void setRenderOverlay(final RenderOverlay renderOverlay) {
        mFocusOverlayManager.setFocusRenderer(renderOverlay != null ? renderOverlay.getPieRenderer() : null);
    }

    /** Convenience function to swap between front and back facing cameras */
    void swapCamera() {
        Assert.isTrue(mCameraIndex >= 0);
        selectCamera(mCameraInfo.facing == CameraInfo.CAMERA_FACING_FRONT ? CameraInfo.CAMERA_FACING_BACK
                : CameraInfo.CAMERA_FACING_FRONT);
    }

    /**
     * Selects the first camera facing the desired direction, or the first camera if
     * there is no
    ...
    boolean selectCamera(final int desiredFacing) {
        try {
            ...
            // There's no camera in the desired facing direction, just select the first
            // camera
    ...
    /**
     * Opens the camera on a separate thread and initiates the preview if one is
     * available
     */
    void openCamera() {
        if (mCameraIndex == NO_CAMERA_SELECTED) {
            // Ensure a selected camera if none is currently selected. This may happen if
            // the
            // camera chooser is not the default media chooser.
            selectCamera(CameraInfo.CAMERA_FACING_BACK);
        }
        mOpenRequested = true;
        // We're already opening the camera or already have the camera handle, nothing
        // more to do
        if (mPendingOpenCameraIndex == mCameraIndex || mCamera != null) {
            return;
        }

        // True if the task to open the camera has to be delayed until the current one
        // completes
    ...
    /**
     * Sets the listener which will be notified of errors or other events in the
     * camera
     *
    ...
    void takePicture(final float heightPercent, @NonNull final MediaCallback callback) {
        ...
        if (mCamera == null) {
            // The caller should have checked isCameraAvailable first, but just in case,
            // protect
            // against a null camera by notifying the callback that taking the picture
            // didn't work
    ...
    private void updateCameraOrientation() {
        ...
        final WindowManager windowManager = (WindowManager) mCameraPreview.getContext().getSystemService(
        ...
        // The clockwise rotation angle relative to the orientation of the camera. This
        // affects
    ...
    void stopVideo() {
        ...
        try {
            ...
            // MediaRecorder.stop will throw a RuntimeException if the video was too short,
            // let the
    ...
    /**
     * External components call into this to report if hardware acceleration is
     * supported. When
     * hardware acceleration isn't supported, we need to report an error through the
     * listener
     * interface
     *
     * @param isHardwareAccelerationSupported True if the preview is rendering in a
     *                                        hardware
     *                                        accelerated view.
     */
    ...
    /**
     * Returns the scale factor to scale the width/height to max allowed in
     * MmsConfig
     */
    private float getScaleFactorForMaxAllowedSize(final int width, final int height,
            final int maxWidth, final int maxHeight) {
        if (maxWidth <= 0 || maxHeight <= 0) {
            // MmsConfig initialization runs asynchronously on application startup, so
            // there's a
    ...
    private MmsConfig getMmsConfig() {
        final int subId = mSubscriptionDataProvider != null ? mSubscriptionDataProvider.getConversationSelfSubId()
                : ParticipantData.DEFAULT_SELF_SUB_ID;
        return MmsConfig.get(subId);
    }

    /**
     * Choose the best picture size by trying to find a size close to the
     * MmsConfig's max size,
    ...
    private Camera.Size chooseBestPictureSize() {
        ...
        if (displayOrientation == Configuration.ORIENTATION_LANDSCAPE) {
            // Rotate the camera orientation 90 degrees to compensate for the rotated
            // display
            // metrics. Direction doesn't matter because we're just using it for
            // width/height
    ...
    /**
     * Chose the best preview size based on the picture size. Try to find a size
     * with the same
    ...
    private static class SizeComparator implements Comparator<Camera.Size> {
        ...
        // The max width/height for the preferred size. Integer.MAX_VALUE if no size
        // limit
        ...
        /**
         * Returns a negative value if left is a better choice than right, or a positive
         * value if
        ...
        public int compare(final Camera.Size left, final Camera.Size right) {
            ...
            if ((left.width <= mMaxWidth && left.height <= mMaxHeight) != (right.width <= mMaxWidth
                    && right.height <= mMaxHeight)) {
            ...
            // than the max size or both smaller than the max size, so prefer the one
            // closest
            // to target size
    ...
    public void autoFocus() {
        ...
        try {
            ...
            // If autofocus fails, the camera should have called the callback with
            // success=false,
    ...
    public void setFocusParameters() {
        ...
        try {
            ...
            if (parameters.getMaxNumFocusAreas() > 0) {
                // Don't set focus areas (even to null) if focus areas aren't supported, camera
                // may
                // crash
    ...
    private void logCameraSize(final String prefix, final Camera.Size size) {
        // Log the camera size and aspect ratio for help when examining bug reports for
        // camera
        // failures
    ...
    private void lockOrientation() {
        ...
        final WindowManager windowManager = (WindowManager) a.getSystemService(Context.WINDOW_SERVICE);
