...
public class AudioRecordView extends FrameLayout implements
    ...
    /**
     * The user has pressed the record button and we are playing the sound
     * indicating the
     * start of recording session. Don't record yet since we don't want the beeping
     * sound
    ...
    /**
     * When the user has finished recording, we need to record for some additional
     * time.
     */
    private static final int MODE_STOPPING = 4;

    // Bug: 16020175: The framework's MediaRecorder would cut off the ending portion
    // of the
    // recorded audio by about half a second. To mitigate this issue, we continue
    // the recording
    // for some extra time before stopping it.
    private static final int AUDIO_RECORD_ENDING_BUFFER_MILLIS = 500;

    /**
     * The minimum duration of any recording. Below this threshold, it will be
     * treated as if the
    ...
    public void onPause() {
        // The conversation draft cannot take any updates when it's paused. Therefore,
        // forcibly
    ...
    boolean onRecordButtonTouchUp() {
        if (System.currentTimeMillis() - mAudioRecordStartTimeMillis < AUDIO_RECORD_MINIMUM_DURATION_MILLIS) {
    ...
    private void onFinishedRecording() {
        ...
        if (outputUri != null) {
            ...
            final MediaPickerMessagePartData audioItem = new MediaPickerMessagePartData(startRect,
