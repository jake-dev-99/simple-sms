...
/**
 * This class is provides the same interface and functionality as
 * android.media.AsyncPlayer
 * with the following differences:
 * - whenever audio is played, audio focus is requested,
 * - whenever audio playback is stopped or the playback completed, audio focus
 * is abandoned.
 *
 * This file has been copied from com.android.server.NotificationPlayer. The
 * only modification is
 * the addition of a volume parameter. Hopefully the framework will adapt
 * AsyncPlayer to support
...
public class NotificationPlayer implements OnCompletionListener {
    ...
    /*
     * Besides the use of audio focus, the only implementation difference between
     * AsyncPlayer and
     * NotificationPlayer resides in the creation of the MediaPlayer. For the
     * completion callback,
     * OnCompletionListener, to be called at the end of the playback, the
     * MediaPlayer needs to
    ...
    public void onCompletion(final MediaPlayer mp) {
        ...
        // if there are no more sounds to play, end the Looper to listen for media
        // completion
        synchronized (mCmdQueue) {
            if (mCmdQueue.size() == 0) {
                synchronized (mCompletionHandlingLock) {
     ...
     * @param volume  The volume at which to play this sound, as a fraction of the
     *                system volume for
     *                the relevant stream type. A value of 1 is the maximum and
     *                means play at the system
     ...
     * probably is
     * optional, but it won't hurt to have it too. The problem is that if you start
     * a sound
     * while you're holding a wake lock (e.g. an alarm starting a notification), you
     * want the
     * sound to play, but if the CPU turns off before mThread gets to work, it
     * won't. The
     * simplest way to deal with this is to make it so there is a wake lock held
     * while the
     * thread is starting or running. You're going to need the WAKE_LOCK permission
     * if you're
     * going to call this.
     *
