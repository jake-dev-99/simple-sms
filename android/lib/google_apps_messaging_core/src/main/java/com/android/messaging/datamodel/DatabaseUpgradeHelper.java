...
public class DatabaseUpgradeHelper {
    ...
    /**
     * Checks db version correctness at the end of each milestone release. If target
     * database
     * version lies beyond the version range that the current release may handle, we
     * snap the
     * current version to the end of the release, so that we may go on to the next
     * release' upgrade
     * path. Otherwise, if target version is within reach of the current release,
     * but we are not
    ...
    private int checkAndUpdateVersionAtReleaseEnd(final int currentVersion,
        ...
        // If we are here, this means the current release' upgrade handler should
        // upgrade to
        // target version...
