...
/**
 * A thin wrapper for getting GServices value. During constructor time a one
 * time background thread
 * will cache all GServices key with the prefix of "bugle_". All get calls will
 * wait for Gservices
 * to finish caching the first time. In practice, the background thread will
 * finish before any get
 * request.
 */
