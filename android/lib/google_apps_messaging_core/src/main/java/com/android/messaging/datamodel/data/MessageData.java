...
public class MessageData implements Parcelable {
    ...
    // SQL statement to insert a "complete" message row (columns based on the
    // projection above).
    private static final String INSERT_MESSAGE_SQL = "INSERT INTO " + DatabaseHelper.MESSAGES_TABLE + " ( "
    ...
    // Detailed MMS failures. Most of the values are defined in PduHeaders. However,
    // a few are
    // defined here instead. These are never returned in the MMS HTTP response, but
    // are used
    // internally. The values here must not conflict with any of the existing
    // PduHeader values.
    ...
    // Unknown result code for MMS sending/downloading. This is used as the default
    // value
    ...
    /**
     * Create a draft message for a particular conversation based on supplied
     * content
     */
    ...
    /**
     * Create a message received from a particular number in a particular
     * conversation
     */
    ...
    public static MessageData createMmsMessage(final String messageUri, final String participantId,
        ...
        if (bugleStatus == BUGLE_STATUS_INCOMING_RETRYING_AUTO_DOWNLOAD ||
            ...
            // Set the retry start timestamp if this message is already in process of
            // retrying
            // Either as autodownload is starting or sending already in progress (MMS
            // update)
    ...
    public void addPart(final MessagePartData part) {
        if (part instanceof PendingAttachmentData) {
            // Pending attachments may only be added to shared message data that's not
            // associated
    ...
    /**
     * Bind to the draft message data for a conversation. The conversation's self id
     * is used as
     * the draft's self id.
     */
    ...
    static boolean getShowDownloadMessage(final int status) {
        if (OsUtil.isSecondaryUser()) {
            // Secondary users can't download mms's. Mms's are downloaded by bugle running
            // as the
            // primary user.
            return false;
        }
    ...
    /**
     * Takes all captions from attachments and adds them as a prefix to the first
     * text part or
     * appends a text part
     */
     ...
     * Can be used to reset the messageId prior to persisting (which will assign a
     * new messageId)
     * or can be called on a message that does not yet have a valid messageId to set
     * it.
     */
    public void updateMessageId(final String messageId) {
        ...
        // TODO : This should probably also call updateMessageId on the message parts.
        // We
        // may also want to make messages effectively immutable once they have a valid
        // message id.
    }
