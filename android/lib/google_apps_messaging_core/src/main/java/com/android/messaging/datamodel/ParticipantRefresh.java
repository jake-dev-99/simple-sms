...
/**
 * Utility class for refreshing participant information based on matching
 * contact. This updates
 ...
 * 1. Full refresh, this is triggered at application start or activity resumes
 * after contact
 * change is detected.
 * 2. Partial refresh, this is triggered when a participant is added to a
 * conversation. This
...
public class ParticipantRefresh {
    ...
    /**
     * Refresh participants only if needed, i.e., application start or contact
     * changed.
     */
    ...
    private static boolean getNeedFullRefresh() {
        ...
        if (observer == null) {
            // If there is no observer (for unittest cases), we don't need to refresh
            // participants.
            return false;
        }
    ...
    /**
     * This class is totally static. Make constructor to be private so that an
     * instance
     ...
     *                    {@link #REFRESH_MODE_INCREMENTAL}, and
     *                    {@link #REFRESH_MODE_SELF_ONLY}
     */
    @VisibleForTesting
    static void refreshParticipants(final int refreshMode) {
        ...
        if (refreshMode == REFRESH_MODE_INCREMENTAL) {
            // In case of incremental refresh, filter out participants that are already
            // resolved.
        ...
        try {
            ...
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    try {
                        final ParticipantData participantData = ParticipantData.getFromCursor(cursor);
    ...
    private static final String UPDATE_SELF_PARTICIPANT_SUBSCRIPTION_SQL = "UPDATE " + DatabaseHelper.PARTICIPANTS_TABLE
            + " SET "
    ...
    /**
     * Ensure that there is a self participant corresponding to every active SIM.
     * Also, ensure
    ...
    private static void refreshSelfParticipantList() {
        ...
        final List<SubscriptionInfo> subInfoRecords = PhoneUtils.getDefault().toLMr1().getActiveSubscriptionInfoList();
        ...
        // Fix up conversation self ids by reverting to default self for conversations
        // whose self
    ...
    public static boolean refreshParticipant(final DatabaseWrapper db,
        ...
        if (participantData.isSelf()) {
            ...
            if (selfChange == SELF_PROFILE_EXISTS) {
                // If a self-profile exists, it takes precedence over Contacts data. So we are
                // done.
                return true;
            }
    ...
    private static int refreshFromSelfProfile(final DatabaseWrapper db,
        ...
        if (OsUtil.isAtLeastL_MR1()) {
            ...
            final SubscriptionInfo subscriptionInfo = PhoneUtils.get(participantData.getSubId()).toLMr1()
        ...
        try {
            ...
            // It's possible for contact query to fail and we don't want that to crash our
            // app.
            // However, we need to at least log the exception so we know something was
            // wrong.
    ...
    private static boolean refreshFromContacts(final DatabaseWrapper db,
        ...
        if (TextUtils.isEmpty(normalizedDestination)) {
            // The normalized destination can be "" for the self id if we can't get it from
            // the
        ...
        try {
            ...
            while (matchingContactCursor.moveToNext()) {
                ...
                // There is no need to try other contacts if the current contactId was not
                // filled...
    ...
    /**
     * Update participant with matching contact's contactId, displayName and
     * photoUri.
     */
    ...
    private static List<String> getConversationsWithSelfParticipantIds(final List<String> selfIds) {
        ...
        try {
            ...
            final String selection = ConversationColumns.CURRENT_SELF_ID + " IN (" + selectionList + ")";
    ...
    /**
     * After refreshing the self participant list, find all conversations with
     * inactive self ids,
