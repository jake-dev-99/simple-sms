...
public class CameraMediaChooserView extends FrameLayout implements PersistentInstanceState {
    ...
    // True if we have at least queued an update to the view tree to support
    // software rendering
    // fallback
    ...
    protected void onDraw(final Canvas canvas) {
        super.onDraw(canvas);
        // If the canvas isn't hardware accelerated, we have to replace the
        ...
        if (!canvas.isHardwareAccelerated() && !mIsSoftwareFallbackActive) {
            ...
            // Post modifying the tree since we can't modify the view tree during a draw
            // pass
            ThreadUtil.getMainThreadHandler().post(new Runnable() {
                @Override
                public void run() {
                    final HardwareCameraPreview cameraPreview = (HardwareCameraPreview) findViewById(
                            R.id.camera_preview);
                    ...
                    final SoftwareCameraPreview softwareCameraPreview = new SoftwareCameraPreview(getContext());
