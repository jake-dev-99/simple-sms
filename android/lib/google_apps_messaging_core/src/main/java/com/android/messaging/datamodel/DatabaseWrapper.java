...
public class DatabaseWrapper {
    ...
    /**
     * Set mExplainQueryPlanRegexp (via
     * {@link BugleGservicesKeys#EXPLAIN_QUERY_PLAN_REGEXP}
     * to regex matching queries to see query plans. For example, ".*" to show all
     * query plans.
     */
    // See
    ...
    private static ThreadLocal<Stack<TransactionData>> sTransactionDepth = new ThreadLocal<Stack<TransactionData>>() {
        @Override
    ...
    private void explainQueryPlan(final SQLiteQueryBuilder qb, final SQLiteDatabase db,
            ...
            @SuppressWarnings("unused") final String[] queryArgs,
            final String groupBy,
            @SuppressWarnings("unused") final String having,
    ...
    public long queryNumEntries(final String table, final String selection,
        ...
        maybePlayDebugNoise();
        final long retval = DatabaseUtils.queryNumEntries(mDatabase, table, selection, selectionArgs);
        if (mLog) {
            printTiming(
                    t1,
