...
/**
 * Class wrapping the conversation list view used to display each item in
 * conversation list
 */
public class ConversationListItemData {
    ...
    public void bind(final Cursor cursor, final boolean ignoreDraft) {
        ...
        if (!ignoreDraft) {
            ...
            mDraftPreviewUri = TextUtils.isEmpty(draftPreviewUriString) ? null : Uri.parse(draftPreviewUriString);
        ...
        mSnippetSenderDisplayDestination = cursor.getString(INDEX_SNIPPET_SENDER_DISPLAY_DESTINATION);
     ...
     * @return the contact id of the participant if it is a 1:1 conversation, -1 for
     *         group.
     */
    ...
    private static final String CONVERSATION_LIST_VIEW_PROJECTION = DatabaseHelper.CONVERSATIONS_TABLE + '.'
    ...
    private static final String JOIN_PARTICIPANTS = " LEFT JOIN " + DatabaseHelper.PARTICIPANTS_TABLE + " ON ("
    ...
    // View that makes latest message read flag available with rest of conversation
    // data.
    ...
    public static class ConversationListViewColumns implements BaseColumns {
        ...
        static final String INCLUDE_EMAIL_ADDRESS = ConversationColumns.INCLUDE_EMAIL_ADDRESS;
    ...
    public static String generateConversationName(final List<ParticipantData> participants) {
