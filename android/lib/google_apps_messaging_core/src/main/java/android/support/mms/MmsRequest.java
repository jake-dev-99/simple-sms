...
import android.support.mms.pdu.GenericPdu;
import android.support.mms.pdu.PduHeaders;
import android.support.mms.pdu.PduParser;
import android.support.mms.pdu.SendConf;
...
abstract class MmsRequest implements Parcelable {
     ...
     * @return true if loading request PDU from calling app succeeds, false
     *         otherwise
     */
     ...
     * @return true if transferring response PDU to calling app succeeds, false
     *         otherwise
     */
    ...
    // Maximum time to spend waiting to read data from a content provider before
    // failing with error.
     ...
     * @param networkManager            the MmsNetworkManager to use to setup MMS
     *                                  network
    ...
    void execute(final Context context, final MmsNetworkManager networkManager,
        ...
        if (mmsConfig == null) {
            ...
            try {
                ...
                // Load the potential APNs. In most cases there should be only one APN
                // available.
                // On some devices on which we can't obtain APN from system, we look up our own
                // APN list. Since we don't have exact information, we may get a list of
                // potential
     ...
     * Sometimes even if you send to the wrong APN, a response in valid PDU format
     * can still
            ...
            try {
                final GenericPdu pdu = new PduParser(
                        ...
                        mmsConfig.getBoolean(
                                CarrierConfigValuesLoader.CONFIG_SUPPORT_MMS_CONTENT_DISPOSITION,
                ...
                if (pdu != null && pdu instanceof SendConf) {
                    ...
                    return responseStatus == PduHeaders.RESPONSE_STATUS_ERROR_PERMANENT_SENDING_ADDRESS_UNRESOLVED ||
    ...
    private static final Integer TYPE_MOBILE_MMS = Integer.valueOf(ConnectivityManager.TYPE_MOBILE_MMS);

    /**
     * Wrapper for platform API requestRouteToHostAddress
     *
     * We first try the hidden but correct method on ConnectivityManager. If we
     * can't, use
