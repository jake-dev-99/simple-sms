...
/**
 * A PagerAdapter that provides a fixed number of paged Views provided by a
 * fixed set of
 * {@link PagerViewHolder}'s. This allows us to put a fixed number of Views,
...
public class FixedViewPagerAdapter<T extends PagerViewHolder> extends PagerAdapter {
    ...
    public Parcelable saveState() {
        // The paged views in the view pager gets created and destroyed as the user
        // scrolls through
        // them. By default, only the pages to the immediate left and right of the
        // current visible
        // page are realized. Moreover, if the activity gets destroyed and recreated,
        // the pages are
        // automatically destroyed. Therefore, in order to preserve transient page UI
        // states that
        // are not persisted in the DB we'd like to store them in a Bundle when views
        // get
        // destroyed. When the views get recreated, we rehydrate them by passing them
        // the saved
        // data. When the activity gets destroyed, it invokes saveState() on this
        // adapter to
