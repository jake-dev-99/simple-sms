...
public class ContactDropdownLayouter extends DropdownChipLayouter {
    ...
    /**
     * Bind a drop down view to a RecipientEntry. We'd like regular dropdown items
     * (BASE_RECIPIENT)
     * to behave the same as regular ContactListItemViews, while using the chips
     * library's
     * item styling for alternates dropdown items (happens when you click on a
     * chip).
     */
    @Override
    public View bindView(final View convertView, final ViewGroup parent, final RecipientEntry entry,
        ...
        if (type != AdapterType.BASE_RECIPIENT) {
            if (type == AdapterType.SINGLE_RECIPIENT) {
                // Treat single recipients the same way as alternates. The base implementation
                // of
