...
/**
 * Data model object used to power ContactListItemViews, which may be displayed
 * either in
 * our contact list, or in the chips UI search drop down presented by
 * ContactDropdownLayouter.
 */
public class ContactListItemData {
    // Keeps the contact data in the form of RecipientEntry that
    // RecipientEditTextView can
    // directly use.
    ...
    // If this contact is the first in the list for its first letter, then this will
    // be the
    ...
    // existing chip for which we show full contact detail for the selected
    // contact).
    ...
    public void bind(final Cursor cursor, final String alphabetHeader) {
        ...
        // Check whether this contact is first level (i.e. whether it's the first entry
        // of this
    ...
    /**
     * Bind to a RecipientEntry produced by the chips text view in the search drop
     * down, plus
    ...
    public Uri getPhotoThumbnailUri() {
        return mRecipientEntry.getPhotoThumbnailUri() == null ? null : mRecipientEntry.getPhotoThumbnailUri();
    }

    public CharSequence getDestination() {
        final CharSequence destination = mStyledDestination != null ? mStyledDestination
                : ContactRecipientEntryUtils.formatDestination(mRecipientEntry);
    ...
    /**
     * Returns if this item is "first-level," i.e. whether it's the first entry of
     * the contact
     * that it represents in the list. For example, if John Smith has 3 different
     * phone numbers,
     * then the first number is considered first-level, while the other two are
     * considered
     * second-level.
     */
    public boolean getIsFirstLevel() {
        // Treat the item as first level if it's a top-level recipient entry, or if it's
        // the only
        // item in the list.
    ...
    /**
     * Returns if this item is simple, i.e. it has only avatar and a display name
     * with phone number
    ...
    /**
     * Returns a RecipientEntry instance readily usable by the
