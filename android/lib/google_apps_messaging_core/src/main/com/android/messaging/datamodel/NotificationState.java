...
/**
 * Base class for representing notifications. The main reason for this class is
 * that in order to
 * show pictures or avatars they might need to be loaded in the background. This
 * class and
 * subclasses can do the main work to get the notification ready and then wait
 * until any images
 * that are needed are ready before posting.
 *
 * The creation of a notification is split into two parts. The NotificationState
 * ctor should
 * setup the basic information including the mContentIntent. A Notification
 * Builder is created in
 * RealTimeChatNotifications and passed to the build() method of each
 * notification where the
 * Notification is fully specified.
 *
 * TODO: There is still some duplication and inconsistency in the utility
 * functions and
 * placement of different building blocks across notification types (e.g.
 * summary text for accounts)
 */
public abstract class NotificationState {
    ...
    public interface FailedMessageQuery {
        static final String FAILED_MESSAGES_WHERE_CLAUSE = "((" + MessageColumns.STATUS + " = " +
    ...
    // Returns the mime type of the attachment (See ContentType class for
    // definitions)
    ...
    /**
     * Reserves request codes for this notification type. By default 2 codes are
     * reserved, one for
     * the main intent and another for the cancel intent. Override this function to
     * reserve more.
     */
    ...
    /**
     * @return the type of notification that should be used from
     *         {RealTimeChatNotifications}
