...
public class DraftMessageData extends BindableData implements ReadDraftDataActionListener {
    ...
    // Flags sent to onDraftChanged to help the receiver limit the amount of work
    // done
    ...
    // ALL_CHANGED intentionally doesn't include WIDGET_CHANGED.
    // ConversationFragment needs to
    // be notified if the draft it is looking at is changed externally (by a desktop
    // widget) so it
    ...
    /**
     * Keeps track of completed attachments in the message draft. This data is
     * persisted to db
     */
    ...
    /**
     * Keeps track of pending attachments that are being loaded. The pending
     * attachments are
     * transient, because they are not persisted to the database and are dropped
     * once we go
    ...
    /**
     * A read-only wrapper on mPendingAttachments surfaced to the UI layer for
     * rendering
     */
    private final List<PendingAttachmentData> mReadOnlyPendingAttachments;

    /**
     * Is the current draft a cached copy of what's been saved to the database. If
     * so, we
    ...
    public void updateFromMessageData(final MessageData message, final String bindingId) {
        ...
        // Do not use the loaded draft if the user began composing a message before the
        // draft loaded
        // During config changes (orientation), the text fields preserve their data, so
        // allow them
        ...
        if (isDraftEmpty() || (TextUtils.equals(mMessageText, message.getMessageText()) &&
            ...
            // The user has started a new message so we throw out the draft message data if
            // there
            // is one but we also loaded the self metadata and need to let our listeners
            // know.
    ...
    /**
     * Create a MessageData object containing a copy of all the parts in this
     * DraftMessageData.
     *
     * @param clearLocalCopy whether we should clear out the in-memory copy of the
     *                       parts. If we
     *                       are simply pausing/resuming and not sending the
     *                       message, then we can keep
    ...
    public void addAttachments(final Collection<? extends MessagePartData> attachments) {
        // If the incoming attachments contains a single-only attachment, we need to
        // clear
        ...
        // If the existing attachments contain a single-only attachment, we need to
        // clear the
        ...
        // If any of the pending attachments contain a single-only attachment, we need
        // to clear the
        // existing attachments to make room for the incoming attachment.
    ...
    /**
     * Try to add one attachment to the attachment list, while guarding against
     * duplicates and
    ...
    private boolean addOneAttachmentNoNotify(final MessagePartData attachment) {
        ...
        for (final MessagePartData existingAttachment : mAttachments) {
            if (existingAttachment.getContentUri().equals(attachment.getContentUri())) {
                // Destroy existing attachment and replace with new attachment instead of
                // destroying
                // new one so that mSelectedImages in GalleryGridView could be maintained
                // correctly.
    ...
    public void updatePendingAttachment(final MessagePartData updatedAttachment,
        ...
        // If we are here, this means the pending attachment has been dropped before the
        // task
        // to load it was completed. In this case destroy the temporarily staged file
        // since it
        // is no longer needed.
     ...
     * 
     * @param flags typically this will be ATTACHMENTS_CHANGED. When attachments are
     *              cleared in a
    ...
    public boolean loadFromStorage(final BindingBase<DraftMessageData> binding,
        ...
        mIsDraftCachedCopy = false;
        // Before reading message from db ensure the caller is bound to us (and knows
        // the id)
    ...
    /**
     * Saves the current draft to db. This will save the draft and drop any pending
     * attachments
     * we have. The UI typically goes into the background when this is called, and
     * instead of
     * trying to persist the state of the pending attachments (the app may be
     * killed, the activity
    ...
    private void saveToStorageInternal(final BindingBase<DraftMessageData> binding) {
        // Create MessageData to store to db, but don't clear the in-memory copy so UI
        // will
        // continue to display it.
        // If self id is null then we'll not attempt to change the conversation's self
        // id.
        final MessageData message = createMessageWithCurrentAttachments(false /* clearLocalCopy */);
        // Before writing message to db ensure the caller is bound to us (and knows the
        // id)
    ...
    /**
     * Called when we are ready to send the message. This will assemble/return the
     * MessageData for
     * sending and clear the local draft data, both from memory and from DB. This
     * will also bind
     * the message data with a self Id through which the message will be sent.
     *
     * @param binding the binding object from our consumer. We need to make sure we
     *                are still bound
    ...
    public MessageData prepareMessageForSending(final BindingBase<DraftMessageData> binding) {
        ...
        final MessageData messageToSend = createMessageWithCurrentAttachments(true /* clearLocalCopy */);
    ...
    public void onReadDraftDataSucceeded(final ReadDraftDataAction action, final Object data,
        ...
        // Before passing draft message on to ui ensure the data is bound to the same
        // bindingid
    ...
    private void dispatchChanged(final int changeFlags) {
        ...
        // Any change in the draft will cancel any pending draft checking task, since
        // the
    ...
    /**
     * Drop any pending attachments that haven't finished. This is called after the
     * UI goes to
    ...
    public class CheckDraftForSendTask extends SafeAsyncTask<Void, Void, Integer> {
        ...
        public CheckDraftForSendTask(final boolean checkMessageSize, final int selfSubId,
            ...
            // Obtain an immutable copy of the attachment list so we can operate on it in
            // the
            // background thread.
         ...
         * 2. Computes the minimum size that this message could be
         * compressed/downsampled/encoded
