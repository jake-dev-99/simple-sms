...
/**
 * The base ImageRequest descriptor that describes the requirement of the
 * requested image
 * resource, including the desired size. It holds request info that will be
 * consumed by
...
public abstract class ImageRequestDescriptor extends MediaRequestDescriptor<ImageResource> {
    ...
    /**
     * Source size of the image (if known). This is used so that we don't have to
     * manually decode
    ...
    /**
     * A static image resource is required, even if the image format supports
     * animation (like Gif).
     */
    ...
    /**
     * Creates a new image request with unspecified width and height. In this case,
     * the full
     * bitmap is loaded and decoded, so unless you are sure that the image will be
     * of
     * reasonable size, you should consider limiting at least one of the two
     * dimensions
     * (for example, limiting the image width to the width of the ImageView
     * container).
     */
