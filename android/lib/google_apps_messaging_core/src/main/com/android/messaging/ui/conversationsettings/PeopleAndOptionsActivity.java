...
public class PeopleAndOptionsActivity extends BugleActionBarActivity {
    ...
    public void onAttachFragment(final Fragment fragment) {
        if (fragment instanceof PeopleAndOptionsFragment) {
            final String conversationId = getIntent().getStringExtra(UIIntents.UI_INTENT_EXTRA_CONVERSATION_ID);
            Assert.notNull(conversationId);
            final PeopleAndOptionsFragment peopleAndOptionsFragment = (PeopleAndOptionsFragment) fragment;
    ...
    public boolean onOptionsItemSelected(final MenuItem item) {
        switch (item.getItemId()) {
            case android.R.id.home:
                ...
                // ConversationActivity, it doesn't lose its original intent (conversation id
                // etc.)
                onBackPressed();
                return true;
