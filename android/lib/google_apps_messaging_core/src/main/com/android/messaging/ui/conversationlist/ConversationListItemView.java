...
public class ConversationListItemView extends FrameLayout implements OnClickListener,
    ...
    private final OnClickListener fullScreenPreviewClickListener = new OnClickListener() {
        ...
        public void onClick(final View v) {
            final String previewType = mData.getShowDraft() ? mData.getDraftPreviewContentType()
    ...
    private void setSubject() {
        final String subjectText = mData.getShowDraft() ? mData.getDraftSubject()
    ...
    // Resource Id of the secondary part of the content description for an edge case
    // of a message
    ...
    public static String buildContentDescription(final Resources resources,
        ...
        int resId = sPrimaryContentDescriptions[data.getIsGroup() ? CONV_TYPE_ONE_GROUP_INDEX
                : CONV_TYPE_ONE_ON_ONE_INDEX][outgoingSnippet ? DIRECTION_OUTGOING_INDEX
                        : DIRECTION_INCOMING_INDEX][messageStatusIndex];

        final String snippetText = data.getShowDraft() ? data.getDraftSnippetText() : data.getSnippetText();
        ...
        if (outgoingSnippet && data.getShowDraft() && data.getIsFailedStatus()) {
            ...
            String secondaryContentDescription = resources.getString(sSecondaryContentDescription);
     ...
     *
     * @param cursor The cursor from a ConversationList that this view is in,
     *               pointing to its
     *               entry.
     */
    ...
    /**
     * Notifies this view that it is undergoing animation. This view should disable
     * its click
     * targets.
     *
     * The animating counter is used to reset the swipe controller when the counter
     * becomes 0. A
