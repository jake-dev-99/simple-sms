...
/**
 * A cache-facing image resource that's much more compact than the raw Bitmap
 * objects stored in
 * {@link DecodedImageResource}.
 *
 * This resource is created from a regular Bitmap-based ImageResource before
 * being pushed to
 ...
 * decode task, which decodes the compressed byte array back to a regular image
 * resource to
...
public class EncodedImageResource extends ImageResource {
    ...
    /**
     * A MediaRequest that decodes the encoded image resource. This class is chained
     * to the
     * original media request that requested the image, so it inherits the listener
     * and
    ...
    private class DecodeImageRequest implements MediaRequest<ImageResource> {
        ...
        public MediaCache<ImageResource> getMediaCache() {
            // Decoded resource is non-cachable, it's for UI consumption only (for now at
            // least)
            return null;
        }
