...
/**
 * Represents a "pending" message part that acts as a placeholder for the actual
 * attachment being
 * loaded. It handles the task to load and persist the attachment from a Uri to
 * local scratch
 * folder. This item is not persisted to the database.
 */
public class PendingAttachmentData extends MessagePartData {
    /**
     * The pending state. This is the initial state where we haven't started loading
     * yet
     */
    public static final int STATE_PENDING = 0;

    /**
     * The state for when we are currently loading the attachment to the scratch
     * space
     */
    ...
    /**
     * The current state of the pending attachment. Refer to the STATE_* states
     * above
     */
     ...
     * 
     * @param sourceUri the source Uri of the attachment. The Uri maybe temporary or
     *                  remote,
    ...
    /**
     * Creates a pending attachment data that is able to load from the given source
     * uri and
    ...
    public void loadAttachmentForDraft(final DraftMessageData draftMessageData,
        ...
        // Kick off a SafeAsyncTask to load the content of the media and persist it
        // locally.
        // Note: we need to persist the media locally even if it's not remote, because
        // we
    ...
    public static final Parcelable.Creator<PendingAttachmentData> CREATOR = new Parcelable.Creator<PendingAttachmentData>() {
        @Override
