 ...
 *
 * Message Notifications are built in several stages with several utility
 * classes.
 * 1) Perform a database query and fill a data structure with information on
 * messages and
 * conversations which need to be notified.
 * 2) Based on the data structure choose an appropriate NotificationState
 * subclass to
 * represent all the notifications.
 * -- For one or more messages in one conversation:
 * MultiMessageNotificationState.
 * -- For multiple messages in multiple conversations:
 * MultiConversationNotificationState
 *
 * A three level structure is used to coalesce the data from the database. From
 * bottom to top:
 * 1) NotificationLineInfo - A single message that needs to be notified.
 * 2) ConversationLineInfo - A list of NotificationLineInfo in a single
 * conversation.
 * 3) ConversationInfoList - A list of ConversationLineInfo and the total number
 * of messages.
 *
 * The createConversationInfoList function performs the query and creates the
 * data structure.
 */
public abstract class MessageNotificationState extends NotificationState {
    ...
    public int getPriority() {
        // Returning PRIORITY_HIGH causes L to put up a HUD notification. Without it,
        // the ticker
        // isn't displayed.
    ...
    static class ConversationLineInfo {
        // Conversation id of the latest message in the notification for this merged
        // conversation.
        ...
        // Total number of messages. Might be different that mLineInfos.size() as the
        // number of
    ...
    public static class MultiConversationNotificationState extends MessageNotificationState {
        ...
        public final List<MessageNotificationState> mChildren = new ArrayList<MessageNotificationState>();
        ...
        protected NotificationCompat.Style build(final Builder builder) {
            ...
            for (int i = 0; i < mConvList.mConvInfos.size(); i++) {
                ...
                if (convInfo.mIsGroup) {
                    sender = (convInfo.mGroupConversationName.length() > MAX_CHARACTERS_IN_GROUP_NAME)
    ...
    /**
     * Truncate group conversation name to be displayed in the notifications. This
     * either truncates
     * the entire group name or finds the last comma in the available length and
     * truncates the name
     * at that point
     */
    private static String truncateGroupMessageName(final String conversationName) {
        ...
        for (int i = MAX_CHARACTERS_IN_GROUP_NAME; i >= 0; i--) {
            // The dividing marker should stay consistent with
            // ConversationListItemData.DIVIDER_TEXT
    ...
    /**
     * Notification for multiple messages in a single conversation. Also used if
     * there is a single
    ...
    public static class MultiMessageNotificationState extends MessageNotificationState {
        ...
        protected NotificationCompat.Style build(final Builder builder) {
            ...
            // At this point, all the messages come from the same conversation. We need to
            // load
            ...
            if (messageCount == 1) {
                ...
                if (mAttachmentUri != null && shouldShowImage) {
                    ...
                    // "picture".
                    final CharSequence tickerTag = BugleNotifications.formatAttachmentTag(authorFirstName,
                            mAttachmentType);
    ...
    // Essentially, we're building a list of the past 20 messages for this
    ...
    public static Notification buildConversationPageForWearable(final String conversationId,
        ...
        // Limit the number of messages to show. We just want enough to provide context
        // for the
        // notification. Fetch one more than we need, so we can tell if there are more
        // messages
        // before the one we're showing.
        // TODO: in the query, a multipart message will contain a row for each part.
        // We might need a smarter GROUP_BY. On the other hand, we might want to show
        // each of the
        ...
        try {
            ...
            do {
                ...
                // Track whether there are messages prior to the one(s) shown in the
                // notification.
        ...
        // If there is no conversation history prior to what is already visible in the
        // main
        ...
        // Indicate this by inserting an ellipsis at the beginning of the conversation
        // log.
        ...
        final NotificationCompat.Style notifStyle = new NotificationCompat.BigTextStyle(notifBuilder).bigText(bigText);
    ...
    /**
     * Notification for one or more messages in a single conversation, which is
     * bundled together
    ...
    private static ConversationInfoList createConversationInfoList() {
        // Map key is conversation id. We use LinkedHashMap to ensure that entries are
        // iterated in
        // the same order they were originally added. We scan unseen messages from
        // newest to oldest,
    ...
    /**
     * Scans all the attachments for a message and returns the most interesting one
     * that we'll
     * show in a notification. By order of importance, in case there are multiple
     * attachments:
    ...
    private static MessagePartData getMostInterestingAttachment(
        ...
        // 99.99% of the time there will be 0 or 1 part, since receiving slideshows is
        // so
        // uncommon.
    ...
    /**
     * Scans the database for messages that need to go into notifications. Creates
     * the appropriate
    ...
    public static NotificationState getNotificationState() {
        ...
        if (convList == null || convList.mConvInfos.size() == 0) {
            ...
            if (convList.mConvInfos.size() > 1) {
                // We've got notifications across multiple conversations. Pass in the
                // notification
    ...
    @Override
    public int getLatestMessageNotificationType() {
        // This function is called to determine whether the most recent notification
        // applies
        // to an sms conversation or a hangout conversation. We have different
        // ringtone/vibrate
     ...
     * MessageColumns.STATUS + "=" + EsProvider.MESSAGE_STATUS_FAILED_TO_SEND +
     * " OR (" +
