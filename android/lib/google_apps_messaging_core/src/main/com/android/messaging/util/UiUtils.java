...
public class UiUtils {
    ...
    public static final int MEDIAPICKER_TRANSITION_DURATION = getApplicationContext().getResources().getInteger(
            R.integer.mediapicker_transition_duration);
    /** Short transition duration in ms */
    public static final int ASYNCIMAGE_TRANSITION_DURATION = getApplicationContext().getResources().getInteger(
            R.integer.asyncimage_transition_duration);
    /** Compose transition duration in ms */
    public static final int COMPOSE_TRANSITION_DURATION = getApplicationContext().getResources().getInteger(
            R.integer.compose_transition_duration);
    /** Generic duration for revealing/hiding a view */
    public static final int REVEAL_ANIMATION_DURATION = getApplicationContext().getResources().getInteger(
    ...
    public static void doOnceAfterLayoutChange(final View view, final Runnable runnable) {
        final OnLayoutChangeListener listener = new OnLayoutChangeListener() {
            ...
            public void onLayoutChange(final View v, final int left, final int top, final int right,
                    ...
                    final int oldBottom) {
                // Call the runnable outside the layout pass because very few actions are
                // allowed in
                // the layout pass
     ...
     * @param desiredVisibility desired visibility (e.g. View.GONE) for the animated
     *                          view.
    ...
    public static void revealOrHideViewWithAnimation(final View view, final int desiredVisibility,
        ...
        if (needAnimation) {
            ...
            final ScaleAnimation showHideAnimation = new ScaleAnimation(fromScale, toScale, fromScale, toScale,
            ...
            // We are playing a view Animation; unlike view property animations, we can
            // commit the
    ...
    public static void lockOrientation(final Activity activity) {
        ...
        // It is possible to have a rotation of 0 (device in its natural orientation) in
        // portrait
        // (phone), or in landscape (tablet), so we have to check both values to
        // determine what to
     ...
     * 
     * @return true if {@link Activity#finish()} was called because redirection was
     *         performed
     */
    ...
    /*
     * Removes all html markup from the text and replaces links with the the text
     * and a text version
     * of the href.
     * 
    ...
    public static void setActionBarShadowVisibility(final AppCompatActivity activity, final boolean visible) {
        ...
        actionBar.setElevation(
                visible ? activity.getResources().getDimensionPixelSize(R.dimen.action_bar_elevation) : 0);
        final View actionBarView = activity.getWindow().getDecorView().findViewById(
                androidx.appcompat.R.id.decor_content_parent);
        if (actionBarView != null) {
            // AppCompatActionBar has one drawable Field, which is the shadow for the action
            // bar
    ...
    /**
     * Get the activity that's hosting the view, typically casting view.getContext()
     * as an Activity
     * is sufficient, but sometimes the context is a context wrapper, in which case
     * we need to case
     * the base context
     */
    ...
    /**
     * Get the activity for the supplied context, typically casting context as an
     * Activity
     * is sufficient, but sometimes the context is a context wrapper, in which case
     * we need to case
     * the base context
