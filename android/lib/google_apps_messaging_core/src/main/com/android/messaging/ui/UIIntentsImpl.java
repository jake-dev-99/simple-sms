...
public class UIIntentsImpl extends UIIntents {
    private static final String CELL_BROADCAST_LIST_ACTIVITY = "com.android.cellbroadcastreceiver.CellBroadcastListActivity";
    private static final String CALL_TARGET_CLICK_KEY = "touchPoint";
    private static final String CALL_TARGET_CLICK_EXTRA_KEY = "android.telecom.extra.OUTGOING_CALL_EXTRAS";
    private static final String MEDIA_SCANNER_CLASS = "com.android.providers.media.MediaScannerService";
    ...
    private Intent getConversationActivityIntent(final Context context,
        ...
        // Always try to reuse the same ConversationActivity in the current task so that
        // we don't
        ...
        if (draft != null) {
            ...
            // If draft attachments came from an external content provider via a share
            // intent, we
            // need to propagate the URI permissions through to ConversationActivity. This
            // requires
            // putting the URIs into the ClipData (setData also works, but accepts only one
            // URI).
        ...
        if (!(context instanceof Activity)) {
            // If the caller supplies an application context, and not an activity context,
            // we must
            // include this flag
    ...
    public void launchAddContactActivity(final Context context, final String destination) {
        ...
        final String destinationType = MmsSmsUtils.isEmailAddress(destination) ? Intents.Insert.EMAIL
    ...
    public void launchFullScreenPhotoViewer(final Activity activity, final Uri initialPhoto,
        ...
        final PhotoViewIntentBuilder builder = com.android.ex.photo.Intents.newPhotoViewIntentBuilder(
        ...
        // Set the location of the imageView so that the photoviewer can animate from
        // that location
        // to full screen.
    ...
    /**
     * Gets a PendingIntent associated with an Intent to start an Activity. All
     * notifications
     * that starts an Activity must use this method to get a PendingIntent, which
     * achieves two
     * goals:
     * 1. The target activities will be created, with any existing ones destroyed.
     * This ensures
     * we don't end up with multiple instances of ConversationListActivity, for
     * example.
     * 2. The target activity, when launched, will have its backstack correctly
     * constructed so
    ...
    private static PendingIntent getPendingIntentWithParentStack(final Context context,
        ...
        final PendingIntent resultPendingIntent = stackBuilder.getPendingIntent(requestCode,
                PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    public PendingIntent getWidgetPendingIntentForConversationActivity(final Context context,
        ...
        if (conversationId != null) {
            ...
            // Set the action to something unique to this conversation so if someone calls
            // this
            // function again on a different conversation, they'll get a new PendingIntent
            // instead
            // of the old one.
