...
public class AsyncImageView extends androidx.appcompat.widget.AppCompatImageView
        implements MediaResourceLoadListener<ImageResource> {
    private static final String TAG = LogUtil.BUGLE_DATAMODEL_TAG;
    // 100ms delay before disposing the image in case the AsyncImageView is re-added
    // to the UI
    private static final int DISPOSE_IMAGE_DELAY = 100;

    // AsyncImageView has a 1-1 binding relationship with an ImageRequest instance
    // that requests
    // the image from the MediaResourceManager. Since the request is done
    // asynchronously, we
    // want to make sure the image view is always bound to the latest image request
    // that it
    // issues, so that when the image is loaded, the ImageRequest (which extends
    // BindableData)
    // will be able to figure out whether the binding is still valid and whether the
    // loaded image
    ...
    /**
     * True if we want the image to reveal (scale) when it loads. When set to true,
     * this
    ...
    // A placeholder drawable that takes the spot of the image when it's loading.
    // The default
    ...
    private final Runnable mDisposeRunnable = new Runnable() {
        ...
        public void run() {
            if (mImageRequestBinding.isBound()) {
                mDetachedRequestDescriptor = (ImageRequestDescriptor) mImageRequestBinding.getData().getDescriptor();
            }
            unbindView();
    ...
    public void setImageResourceId(@Nullable final ImageRequestDescriptor descriptor) {
        ...
        if (!TextUtils.isEmpty(requestKey)) {
            ...
            final BindableMediaRequest<ImageResource> imageRequest = descriptor.buildAsyncMediaRequest(getContext(),
                    this);
    ...
    private void maybeSetupPlaceholderDrawable(final ImageRequestDescriptor descriptor) {
        if (!TextUtils.isEmpty(descriptor.getKey()) && mPlaceholderDrawable != null) {
            if (descriptor.sourceWidth != ImageRequest.UNSPECIFIED_SIZE &&
                ...
                // Set a transparent inset drawable to the foreground so it will mimick the
                // final
    ...
    protected void setImage(final ImageResource resource, final boolean isCached) {
        ...
        // resource and addRef() on the new resource so that the underlying bitmaps
        // don't
        ...
        // The drawable may require work to get if its a static object so try to only
        // make this call
        // once.
    ...
    /**
     * Resets transient view states (eg. alpha, animations) before rebinding/reusing
     * the view.
     */
    ...
    protected void onAttachedToWindow() {
        ...
        // When the image view gets detached and immediately re-attached, any fade-in
        // animation
        // will be terminated, leaving the view in a semi-transparent state. Make sure
        // we restore
        ...
        // Check whether we are in a simple reuse scenario: detached from window, and
        // reattached
        // later without rebinding. This may be done by containers such as the
        // RecyclerView to
        // reuse the views. In this case, we would like to rebind the original image
        // request.
    ...
    protected void onDetachedFromWindow() {
        ...
        // Dispose the bitmap, but if an AysncImageView is removed from the window, then
        // quickly
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // The base implementation does not honor the minimum sizes. We try to to honor
        // it here.

        final int measuredWidth = getMeasuredWidth();
        final int measuredHeight = getMeasuredHeight();
        if (measuredWidth >= getMinimumWidth() || measuredHeight >= getMinimumHeight()) {
            // We are ok if either of the minimum sizes is honored. Note that satisfying
            // both the
            // sizes may not be possible, depending on the aspect ratio of the image and
            // whether
            // a maximum size has been specified. This implementation only tries to handle
            // the case
        ...
        if (!getAdjustViewBounds()) {
            // The base implementation is reasonable in this case. If the view bounds cannot
            // be
    ...
    /**
     * As a performance optimization, the consumer of the AsyncImageView may opt to
     * delay loading
     * the image when it's busy doing other things (such as when a list view is
     * scrolling). In
     * order to do this, the consumer can create a new AsyncImageViewDelayLoader
     * instance to be
     * shared among all relevant AsyncImageViews (through setDelayLoader() method),
     * and call
