...
public class ConversationListSwipeHelper implements OnItemTouchListener {
    ...
    /**
     * Animate the dismissal of the given item. The given velocityX is taken into
     * consideration for
     * the animation duration. Whether the item is dismissed to the left or right is
     * dependent on
     * the given velocityX.
     */
    ...
    private boolean isTargetSwipedFastEnough() {
        ...
        return (Math.abs(velocityX) > minVelocity) // Fast enough.
                ...
                && (velocityX > 0) == (translationX > 0) // Right direction.
                && Math.abs(translationX) > FLING_PERCENTAGE_OF_WIDTH_TO_DISMISS * width; // Enough movement.
    }

    /**
     * Only used during a swipe gesture. Determine if the swipe has enough distance
     * to be
     * dismissed.
     */
