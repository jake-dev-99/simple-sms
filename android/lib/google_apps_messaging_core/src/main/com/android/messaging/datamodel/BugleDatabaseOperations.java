...
public class BugleDatabaseOperations {
    ...
    // Global cache of phone numbers -> participant id mapping since this call is
    // expensive.
    private static final ArrayMap<String, String> sNormalizedPhoneNumberToParticipantIdCache = new ArrayMap<String, String>();

    /**
     * Convert list of recipient strings (email/phone number) into list of
    ...
    /**
     * Sanitize a given list of conversation participants by de-duping and stripping
     * out self
    ...
    /**
     * Convert list of ConversationParticipants into recipient strings (email/phone
     * number)
     */
    @DoesNotRunOnMainThread
     ...
     *
     * NOTE: There are phones on which you can't get the recipients from the thread
     * id for SMS
     * until you have a message, so use getOrCreateConversationFromRecipient
     * instead.
     *
    ...
    public static String getOrCreateConversationFromThreadId(final DatabaseWrapper db,
        ...
        final ArrayList<ParticipantData> participants = getConversationParticipantsFromRecipients(recipients, refSubId);
     ...
     * @param db             the database
     * @param threadId       The message's thread
     * @param archived       Flag whether the conversation should be created
     *                       archived
    ...
    public static String getOrCreateConversation(final DatabaseWrapper db, final long threadId,
        ...
        if (conversationId == null) {
            ...
            // Create the conversation with the default self participant which always maps
            // to
     ...
     * @return The thread id. Returns -1 if the conversation was not found or if it
     *         was found
     ...
     *
     * It's up to the caller to make sure that this is all inside a transaction. It
     * will return
    ...
    static String createConversationInTransaction(final DatabaseWrapper dbWrapper,
        ...
        // Start with conversation hidden - sending a message or saving a draft will
        // change that
        ...
        // Now fully resolved participants available can update conversation name /
        // avatar.
        // b/16437575: We cannot use the participants directly, but instead have to call
        // getParticipantsForConversation() to retrieve the actual participants. This is
        // needed
        // because the call to addParticipantToConversation() won't fill up the
        // ParticipantData
        // if the participant already exists in the participant table. For example, say
        // you have
        // an existing conversation with John. Now if you create a new group
        // conversation with
        // Jeff & John with only their phone numbers, then when we try to add John's
        // number to the
        // group conversation, we see that he's already in the participant table,
        // therefore we
        // short-circuit any steps to actually fill out the ParticipantData for John
        // other than
        // just returning his participant id. Eventually, the ParticipantData we have is
        // still the
        // raw data with just the phone number. getParticipantsForConversation(), on the
        // other
        // hand, will fill out all the info for each participant from the participants
        // table.
    ...
    public static boolean deleteConversation(final DatabaseWrapper dbWrapper,
        ...
        try {
            ...
            if (cutoffTimestamp == Long.MAX_VALUE) {
                ...
                // because under certain scenarios (e.g. sending messages in progress), the
                // draft
                // timestamp can be larger than the cutoff time, which is generally the
                // conversation
                // sort timestamp. Because of how the sms/mms provider works on some newer
                // devices, it's important that we never delete all the messages in a
                // conversation
    ...
    public static void updateConversationMetadataInTransaction(final DatabaseWrapper dbWrapper,
        ...
        // When the conversation gets updated with new messages, unarchive the
        // conversation unless
        ...
        // Conversation always exists as this method is called from ActionService only
        // after
        ...
        if (shouldAutoSwitchSelfId && OsUtil.isAtLeastL_MR1()) {
            // Normally, the draft message compose UI trusts its UI state for providing
            // up-to-date
            // conversation self id. Therefore, notify UI through local broadcast receiver
            // about
    ...
    /**
     * Adds self-id auto switch info for a conversation if the last message has a
     * different
    ...
    static boolean addSelfIdAutoSwitchInfoToContentValues(final DatabaseWrapper dbWrapper,
        ...
        // Get the sub IDs in effect for both the message and the conversation and
        // compare them:
        // 1. If message is unbound (using default sub id), then the message was sent
        // with
        // pre-MSIM support. Don't auto-switch because we don't know the subscription
        // for the
        // message.
        // 2. If message is bound,
        // i. If conversation is unbound, use the system default sub id as its effective
        // sub.
        // ii. If conversation is bound, use its subscription directly.
        // Compare the message sub id with the conversation's effective sub id. If they
        // are
        ...
        final int conversationEffectiveSubId = PhoneUtils.getDefault().getEffectiveSubId(conversationSelf.getSubId());
    ...
    private static void updateConversationNameAndAvatarInTransaction(
        ...
        final boolean hasAnyEnterpriseContact = ConversationListItemData.hasAnyEnterpriseContact(participants);
        values.put(ConversationColumns.IS_ENTERPRISE, hasAnyEnterpriseContact);

        fillParticipantData(values, participants);

        // Used by background thread when refreshing conversation so conversation could
        // be deleted.
        updateConversationRowIfExists(dbWrapper, conversationId, values);
     ...
     * @param checkAttachmentFilesExist check each attachment file and only include
     *                                  if file exists
     */
            ...
            while (cursor.moveToNext()) {
                ...
                if (checkAttachmentFilesExist && messagePartData.isAttachment() &&
                    ...
                    try {
                        ...
                        final ParcelFileDescriptor fileDescriptor = contentResolver.openFileDescriptor(
     ...
     * <p>
     * This is necessary because the telephony database has a trigger that deletes
     * threads after
     * their last message is deleted. We need to ensure that if a thread goes away,
     * we also delete
     * the conversation in Bugle. We don't store draft messages in telephony, so we
     * ignore those
    ...
    /**
     * Update conversation snippet, timestamp and optionally self id to match latest
     * message in
     * conversation.
     */
    @DoesNotRunOnMainThread
    ...
    /**
     * When moving/removing an existing message update conversation metadata if
     * necessary
     *
     ...
     * @param shouldAutoSwitchSelfId should we try to auto-switch the conversation's
     *                               self-id as a
     *                               result of this call when we see a new latest
     *                               message?
     * @param keepArchived           should we keep the conversation archived
     *                               despite refresh
     */
    @DoesNotRunOnMainThread
    ...
    /**
     * Note this is not thread safe so callers need to make sure they own the
     * wrapper + statements
     ...
     * @param message        Optional message to preserve attachments for (either as
     *                       draft or for
     *                       sending)
     ...
     *
     * @return If the participant is available in our cache, or the DB, this returns
     *         the
     *         participant id for the given subid/phone number. Otherwise it returns
     *         null.
     */
    @VisibleForTesting
    ...
    /**
     * Refresh conversation names/avatars based on a list of participants that are
     * changed.
     */
    @DoesNotRunOnMainThread
