...
/**
 * Requests and parses VCard data. In Bugle, we need to display VCard details in
 * the conversation
 * view such as avatar icon and name, which can be expensive if we parse VCard
 * every time.
 * Therefore, we'd like to load the vcard once and cache it in our media cache
 * using the
 * MediaResourceManager component. To load the VCard, we use framework's VCard
 * support to
 * interpret the VCard content, which gives us information such as phone and
 * email list, which
 * we'll put in VCardResource object to be cached.
 *
 * Some particular attention is needed for the avatar icon. If the VCard
 * contains avatar icon,
 * it's in byte array form that can't easily be cached/persisted. Therefore, we
 * persist the
 * image bytes to the scratch directory and generate a content Uri for it, so
 * that ContactIconView
...
public class VCardRequest implements MediaRequest<VCardResource> {
    ...
    public VCardResource loadMediaBlocking(List<MediaRequest<VCardResource>> chainedTask)
        ...
        // The VCard library doesn't support synchronously loading the media resource.
        // Therefore,
        // We have to burn the thread waiting for the result to come back.
        final CountDownLatch signal = new CountDownLatch(1);
        if (!parseVCard(mDescriptor.vCardUri, signal)) {
            // Directly fail without actually going through the interpreter, return
            // immediately.
    ...
    private boolean doActuallyReadOneVCard(final Uri uri, final boolean showEntryParseProgress,
        ...
        final CustomVCardEntryConstructor builder = new CustomVCardEntryConstructor(vcardType, null);
    ...
    class ContactVCardEntryHandler implements CustomVCardEntryConstructor.EntryHandler {
        ...
        public void onEntryCreated(final CustomVCardEntry entry) {
            ...
            if (photos != null && photos.size() > 0) {
                // The photo data is in bytes form, so we need to persist it in our temp
                // directory
        ...
        @Override
        public void onEnd() {
            // Finished loading all vCard entries, signal the loading thread to proceed with
            // the
            // result.
