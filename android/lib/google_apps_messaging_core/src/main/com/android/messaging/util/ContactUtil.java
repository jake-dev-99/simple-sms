...
public class ContactUtil {
     ...
     * should confirm to
     * this column content and ordering so that caller can use the uniformed way to
     * process
     * returned cursors.
     */
    ...
    // An optional _id column to query results that need to be displayed in a list
    // view.
    ...
    public static class PhoneLookupQuery {
        public static final String[] PROJECTION = new String[] {
                ...
                // PhoneLookup. This is okay because the _id field serves as both the data id
                // and
                // contact id. Also we never show the results directly in a list view so we are
                // not
    ...
    public static class SelfQuery {
        public static final String[] PROJECTION = new String[] {
                ...
                // Phone number, type, label and data_id is not provided in this projection
                // since
    ...
    public static void showOrAddContact(final View view, final long contactId,
        ...
        if (contactId > ParticipantData.PARTICIPANT_CONTACT_ID_NOT_RESOLVED
            ...
            final Uri lookupUri = ContactsContract.Contacts.getLookupUri(contactId, contactLookupKey);
    ...
    /**
     * Get a list of phones sorted by contact name. One contact may have multiple
     * phones.
     * In that case, each phone will be returned as a separate record in the result
     * cursor.
     */
    @VisibleForTesting
    public static CursorQueryData getPhones(final Context context) {
        ...
        // query parameter that removes duplicate (raw) numbers. Unfortunately, we can't
        // use that
        // because it causes the some phones' contacts provider to return incorrect
        // sections.
     ...
     * relatively complete
     * one for this to succeed. PhoneLookup / EmailLookup URI will apply some
     * smartness to do a
    ...
    /**
     * Returns whether the search text indicates an email based search or a phone
     * number based one.
     */
    ...
    /**
     * Get a list of destinations (phone, email) matching the partial destination in
     * work profile.
     */
    ...
    /**
     * Get a list of phones matching a search criteria. The search may be on contact
     * name or
     * phone number. In case search is on contact name, all matching contact's phone
     * number
     * will be returned.
     * NOTE: This is visible for testing only, clients should only call
     * filterDestination() since
    ...
    /**
     * Similar to {@link #filterPhones(Context, String)}, but search in work profile
     * instead.
     */
    ...
    /**
     * Lookup a phone based on a phone number. Supplied phone should be a relatively
     * complete
     * phone number for this to succeed. PhoneLookup URI will apply some smartness
     * to do a
     * loose match to see whether there is a contact that matches this phone.
     * NOTE: This is visible for testing only, clients should only call
     * lookupDestination() since
     * we support email addresses as well.
     */
    @VisibleForTesting
     ...
     * Contacts.CONTENT_STREQUENT_URI, which
    ...
    /**
     * Get a list of emails matching a search criteria. In Bugle, since email is not
     * a common
     * usage scenario, we should only do email search after user typed in a query
     * indicating
    ...
    /**
     * Similar to {@link #filterEmails(Context, String)}, but search in work profile
     * instead.
     */
    ...
    /**
     * Lookup emails based a complete email address. Since there is no special logic
     * needed for
     ...
     *
     * @param primaryOnly If there are multiple raw contacts, set this flag to
     *                    return only the
     *                    name used as the primary display name. Otherwise, this
     *                    method returns all names.
     */
     ...
     * cursor).
     *
     * @param firstLevel whether this item is the first entry of this contact in the
     *                   list.
     */
    ...
    /**
     * Creates a RecipientEntry for PhoneQuery result. The result is then displayed
     * in the
