...
public class SyncManager {
    ...
    /**
     * Timestamp of current sync batch upper bound - used to determine if message
     * makes batch dirty
     */
    private long mCurrentUpperBoundTimestamp = -1;

    /**
     * Timestamp of messages inserted since sync batch started - used to determine
     * if batch dirty
     */
    ...
    /**
     * User customization to conversations. If this is set, we need to recover them
     * after
     * a full sync.
     */
    ...
    /**
     * Called from data model thread at end of batch to determine if any messages
     * added in window
     * 
     * @param lowerBoundTimestamp lower bound timestamp for sync batch
     * @return true if message added within window from lower to upper bound
     *         timestamp of batch
     */
    ...
    /**
     * Called from data model or background worker thread to indicate start of
     * message add process
     * (add must complete on that thread before action transitions to new
     * thread/stage)
     * 
     ...
     * 
     * @param full           - true indicates a full (not incremental) sync
     *                       operation
    ...
    /**
     * Return delay (in ms) until allowed to run a full sync (0 meaning can run
     * immediately)
     * 
    ...
    /**
     * Check if sync batch should be in progress - compares upperBound with in
     * memory value
     * 
     * @param upperBoundTimestamp - upperbound timestamp for sync batch
     * @return - true if timestamps match (otherwise batch is orphan from older
     *         process)
     */
    ...
    public boolean getHasFirstSyncCompleted() {
        ...
        return prefs.getLong(BuglePrefsKeys.LAST_SYNC_TIME,
                BuglePrefsKeys.LAST_SYNC_TIME_DEFAULT) != BuglePrefsKeys.LAST_SYNC_TIME_DEFAULT;
    }
    ...
    public void updateSyncObserver(final Context context) {
        ...
        // Trigger an sms sync in case we missed and messages before registering this
        // observer or
    ...
    private void registerObserver(final Context context) {
        if (!PhoneUtils.getDefault().isDefaultSmsApp()) {
            ...
            // Secondary users default SMS app - need to actively monitor telephony and
            // notify
            mNotifyOnChanges = true;
            mSyncOnChanges = true;
        } else {
            // Primary users default SMS app - don't monitor telephony (most changes from
            // this app)
            mNotifyOnChanges = false;
    ...
    private class TelephonyMessagesObserver extends ContentObserver {
        ...
        // the onChange(boolean, Uri) method to ensure correct operation on older
        // versions
    ...
    public static class ThreadInfoCache {
         ...
         * @param refSubId      The subId used for normalizing phone numbers in the
         *                      thread
         * @param customization The user setting customization to the conversation if
         *                      any
        ...
        public synchronized String getOrCreateConversation(final DatabaseWrapper db,
            ...
            final ArrayList<ParticipantData> participants = BugleDatabaseOperations
                    .getConversationParticipantsFromRecipients(recipients,
                            refSubId);
