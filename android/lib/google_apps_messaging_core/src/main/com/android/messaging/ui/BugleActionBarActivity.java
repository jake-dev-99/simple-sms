...
/**
 * Base class for app activities that use an action bar. Responsible for
 * logging/telemetry/other
 * needs that will be common for all activities. We can break out the common
 * code if/when we need
...
public class BugleActionBarActivity extends AppCompatActivity implements ImeUtil.ImeStateHost {
    ...
    // The ActionMode that represents the modal contextual action bar, using our own
    // implementation
    ...
    // Used to determine if a onDisplayHeightChanged was due to the IME opening or
    // rotation of the
    // device
    ...
    /**
     * Receives and handles action bar invalidation request from sub-components of
     * this activity.
     *
     * <p>
     * Normally actions have sole control over the action bar, but in order to
     * support seamless
     * transitions for components such as the full screen media picker, we have to
     * let it take over
     * the action bar and then restore its state afterwards
     * </p>
     *
     * <p>
     * If a fragment does anything that may change the action bar, it should call
     * this method
     * and then it is this method's responsibility to figure out which component
     * "controls" the
     * action bar and delegate the updating of the action bar to that component
     * </p>
     */
    ...
    /**
     * Custom ActionMode implementation which allows us to just replace the contents
     * of the main
