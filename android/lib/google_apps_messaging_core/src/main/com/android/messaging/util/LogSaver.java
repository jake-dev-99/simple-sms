...
public abstract class LogSaver {
    /**
     * Writes the accumulated log entries, from oldest to newest, to the specified
     * PrintWriter.
     * Log lines are emitted in much the same form as logcat -v threadtime --
     * specifically,
    ...
    /**
     * A circular in-memory log to be used to log potentially verbose logs. The logs
     * will be
     * persisted in memory in the application and can be dumped by various dump()
     * methods.
    ...
    private static final class MemoryLogSaver extends LogSaver {
        /**
         * Record to store a single log entry. Stores timestamp, tid, level, tag, and
         * message.
         * It can be reused when the circular log rolls over. This avoids creating new
         * objects.
         */
    ...
    /**
     * A persistent, on-disk log saver. It uses the standard Java util logger along
     * with
     * a rotation log file set to store the logs in app's local file directory
     * "app_logs".
     */
