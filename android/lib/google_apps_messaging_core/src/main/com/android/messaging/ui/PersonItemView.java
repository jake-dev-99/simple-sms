...
/**
 * Shows a view for a "person" - could be a contact or a participant. This
 * always shows a
 * contact icon on the left, and the person's display name on the right.
 *
 * This view is always bound to an abstract PersonItemData class, so to use it
 * for a specific
 * scenario, all you need to do is to create a concrete PersonItemData subclass
 * that bridges
 * between the underlying data (e.g. ParticipantData) and what the UI wants
 * (e.g. display name).
 */
public class PersonItemView extends LinearLayout implements PersonItemDataListener,
    ...
    /**
     * When set to true, we display only the avatar of the person and hide
     * everything else.
     */
