...
/**
 * Provides utility methods around creating RecipientEntry instance specific to
 * Bugle's needs.
 */
public class ContactRecipientEntryUtils {
    /**
     * A special contact id for generated contacts with no display name (number
     * only) and avatar.
     * By default, the chips UI doesn't load any avatar for chips with no display
     * name, or where
     * the display name is the same as phone number (which is true for unknown
     * contacts).
     * Since Bugle always generate a default avatar for all contacts, this is used
     * to replace
    ...
    /**
     * A generated special contact which says "Send to xxx" in the contact list,
     * which allows
    ...
    private static RecipientEntry constructSpecialRecipientEntry(final String destination,
        ...
        // For the send-to-destination (e.g. "Send to xxx" in the auto-complete
        // drop-down)
        // we want to show a default avatar with a static background so that it doesn't
        // flicker
        // as the user types.
        final Uri avatarUri = contactId == CONTACT_ID_SENDTO_DESTINATION ? AvatarUriUtil.DEFAULT_BACKGROUND_AVATAR
                : null;
    ...
    /**
     * Gets the display name for contact list only. For most cases this is the same
     * as the normal
     * contact name, but there are cases where these two differ. For example, for
     * the
     * send to typed number item, we'd like to show "Send to xxx" in the contact
     * list. However,
     * when this item is actually added to the chips edit box, we would like to show
     * just the
