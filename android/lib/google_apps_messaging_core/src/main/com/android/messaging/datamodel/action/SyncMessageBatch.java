...
/**
 * Update local database with a batch of messages to add/delete in one
 * transaction
 */
class SyncMessageBatch {
    ...
    private void storeSms(final DatabaseWrapper db, final SmsMessage sms) {
        ...
        final ParticipantData sender = isOutgoing ? self
        ...
        // Keep track of updated conversation for later updating the conversation
        // snippet, etc.
    ...
    private void storeMms(final DatabaseWrapper db, final MmsMessage mms) {
        ...
        final String selfId = BugleDatabaseOperations.getOrCreateParticipantInTransaction(db, self);
        final ParticipantData sender = isOutgoing ? self
                : ParticipantData.getFromRawPhoneBySimLocale(senderId, mms.getSubId());
        final String participantId = (isOutgoing ? selfId
                : BugleDatabaseOperations.getOrCreateParticipantInTransaction(db, sender));
        ...
        // database. Ideally we would load and parse the SMIL which describes how the
        // parts relate
        // to one another.
    ...
    private static void rethrowSQLiteConstraintExceptionWithDetails(SQLiteConstraintException e,
        ...
        // Add some extra debug information to the exception for tracking down
        // b/18375758.
        // The default detail message for SQLiteConstraintException tells us that a
        // foreign
        // key constraint failed, but not which one! Messages have foreign keys to 3
        // tables:
        // conversations, participants (self), participants (sender). We'll query each
        // one
        // to determine which one(s) violated the constraint, and then throw a new
        // exception
        // with those details.
        ...
        ParticipantData foundSelfParticipant = BugleDatabaseOperations.getExistingParticipant(db, selfId);
        ParticipantData foundSenderParticipant = BugleDatabaseOperations.getExistingParticipant(db, senderId);
    ...
    /**
     * Batch delete database rows by matching a column with a list of values,
     * usually some
     * kind of IDs.
     *
     * @param table
