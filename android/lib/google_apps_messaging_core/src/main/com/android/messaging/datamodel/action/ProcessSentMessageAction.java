 ...
 * Can also update the message itself if a "final" message is now available from
 * telephony db
 */
public class ProcessSentMessageAction extends Action {
    ...
    // These are set when we're processing a message sent by the user. They are null
    // for messages
    // sent automatically (e.g. a NotifyRespInd/AcknowledgeInd sent in response to a
    // download).
    ...
    // These are set for messages we sent ourself (legacy), or which we fast-failed
    // before sending.
        ...
        if (sentByPlatform) {
            ...
            if (resultCode == Activity.RESULT_OK) {
                if (responseImportant) {
                    ...
                    if (sendConf != null) {
                        final MmsUtils.StatusPlusUri result = MmsUtils.updateSentMmsMessageStatus(context, messageUri,
                                sendConf);
    ...
    static void processResult(final String messageId, Uri updatedMessageUri, int status,
        ...
        // We have special handling for when a message to an emergency number fails. In
        // this case,
        // we notify immediately of any failure (even if we auto-retry), and instruct
        // the user to
        ...
        // Update the message status and optionally refresh the message with final
        // parts/values.
        if (SendMessageAction.updateMessageAndStatus(isSms, message, updatedMessageUri, failed)) {
            // We shouldn't show any notifications if we're not allowed to modify Telephony
            // for
            // this message.
            if (failed) {
        ...
        // Whether we succeeded or failed we will check and maybe schedule some more
        // work
        ProcessPendingMessagesAction.scheduleProcessPendingMessagesAction(
    ...
    public static final Parcelable.Creator<ProcessSentMessageAction> CREATOR = new Parcelable.Creator<ProcessSentMessageAction>() {
        @Override
