...
class MmsVideoRecorder extends MediaRecorder {
    ...
    public MmsVideoRecorder(final Camera camera, final int cameraIndex, final int orientation,
        ...
        // The QUALITY_LOW profile might not be low enough to allow for video of a
        // reasonable
        // minimum duration. Adjust a/v bitrates to allow at least MIN_DURATION_LIMIT
        // video
        // to be recorded.
        ...
        if (initialDurationLimit < MIN_DURATION_LIMIT_SECONDS) {
            // Reduce the suggested bitrates. These bitrates are only requests, if
            // implementation
            // can't actually hit these goals it will still record video at higher rate and
            // stop when
