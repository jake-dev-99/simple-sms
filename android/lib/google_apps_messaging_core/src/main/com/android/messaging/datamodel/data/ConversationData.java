...
public class ConversationData extends BindableData {
    ...
    /**
     * A trampoline class so that we can inherit from LoaderManager.LoaderCallbacks
     * multiple times.
     */
    ...
    public void init(final LoaderManager loaderManager,
        ...
        // Remember the binding id so that loader callbacks can check if data is still
        // bound
    ...
    protected void unregisterListeners() {
        ...
        // TODO: May false trigger if destroy happens after "new" conversation is
        // focused.
    ...
    /**
     * Gets the default self participant in the participant table (NOT the
     * conversation's self).
    ...
    public void sendMessage(final BindingBase<ConversationData> binding,
        ...
        if (!OsUtil.isAtLeastL_MR1() || message.getSelfId() == null) {
            ...
            if (systemDefaultSubId != ParticipantData.DEFAULT_SELF_SUB_ID &&
                ...
                // Lock the sub selection to the system default SIM as soon as the user clicks
                // on
                // the send button to avoid races between this and when InsertNewMessageAction
                // is
                // actually executed on the data model thread, during which the user can
                // potentially
    ...
    public void deleteConversation(final Binding<ConversationData> binding) {
        Assert.isTrue(binding.getData() == this);
        // If possible use timestamp of last message shown to delete only messages user
        // is aware of
    ...
    /**
     * Returns a dialable phone number for the participant if we are in a 1-1
     * conversation.
     * 
     * @return the participant phone number, or null if the phone number is not
     *         valid or if there
    ...
    public MessageData createForwardedMessage(final ConversationMessageData message) {
        ...
        for (final MessagePartData part : message.getParts()) {
            ...
            // Depending on the part type, if it is text, we can directly create a text
            // part;
            // if it is attachment, then we need to create a pending attachment data out of
            // it, so
            // that we may persist the attachment locally in the scratch folder when the
            // user picks
    ...
    /**
     * Returns {@link SubscriptionListData
     * .SubscriptionListEntry} for a given self participant so UI can display
     * SIM-related info
    ...
    /**
     * A placeholder implementation of {@link ConversationDataListener} so that
     * subclasses may opt
