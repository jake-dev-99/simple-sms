...
public class MessageDetailsDialog {
    ...
    /**
     * Return a string, separated by newlines, that contains a number of labels and
     * values
    ...
    private static String getMmsMessageDetails(Context context, final ConversationMessageData data,
        ...
        // the case when the message is a PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND and
        // display
        // something different. See the Messaging app's
        // MessageUtils.getNotificationIndDetails()
        ...
        details.append('\n');
        details.append(res.getString(R.string.from_label));
        details.append(!TextUtils.isEmpty(rawSender) ? rawSender : res.getString(R.string.hidden_sender_address));
    ...
    private static void appendDebugInfo(StringBuilder details, ConversationMessageData data) {
        // We grab the thread id from the database, so this needs to run in the
        // background
        ...
        if (self.isActiveSubscription() && !self.isDefaultSelf()) {
            ...
            if (TextUtils.isEmpty(subscriptionName)) {
                outString.append(res.getString(R.string.sim_slot_identifier,
                        String.valueOf(self.getDisplaySlotId())));
            } else {
