...
/**
 * Background worker service is an initial example of a background work queue
 * handler
 * Used to actually "send" messages which may take some time and should not
 * block ActionService
 * or UI
 */
    ...
    private void doBackgroundWork(final Action action, final int attempt) {
        ...
        try {
            ...
            if (!(exception instanceof DataModelException)) {
                // DataModelException is expected (sort-of) and handled in
                // handleFailureFromWorker
