...
/**
 * A class that encapsulates all of the data for a specific participant in a
 * conversation.
 */
public class ParticipantData implements Parcelable {

    private static final ArrayMap<Integer, String> sSubIdtoParticipantIdCache = new ArrayMap<Integer, String>();

    // We always use -1 as default/invalid sub id although system may give us
    // anything negative
    ...
    // Active slot ids are non-negative. Using -1 to designate to inactive self
    // participants.
    ...
    public static ParticipantData getFromRecipientEntry(final RecipientEntry recipientEntry) {
        ...
        pd.mIsEmailAddress = MmsSmsUtils.isEmailAddress(pd.mSendDestination);
        pd.mNormalizedDestination = pd.mIsEmailAddress ? pd.mSendDestination
                : PhoneUtils.getDefault().getCanonicalBySystemLocale(pd.mSendDestination);
        pd.mDisplayDestination = pd.mIsEmailAddress ? pd.mNormalizedDestination
        ...
        if (pd.mContactId < 0) {
            // ParticipantData only supports real contact ids (>=0) based on faith that the
            // contacts
            // provider will continue to only use non-negative ids. The UI uses contactId <
            // 0 for
    ...
    /**
     * Get an instance from a raw phone number and using system locale to normalize
     * it.
     *
     * Use this when creating a participant that is for displaying UI and not
     * associated
     * with a specific SIM. For example, when creating a conversation using user
     * entered
     * phone number.
     *
    ...
    /**
     * Get an instance from a raw phone number and using SIM or system locale to
     * normalize it.
     *
     * Use this when creating a participant that is associated with a specific SIM.
     * For example,
     * the sender of a received message or the recipient of a sending message that
     * is already
    ...
    public static String getParticipantId(final DatabaseWrapper db, final int subId) {
        ...
        try (final Cursor cursor = db.query(DatabaseHelper.PARTICIPANTS_TABLE,
    ...
    private void maybeSetupUnknownSender() {
        if (isUnknownSender()) {
            // Because your locale may change, we setup the display string for the unknown
            // sender
    ...
    public boolean updatePhoneNumberForSelfIfChanged() {
        final String phoneNumber = PhoneUtils.get(mSubId).getCanonicalForSelf(true/* allowOverride */);
        ...
        if (isSelf() && !TextUtils.equals(phoneNumber, mNormalizedDestination)) {
            ...
            mDisplayDestination = mIsEmailAddress ? phoneNumber : PhoneUtils.getDefault().formatForDisplay(phoneNumber);
            changed = true;
        }
        return changed;
    }
    ...
    /**
     * @return whether this sub is active. Note that
     *         {@link ParticipantData#DEFAULT_SELF_SUB_ID} is
    ...
    public static final Parcelable.Creator<ParticipantData> CREATOR = new Parcelable.Creator<ParticipantData>() {
        @Override
