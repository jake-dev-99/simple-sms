...
/**
 * Action used to lookup any messages in the pending send/download state and
 * either fail them or
 * retry their action based on subscriptions. This action only initiates one
 * retry at a time for
 * both sending/downloading. Further retries should be triggered by successful
 * sending/downloading
...
public class ProcessPendingMessagesAction extends Action implements Parcelable {
    ...
    // PENDING_INTENT_BASE_REQUEST_CODE + subId(-1 for pre-L_MR1) is used per
    // subscription uniquely.
    ...
    public static void scheduleProcessPendingMessagesAction(final boolean failed,
        ...
        // Can safely clear any pending alarms or connectivity events as either an
        // action
        // is currently running or we will run now or register if pending actions
        // possible.
        unregister(subId);

        final boolean isDefaultSmsApp = PhoneUtils.getDefault().isDefaultSmsApp();
        boolean scheduleAlarm = false;
        // If message succeeded and if Bugle is default SMS app just carry on with next
        // message
        if (!failed && isDefaultSmsApp) {
            // Clear retry attempt count as something just succeeded
            setRetry(0, subId);

            // Lookup and queue next message for each sending/downloading for immediate
            // processing
            // by background worker. If there are no pending messages, this will do nothing
            // and
            // return true.
            final ProcessPendingMessagesAction action = new ProcessPendingMessagesAction();
        ...
        if (getHavePendingMessages(subId) || scheduleAlarm) {
            ...
            // No more pending messages (presumably the message that failed has expired) or
            // it
    ...
    private static int getNextRetry(int subId) {
        final BuglePrefs prefs = Factory.get().getSubscriptionPrefs(subId);
        final int retryAttempt = prefs.getInt(BuglePrefsKeys.PROCESS_PENDING_MESSAGES_RETRY_COUNT, 0) + 1;
        prefs.putInt(BuglePrefsKeys.PROCESS_PENDING_MESSAGES_RETRY_COUNT, retryAttempt);
        return retryAttempt;
    }
    ...
    private static boolean getHavePendingMessages(final int subId) {
        ...
        // Messages may be in the process of sending/downloading even when there are no
        // pending
        // messages...
        return false;
    }
    ...
    private boolean queueActions(final Action processingAction) {
        ...
        // This keeps outgoing messages "in order" but allow downloads to happen even if
        // sending
        // gets blocked until messages time out. Manual resend bumps messages to head of
        // queue.
        final String toSendMessageId = findNextMessageToSend(db, now, selfId);
        final String toDownloadMessageId = findNextMessageToDownload(db, now, selfId);
        if (toSendMessageId != null) {
    ...
    private static String findNextMessageToSend(final DatabaseWrapper db, final long now,
        ...
        try {
            ...
            if (OsUtil.isAtLeastL_MR1()) {
                final ParticipantData messageSelf = BugleDatabaseOperations.getExistingParticipant(db, selfId);
            ...
            while (cursor.moveToNext()) {
                ...
                // Mark this message as failed if the message's self is inactive or the message
                // is
    ...
    public static final Parcelable.Creator<ProcessPendingMessagesAction> CREATOR = new Parcelable.Creator<ProcessPendingMessagesAction>() {
        @Override
