...
/**
 * Shows a list of galley mediae from external storage in a GridView with
...
public class GalleryGridView extends MediaPickerGridView implements
    ...
    /**
     * Implemented by the owner of this GalleryGridView instance to communicate on
     * media picking and
    ...
    private boolean canToggleMultiSelect() {
        // We allow the user to toggle multi-select mode only when nothing has selected.
        // If
    ...
    public void onDraftChanged(final DraftMessageData data, final int changeFlags) {
        ...
        // Whenever attachment changed, refresh selection state to remove those that are
        // not
        // selected.
    ...
    public void onDraftAttachmentLimitReached(final DraftMessageData data) {
        mDraftMessageDataModel.ensureBound(data);
        // Whenever draft attachment limit is reach, refresh selection state to remove
        // those
    ...
    private void refreshImageSelectionStateOnAttachmentChange() {
        ...
        final Iterator<Map.Entry<Uri, MessagePartData>> iterator = mSelectedImages.entrySet().iterator();
