...
/**
 * An extension on the base {@link BaseRecipientAdapter} that uses data layer
 * from Bugle,
 * such as the ContactRecipientPhotoManager that uses our own
 * MediaResourceManager, and
 * contact lookup that relies on ContactUtil. It provides data source and
 * filtering ability
...
public final class ContactRecipientAdapter extends BaseRecipientAdapter {
    ...
    /**
     * A Filter for a RecipientEditTextView that queries Bugle's ContactUtil for
     * auto-complete
     * results.
     */
    public class ContactFilter extends Filter {

        // Used to sort filtered contacts when it has combined results from email and
        // phone.
        private final RecipientEntryComparator mComparator = new RecipientEntryComparator();

        /**
         * Returns a cursor containing the filtered results in contacts given the search
         * text,
         * and a boolean indicating whether the results are sorted.
         *
         * The queries are synchronously performed since this is not run on the main
         * thread.
         *
         * Some locales (e.g. JPN) expect email addresses to be auto-completed for MMS.
         * If this is the case, perform two queries on phone number followed by email
         * and
        ...
        private CursorResult getFilteredResultsCursor(final String searchText) {
            ...
            if (BugleGservices.get().getBoolean(
                ...
                final CursorResult cursorResult = new CursorResult(personalCursor, false /* sorted */);
        ...
        protected void publishResults(final CharSequence constraint, final FilterResults results) {
            ...
            final ContactReceipientFilterResult contactReceipientFilterResult = (ContactReceipientFilterResult) results.values;
        ...
        private class RecipientEntryComparator implements Comparator<RecipientEntry> {
            ...
            /**
             * Compare two RecipientEntry's, first by locale-aware display name comparison,
             * then by
            ...
            public int compare(RecipientEntry lhs, RecipientEntry rhs) {
                ...
                final int contactCompare = lhsContactId < rhsContactId ? -1 : (lhsContactId == rhsContactId ? 0 : 1);
                if (contactCompare != 0) {
    ...
    /**
     * Called when we need to substitute temporary recipient chips with better
     * alternatives.
     * For example, if a list of comma-delimited phone numbers are pasted into the
     * edit box,
     * we want to be able to look up in the ContactUtil for exact matches and get
     * contact
     * details such as name and photo thumbnail for the contact to display a better
     * chip.
     */
    @Override
    ...
    /**
     * We handle directory header here and then delegate the work of creating
     * recipient views to
     * the {@link BaseRecipientAdapter}. Please notice that we need to fix the
     * position
     * before passing to {@link BaseRecipientAdapter} because it is not aware of the
     * existence of
     * directory headers.
     */
    @Override
