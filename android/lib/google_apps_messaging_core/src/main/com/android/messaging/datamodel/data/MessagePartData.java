...
/**
 * Represents a single message part. Messages consist of one or more parts which
 * may contain
...
public class MessagePartData implements Parcelable {
    ...
    public static final String[] ACCEPTABLE_GALLERY_MEDIA_TYPES = new String[] {
    ...
    // SQL statement to insert a "complete" message part row (columns based on
    // projection above).
    private static final String INSERT_MESSAGE_PART_SQL = "INSERT INTO " + DatabaseHelper.PARTS_TABLE + " ( "
    ...
    /**
     * Updates the part id.
     * Can be used to reset the partId just prior to persisting (which will assign a
     * new partId)
     ...
     * new messageId)
     * or can be called on a part that does not yet have a valid messageId to set
     * it.
     */
    ...
    public static final Parcelable.Creator<MessagePartData> CREATOR = new Parcelable.Creator<MessagePartData>() {
        @Override
    ...
    /**
     * If application owns content associated with this part delete it (on
     * background thread)
     */
    ...
    /**
     * If this is an image part, decode the image header and potentially save the
     * size to the db.
     */
    ...
    /**
     * Computes the minimum size that this MessagePartData could be
     * compressed/downsampled/encoded
     * before sending to meet the maximum message size imposed by the carriers. This
     * is used to
     * determine right before sending a message whether a message could possibly be
     * sent. If not
    ...
    public long getMinimumSizeInBytesForSending() {
        ...
        if (!isAttachment()) {
            // No limit is imposed on non-attachment part (i.e. plain text), so treat it as
            // zero.
            ...
            // This is some unknown media type that we don't know how to handle. Log an
            // error
