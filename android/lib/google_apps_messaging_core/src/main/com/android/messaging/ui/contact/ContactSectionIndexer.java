...
/**
 * Indexes contact alphabetical sections so we can report to the fast scrolling
 * list view
 * where we are in the list when the user scrolls through the contact list,
 * allowing us to show
 * alphabetical indicators for the fast scroller as well as list section
 * headers.
 */
public class ContactSectionIndexer implements SectionIndexer {
    ...
    public int getSectionForPosition(final int position) {
        ...
        // Perform a binary search on the starting positions of the sections to the find
        // the
        // section for the position.
        int left = 0;
        int right = mSectionStartingPositions.size() - 1;

        // According to getSectionForPosition()'s doc, we should always clamp the value
        // when the
    ...
    private void buildIndexerFromDisplayNames(final Cursor cursor) {
        // Loop through the contact cursor and get the starting position for each first
        // character.
        // The result is stored into two arrays, one for the section header (i.e. the
        // first
        // character), and one for the starting position, which is guaranteed to be
        // sorted in
        // ascending order.
        ...
        if (cursor != null) {
            ...
            while (cursor.moveToNext()) {
                // The sort key is typically the contact's display name, so for example, a
                // contact
                // named "Bob" will go into section "B". The Contacts provider generally uses a
                // a slightly more sophisticated heuristic, but as a fallback this is good
                // enough.
    ...
    private void buildIndexer(final Cursor cursor) {
        // First check if we get indexer label extras from the contact provider; if not,
        // fall back
