...
public class DeleteConversationAction extends Action implements Parcelable {
    ...
    // Delete conversation from both the local DB and telephony in the background so
    // sync cannot
    // run concurrently and incorrectly try to recreate the conversation's messages
    // locally. The
    // telephony database can sometimes be quite slow to delete conversations, so we
    // delete from
    ...
    protected Bundle doBackgroundWork() throws DataModelException {
        ...
        if (!TextUtils.isEmpty(conversationId)) {
            ...
            if (BugleDatabaseOperations.deleteConversation(db, conversationId, cutoffTimestamp)) {
                ...
                // Notify the widget the conversation is deleted so it can go into its configure
                // state.
            ...
            // Now delete from telephony DB. MmsSmsProvider throws an exception if the
            // thread id is
            // less than 0. If it's greater than zero, it will delete all messages with that
            // thread
     ...
     * <p>
     * This is a fallback used when the conversation is not associated with any
     * telephony thread,
     * or its thread id is invalid (e.g. negative). This is not common, but can
     * happen sometimes
     * (e.g. the Unknown Sender conversation). In the usual case of deleting a
     * conversation, we
     * don't need this because the telephony provider automatically deletes messages
     * when a thread
     * is deleted.
     */
    ...
    public static final Parcelable.Creator<DeleteConversationAction> CREATOR = new Parcelable.Creator<DeleteConversationAction>() {
        @Override
