...
public class ComposeMessageView extends LinearLayout
    ...
    // Centrally manages all the mutual exclusive UI components accepting user
    // input, i.e.
    ...
    protected void onFinishInflate() {
        ...
        // onFinishInflate() is called before self is loaded from db. We set the default
        // text
        // limit here, and apply the real limit later in
        // updateOnSelfSubscriptionChange().
        ...
        // We need the listener to change the avatar to the send button when the user
        // starts
        ...
        mAttachMediaButton = (ImageButton) findViewById(R.id.attach_media_button);
    ...
    // returns true if it actually shows the subject editor and false if already
    // showing
    ...
    public void onDraftChanged(final DraftMessageData data, final int changeFlags) {
        // As this is called asynchronously when message read check bound before
        // updating text
        mBinding.ensureBound(data);
        ...
        // which immediately reloads the text from the subject and message fields and
        // replaces
        ...
        if ((changeFlags & DraftMessageData.MESSAGE_SUBJECT_CHANGED) == DraftMessageData.MESSAGE_SUBJECT_CHANGED) {
        ...
        if ((changeFlags & DraftMessageData.MESSAGE_TEXT_CHANGED) == DraftMessageData.MESSAGE_TEXT_CHANGED) {
        ...
        if ((changeFlags & DraftMessageData.ATTACHMENTS_CHANGED) == DraftMessageData.ATTACHMENTS_CHANGED) {
    ...
    private void updateVisualsOnDraftChanged(boolean hasAttachmentsChanged) {
        ...
        final List<MessagePartData> attachments = new ArrayList<MessagePartData>(
        ...
        if (draftMessageData.getIsMms()) { // MMS case
            ...
            // Show the counter only if we are going to send more than one message OR we are
            // getting
            // close.
            if (messageCount > 1
                    || codePointsRemaining <= CODEPOINTS_REMAINING_BEFORE_COUNTER_SHOWN) {
                // Update the remaining characters and number of messages required.
                final String counterText = messageCount > 1
        ...
        // Update the send message button. Self icon uri might be null if self
        // participant data
        ...
        if (attachmentCount == 0) {
            final SubscriptionListEntry subscriptionListEntry = mConversationDataModel.getData()
    ...
    public void onTextChanged(final CharSequence s, final int start, final int before,
        ...
        final BugleActionBarActivity activity = (mOriginalContext instanceof BugleActionBarActivity)
                ? (BugleActionBarActivity) mOriginalContext
                : null;
    ...
    /**
     * The selfId of the conversation. As soon as the DraftMessageData successfully
     * loads (i.e.
     * getSelfId() is non-null), the selfId in DraftMessageData is treated as the
     * sole source
     * of truth for conversation self id since it reflects any pending self id
     * change the user
     * makes in the UI.
     */
    ...
    public void selectSim(SubscriptionListEntry subscriptionData) {
        ...
        // Don't attempt to change self if self hasn't been loaded, or if self hasn't
        // changed.
