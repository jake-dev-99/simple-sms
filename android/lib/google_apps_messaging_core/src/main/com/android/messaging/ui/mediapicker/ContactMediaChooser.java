...
/**
 * Chooser which allows the user to select an existing contact from contacts
 * apps on this device.
 * Note that this chooser requires the Manifest.permission.READ_CONTACTS which
 * is one of the miminum
 * set of permissions for this app. Thus no case to request READ_CONTACTS
 * permission on it actually.
 */
            ...
            if (contactUri != null) {
                ...
                if (vCardUri != null) {
                    SafeAsyncTask.executeOnThreadPool(new Runnable() {
                        @Override
                        public void run() {
                            final PendingAttachmentData pendingItem = PendingAttachmentData.createPendingAttachmentData(
