...
/**
 * Displays a grid of attachment previews for the user to choose which to
 * select/unselect
 */
public class AttachmentGridView extends GridView implements
    ...
    // By default everything is selected so only need to keep track of the
    // unselected set.
    ...
    public void onItemClicked(final AttachmentGridItemView view, final MessagePartData attachment) {
        // If the item is an image, show the photo viewer. All the other types (video,
        // audio,
        // vcard) have internal click handling for showing previews so we don't need to
        // handle them
    ...
    /**
     * Persists the item selection state to saved instance state so we can restore
     * on activity
     * recreation
     */
