 ...
 * NOTE: This action must queue a ProcessPendingMessagesAction when it is done
 * (success or failure).
 */
public class ProcessDownloadedMmsAction extends Action {
    ...
    // This is called for fast failing downloading (due to airplane mode or mobile
    // data )
    ...
    protected Bundle doBackgroundWork() throws DataModelException {
        ...
        final boolean sendDeferredRespStatus = actionParameters.getBoolean(KEY_SEND_DEFERRED_RESP_STATUS, false);
        ...
        if (downloadedByPlatform) {
                ...
                if (downloadedData != null) {
                    ...
                    if (retrieveConf != null) {
                        ...
                        final boolean autoDownload = actionParameters.getBoolean(
                                KEY_AUTO_DOWNLOAD);
                        final long receivedTimestampInSeconds = actionParameters.getLong(KEY_RECEIVED_TIMESTAMP);
                        ...
                        final MmsUtils.StatusPlusUri result = MmsUtils.insertDownloadedMessageAndSendResponse(context,
            ...
            // Message was already processed by the internal API, or the download action
            // failed.
    ...
    protected Object processBackgroundResponse(final Bundle response) {
        if (response == null) {
            // No message download to process; doBackgroundWork sent a notify deferred
            // response
        ...
        final int subId = actionParameters.getInt(KEY_SUB_ID, ParticipantData.DEFAULT_SELF_SUB_ID);
        // If we were trying to auto-download but have failed need to send the deferred
        // response
        final boolean needToSendDeferredResp = autoDownload && (status == MmsUtils.MMS_REQUEST_MANUAL_RETRY);
        ...
        // Scheduling pending messages. If auto downloading is failed and it needs to
        // send the
        // deferred response, Skip it here and it will be scheduled after sending the
        // response.
        if (!needToSendDeferredResp) {
        ...
        try {
            if (mms != null) {
                ...
                final ParticipantData sender = ParticipantData.getFromRawPhoneBySimLocale(from,
                        subId);
                final String senderParticipantId = BugleDatabaseOperations.getOrCreateParticipantInTransaction(db,
                        sender);
                ...
                messageInFocusedConversation = DataModel.get().isFocusedConversation(conversationId);
                messageInObservableConversation = DataModel.get().isNewMessageObservable(conversationId);
                ...
                messageInFocusedConversation = DataModel.get().isFocusedConversation(notificationConversationId);
    ...
    public static final Parcelable.Creator<ProcessDownloadedMmsAction> CREATOR = new Parcelable.Creator<ProcessDownloadedMmsAction>() {
        @Override
