...
public class SyncMessagesAction extends Action implements Parcelable {
    ...
    /**
     * Start a full sync (backed off a few seconds to avoid pulling
    ...
    /**
     * Start an incremental sync to pull messages since last sync (backed off a few
     * seconds)..
     */
    ...
    /**
     * Start an incremental sync when the application starts up (no back off as not
     * yet
     * sending/receiving).
     */
    ...
    protected Bundle doBackgroundWork() {
        ...
        // Clear last change time so that we can work out if this batch is dirty when it
        // completes
        final SyncManager syncManager = DataModel.get().getSyncManager();

        // Clear the singleton cache that maps threads to recipients and to
        // conversations.
        ...
        final ArrayList<LocalDatabaseMessage> messagesToDelete = new ArrayList<LocalDatabaseMessage>();
     ...
     * @param cursors             cursor pair holding references to local and remote
     *                            messages
     ...
     * @param cache               cache for conversation id / thread id / recipient
     *                            set mapping
    ...
    private long syncCursorPair(final DatabaseWrapper db, final SyncCursorPair cursors,
        ...
        int localTotal = 0;
        int remoteTotal = 0;
        // Scan through the messages on both sides and prepare messages for local
        // message table
        ...
        try {
            ...
            // We want to catch anything unexpected since this is running in a separate
            // thread
    ...
    protected Object processBackgroundResponse(final Bundle response) {
        ...
        // Check with the sync manager if any conflicting updates have been made to
        // databases
        final SyncManager syncManager = DataModel.get().getSyncManager();
        ...
        if (orphan) {
            ...
            if (lastTimestampMillis == SYNC_FAILED) {
                ...
                // Redo this batch
                final SyncMessagesAction nextBatch = new SyncMessagesAction(lowerBoundTimeMillis, upperBoundTimeMillis,
    ...
    public static final Parcelable.Creator<SyncMessagesAction> CREATOR = new Parcelable.Creator<SyncMessagesAction>() {
        @Override
