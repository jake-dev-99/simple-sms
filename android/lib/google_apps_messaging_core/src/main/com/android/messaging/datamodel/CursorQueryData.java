...
/**
 * Holds parameters and data (such as content URI) for performing queries on the
 * content provider.
 * This class could then be used to perform a query using either a
 * BoundCursorLoader or querying
 * on the content resolver directly.
 *
 * This class is used for cases where the way to load a cursor is not fixed. For
 * example,
 * when using ContactUtil to query for phone numbers, the ContactPickerFragment
 * wants to use
 * a CursorLoader to asynchronously load the data and tie in nicely with its
 * data binding
 * paradigm, whereas ContactRecipientAdapter wants to synchronously perform the
 * query on the
 * worker thread.
 */
public class CursorQueryData {
    ...
    /**
     * Representation of an invalid query. {@link #performSynchronousQuery} will
     * return
     * a null Cursor.
     */
