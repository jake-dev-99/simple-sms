...
public class SnackBarManager {
    ...
    /**
     * The given snackBar is not guaranteed to be shown. If the previous snackBar is
     * animating away,
     * and another snackBar is requested to show after this one, this snackBar will
     * be skipped.
     */
    public void show(final SnackBar snackBar) {
        ...
        if (mCurrentSnackBar != null) {
            ...
            // Dismiss the current snack bar. That will cause the next snack bar to be shown
            // on
            // completion.
        ...
        if (placement == null) {
            ...
            // You'd expect PopupWindow.showAsDropDown to ensure the popup moves with the
            // anchor
            // view, which it does for scrolling, but not layout changes, so we have to
            // manually
    ...
    /**
     * Dismisses the current toast that is showing. If there is a toast waiting to
     * be shown, that
    ...
    private void animateInteractionsOnDismiss(final SnackBar snackBar) {
        ...
        for (final SnackBarInteraction interaction : interactions) {
            if (interaction != null) {
                final ViewPropertyAnimator animator = interaction.animateOnSnackBarDismiss(snackBar);
                if (animator != null) {
                    normalizeAnimator(animator);
                }
            }
    ...
    /**
     * Get the offset from the bottom of the screen where the snack bar should be
     * placed.
     */
    private int getScreenBottomOffset(final SnackBar snackBar) {
        ...
        if (OsUtil.isAtLeastL()) {
            // In L, the navigation bar is included in the space for the popup window, so we
            // have to
