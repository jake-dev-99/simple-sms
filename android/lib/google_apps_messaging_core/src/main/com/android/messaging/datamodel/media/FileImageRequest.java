...
/**
 * Serves file system based image requests. Since file paths can be expressed in
 * Uri form, this
 * extends regular UriImageRequest but performs additional optimizations such as
 * loading thumbnails
...
public class FileImageRequest extends UriImageRequest {
    ...
    protected Bitmap loadBitmapInternal()
            throws IOException {
        // Before using the FileInputStream, check if the Exif has a thumbnail that we
        // can use.
        if (mCanUseThumbnail) {
            byte[] thumbnail = null;
            try {
        ...
        // Fall back to default InputStream-based loading if no thumbnails could be
        // retrieved.
