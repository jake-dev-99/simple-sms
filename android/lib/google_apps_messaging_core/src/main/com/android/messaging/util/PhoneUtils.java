 ...
 * a bit of SmsManager.
 * <p>
 * The class instance can only be obtained via the get(int subId) method
 * parameterized
 * by a SIM subscription ID. On pre-L_MR1, the subId is not used and it has to
 * be
 * the default subId (-1).
 * <p>
 * A convenient getDefault() method is provided for default subId (-1) on any
 * platform
 */
public abstract class PhoneUtils {
    ...
    // the country's cache. Each cache maps from original phone number to
    // canonicalized phone
     ...
     *
     * @param allowOverride Whether to use the app's setting to override the self
     *                      number
    ...
    /**
     * Returns the "effective" subId, or the subId used in the context of actual
     * messages,
     * conversations and subscription-specific settings, for the given "nominal" sub
     * id.
     * <p>
     * For pre-L-MR1 platform, this should always be
     * {com.android.messaging.datamodel.data.ParticipantData#DEFAULT_SELF_SUB_ID};
     * <p>
     * On the other hand, for L-MR1 and above, DEFAULT_SELF_SUB_ID will be mapped to
     * the system
    ...
    public static class PhoneUtilsPreLMR1 extends PhoneUtils {
        ...
        public boolean isMobileDataEnabled() {
            ...
            try {
                final Class<? extends ConnectivityManager> cmClass = mConnectivityManager.getClass();
    ...
    public static class PhoneUtilsLMR1 extends PhoneUtils implements LMr1 {
        ...
        public SubscriptionInfo getActiveSubscriptionInfo() {
            try {
                final SubscriptionInfo subInfo = mSubscriptionManager.getActiveSubscriptionInfo(mSubId);
                if (subInfo == null) {
        ...
        public List<SubscriptionInfo> getActiveSubscriptionInfoList() {
            final List<SubscriptionInfo> subscriptionInfos = mSubscriptionManager.getActiveSubscriptionInfoList();
    ...
    /**
     * Get an instance of PhoneUtils associated with a specific SIM, which is also
     * platform
     * specific.
     *
    ...
    /**
     * Canonicalize phone number using SIM's country, may fall back to system locale
     * country
    ...
    /**
     * Get the SIM's phone number in NATIONAL format with only digits, used in
     * sending
     ...
     * If the country code matches between the system locale and the input phone
     * number,
     * it will be formatted into NATIONAL format, otherwise, the INTERNATIONAL
     * format
     *
    ...
    public String formatForDisplay(final String phoneText) {
        ...
        try {
            ...
            final PhoneNumberFormat phoneNumberFormat = (systemCountryCode > 0
                    && parsedNumber.getCountryCode() == systemCountryCode) ? PhoneNumberFormat.NATIONAL
     ...
     * - On JB (and below) this always returns true, since the setting was added in
     * KLP.
     */
     ...
     *
     * @param runnable a {@link SubscriptionRunnable} for performing work on each
     *                 subscription.
     */
    ...
    private static String getNumberFromPrefs(final Context context, final int subId) {
        ...
        final String mmsPhoneNumberPrefKey = context.getString(R.string.mms_phone_number_pref_key);
