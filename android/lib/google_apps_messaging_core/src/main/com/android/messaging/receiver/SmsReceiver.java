...
/**
 * Class that receives incoming SMS messages through
 * android.provider.Telephony.SMS_RECEIVED
 *
 * This class serves two purposes:
 * - Process phone verification SMS messages
 * - Handle SMS messages when the user has enabled us to be the default SMS app
 * (Pre-KLP)
 */
public final class SmsReceiver extends BroadcastReceiver {
    ...
    /**
     * Enable or disable the SmsReceiver as appropriate. Pre-KLP we use this
     * receiver for
     * receiving incoming SMS messages. For KLP+ this receiver is not used when
     * running as the
     * primary user and the SmsDeliverReceiver is used for receiving incoming SMS
     * messages.
     * When running as a secondary user, this receiver is still used to trigger the
     * incoming
     * notification.
     */
    public static void updateSmsReceiveHandler(final Context context) {
        ...
        if (OsUtil.isAtLeastKLP()) {
            // When we're running as the secondary user, we don't get the new SMS_DELIVER
            // intent,
            // only the primary user receives that. As secondary, we need to go old-school
            // and
            // listen for the SMS_RECEIVED intent. For the secondary user, use this
            // SmsReceiver
            // for both sms and mms notification. For the primary user on KLP (and above),
            // we don't
            // use the SmsReceiver.
    ...
    public static void deliverSmsMessages(final Context context, final int subId,
        ...
        final ContentValues messageValues = MmsUtils.parseReceivedSmsMessage(context, messages, errorCode);
     ...
     * 
     * @return the messages. If there is an error or the message should be ignored,
     *         return null.
     */
    public static android.telephony.SmsMessage[] getMessagesFromIntent(Intent intent) {
        ...
        // the methods on it although the SmsMessage itself is not null. So do this
        // check
