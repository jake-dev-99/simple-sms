...
/**
 * Wraps around the functionalities of MediaRecorder, performs routine setup for
 * audio recording
 * and updates the audio level to be displayed in UI.
 *
 * During the start and end of a recording session, we kick off a thread that
 * polls for audio
 * levels, and updates the thread-safe AudioLevelSource instance. Consumers may
 * bind to the
 * sound level by either polling from the level source, or register for a level
 * change callback
 * on the level source object. In Bugle, the UI element (SoundLevels) polls for
 * the sound level
 * on the UI thread by using animation ticks and invalidating itself.
 *
 * Aside from tracking sound levels, this also encapsulates the functionality to
 * save the file
...
public class LevelTrackingMediaRecorder {
    ...
    // The native amplitude returned from MediaRecorder ranges from 0~32768
    // (unfortunately, this
    // is not a constant that's defined anywhere, but the framework's Recorder app
    // is using the
    // same hard-coded number). Therefore, a constant is needed in order to make it
    // 0~100.
    private static final int MAX_AMPLITUDE_FACTOR = 32768 / 100;

    // We want to limit the max audio file size by the max message size allowed by
    // MmsConfig,
     ...
     * 
     * @return true if a session is successfully started; false if something went
     *         wrong or if
