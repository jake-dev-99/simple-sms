...
/**
 * First-party Kotlin port of the vendored `PduContentTypes`; values preserved
 * 1:1. The WAP well-known content-type table is indexed by the on-wire
 * assigned number (0x00..0x52), so order is load-bearing — `PduParser` /
 * `PduComposer` index into [contentTypes] directly.
 *
 * Exposed as a `@JvmField` static array so the (still-Java) `pdu_alt` codec
 * callers keep reading `PduContentTypes.contentTypes[i]` / `.length`
 * unchanged. Visibility widens from the vendored package-private to public
 * purely for Kotlin↔Java interop while the rest of the package is mid-port;
 * it can tighten once `pdu_alt` is fully Kotlin.
 */
