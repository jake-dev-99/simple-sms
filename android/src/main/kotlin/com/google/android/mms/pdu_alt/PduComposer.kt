/*
 * Copyright (C) 2015 Jacob Klinker
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.google.android.mms.pdu_alt

import android.content.ContentResolver
import android.content.Context
import android.text.TextUtils
import android.util.Log
import java.io.ByteArrayOutputStream
import java.io.FileNotFoundException
import java.io.IOException
import java.io.InputStream

/**
 * First-party Kotlin port of the vendored `PduComposer` (Phase 5 · pdu_alt) — the
 * outbound WSP/MMS byte encoder. Behaviour-faithful 1:1: the same
 * `BufferStack`/`PositionMarker` length-backpatching mechanism and the same
 * encode order for each PDU type, so the wire bytes are unchanged (pinned
 * byte-for-byte by `PduComposeGoldenTest`).
 *
 * The vendored `protected` machinery (`mMessage`/`mPosition`/the `append*`
 * helpers) is made `private` — `PduComposer` is not subclassed and nothing
 * accesses those members externally; the inner `BufferStack`/`PositionMarker`
 * reach them as inner classes. `mPduHeader` is dereferenced from
 * `pdu.getPduHeaders()!!` (a `GenericPdu` always carries headers).
 *
 * `EncodedStringValue.getTextString()` is non-null in the Kotlin port, so the
 * vendored `if (null == textString)` guard in `appendEncodedString` is
 * unreachable and dropped.
 */
class PduComposer(context: Context, pdu: GenericPdu) {

    /** The output message. */
    private var mMessage: ByteArrayOutputStream = ByteArrayOutputStream()

    /** The PDU. */
    private val mPdu: GenericPdu = pdu

    /** Current visiting position of the mMessage. */
    private var mPosition: Int = 0

    /** Message compose buffer stack. */
    private val mStack: BufferStack = BufferStack()

    /** Content resolver. */
    private val mResolver: ContentResolver = context.contentResolver

    /** Header of this pdu. */
    private val mPduHeader: PduHeaders = pdu.getPduHeaders()!!

    /**
     * Make the message. No need to check whether mandatory fields are set,
     * because the constructors of outgoing pdus are taking care of this.
     *
     * @return OutputStream of maked message. Return null if
     *         the PDU is invalid.
     */
    fun make(): ByteArray? {
        // Get Message-type.
        val type = mPdu.messageType

        /* make the message */
        when (type) {
            PduHeaders.MESSAGE_TYPE_SEND_REQ ->
                if (makeSendReqPdu() != PDU_COMPOSE_SUCCESS) {
                    return null
                }
            PduHeaders.MESSAGE_TYPE_NOTIFYRESP_IND ->
                if (makeNotifyResp() != PDU_COMPOSE_SUCCESS) {
                    return null
                }
            PduHeaders.MESSAGE_TYPE_ACKNOWLEDGE_IND ->
                if (makeAckInd() != PDU_COMPOSE_SUCCESS) {
                    return null
                }
            PduHeaders.MESSAGE_TYPE_READ_REC_IND ->
                if (makeReadRecInd() != PDU_COMPOSE_SUCCESS) {
                    return null
                }
            else -> return null
        }

        return mMessage.toByteArray()
    }

    /**
     * Copy buf to mMessage.
     */
    private fun arraycopy(buf: ByteArray, pos: Int, length: Int) {
        mMessage.write(buf, pos, length)
        mPosition += length
    }

    /**
     * Append a byte to mMessage.
     */
    private fun append(value: Int) {
        mMessage.write(value)
        mPosition++
    }

    /**
     * Append short integer value to mMessage.
     */
    private fun appendShortInteger(value: Int) {
        /*
         * From WAP-230-WSP-20010705-a:
         * Short-integer = OCTET
         * ; Integers in range 0-127 shall be encoded as a one octet value
         * ; with the most significant bit set to one (1xxx xxxx) and with
         * ; the value in the remaining least significant bits.
         * In our implementation, only low 7 bits are stored and otherwise
         * bits are ignored.
         */
        append((value or 0x80) and 0xff)
    }

    /**
     * Append an octet number between 128 and 255 into mMessage.
     */
    private fun appendOctet(number: Int) {
        append(number)
    }

    /**
     * Append a short length into mMessage.
     */
    private fun appendShortLength(value: Int) {
        /*
         * From WAP-230-WSP-20010705-a:
         * Short-length = <Any octet 0-30>
         */
        append(value)
    }

    /**
     * Append long integer into mMessage. it's used for really long integers.
     */
    private fun appendLongInteger(longInt: Long) {
        /*
         * From WAP-230-WSP-20010705-a:
         * Long-integer = Short-length Multi-octet-integer
         * ; The Short-length indicates the length of the Multi-octet-integer
         * Multi-octet-integer = 1*30 OCTET
         * ; The content octets shall be an unsigned integer value with the
         * ; most significant octet encoded first (big-endian representation).
         * ; The minimum number of octets must be used to encode the value.
         */
        var size: Int
        var temp = longInt

        // Count the length of the long integer.
        size = 0
        while ((temp != 0L) && (size < LONG_INTEGER_LENGTH_MAX)) {
            temp = temp ushr 8
            size++
        }

        // Set Length.
        appendShortLength(size)

        // Count and set the long integer.
        var shift = (size - 1) * 8

        for (i in 0 until size) {
            append(((longInt ushr shift) and 0xff).toInt())
            shift -= 8
        }
    }

    /**
     * Append text string into mMessage.
     */
    private fun appendTextString(text: ByteArray) {
        /*
         * From WAP-230-WSP-20010705-a:
         * Text-string = [Quote] *TEXT End-of-string
         * ; If the first character in the TEXT is in the range of 128-255,
         * ; a Quote character must precede it. Otherwise the Quote character
         * ;must be omitted. The Quote is not part of the contents.
         */
        if ((((text[0]).toInt()) and 0xff) > TEXT_MAX) { // No need to check for <= 255
            append(TEXT_MAX)
        }

        arraycopy(text, 0, text.size)
        append(0)
    }

    /**
     * Append text string into mMessage.
     */
    private fun appendTextString(str: String) {
        /*
         * From WAP-230-WSP-20010705-a:
         * Text-string = [Quote] *TEXT End-of-string
         * ; If the first character in the TEXT is in the range of 128-255,
         * ; a Quote character must precede it. Otherwise the Quote character
         * ;must be omitted. The Quote is not part of the contents.
         */
        appendTextString(str.toByteArray())
    }

    /**
     * Append encoded string value to mMessage.
     */
    private fun appendEncodedString(enStr: EncodedStringValue) {
        /*
         * From OMA-TS-MMS-ENC-V1_3-20050927-C:
         * Encoded-string-value = Text-string | Value-length Char-set Text-string
         */
        val charset = enStr.characterSet
        val textString = enStr.getTextString()

        /*
         * In the implementation of EncodedStringValue, the charset field will
         * never be 0. It will always be composed as
         * Encoded-string-value = Value-length Char-set Text-string
         */
        mStack.newbuf()
        val start = mStack.mark()

        appendShortInteger(charset)
        appendTextString(textString)

        val len = start.getLength()
        mStack.pop()
        appendValueLength(len.toLong())
        mStack.copy()
    }

    /**
     * Append uintvar integer into mMessage.
     */
    private fun appendUintvarInteger(value: Long) {
        /*
         * From WAP-230-WSP-20010705-a:
         * To encode a large unsigned integer, split it into 7-bit fragments
         * and place them in the payloads of multiple octets. The most significant
         * bits are placed in the first octets with the least significant bits
         * ending up in the last octet. All octets MUST set the Continue bit to 1
         * except the last octet, which MUST set the Continue bit to 0.
         */
        var i: Int
        var max = SHORT_INTEGER_MAX.toLong()

        i = 0
        while (i < 5) {
            if (value < max) {
                break
            }

            max = (max shl 7) or 0x7fL
            i++
        }

        while (i > 0) {
            var temp = value ushr (i * 7)
            temp = temp and 0x7f

            append(((temp or 0x80) and 0xff).toInt())

            i--
        }

        append((value and 0x7f).toInt())
    }

    /**
     * Append date value into mMessage.
     */
    private fun appendDateValue(date: Long) {
        /*
         * From OMA-TS-MMS-ENC-V1_3-20050927-C:
         * Date-value = Long-integer
         */
        appendLongInteger(date)
    }

    /**
     * Append value length to mMessage.
     */
    private fun appendValueLength(value: Long) {
        /*
         * From WAP-230-WSP-20010705-a:
         * Value-length = Short-length | (Length-quote Length)
         * ; Value length is used to indicate the length of the value to follow
         * Short-length = <Any octet 0-30>
         * Length-quote = <Octet 31>
         * Length = Uintvar-integer
         */
        if (value < LENGTH_QUOTE) {
            appendShortLength(value.toInt())
            return
        }

        append(LENGTH_QUOTE)
        appendUintvarInteger(value)
    }

    /**
     * Append quoted string to mMessage.
     */
    private fun appendQuotedString(text: ByteArray) {
        /*
         * From WAP-230-WSP-20010705-a:
         * Quoted-string = <Octet 34> *TEXT End-of-string
         * ;The TEXT encodes an RFC2616 Quoted-string with the enclosing
         * ;quotation-marks <"> removed.
         */
        append(QUOTED_STRING_FLAG)
        arraycopy(text, 0, text.size)
        append(END_STRING_FLAG)
    }

    /**
     * Append quoted string to mMessage.
     */
    private fun appendQuotedString(str: String) {
        /*
         * From WAP-230-WSP-20010705-a:
         * Quoted-string = <Octet 34> *TEXT End-of-string
         * ;The TEXT encodes an RFC2616 Quoted-string with the enclosing
         * ;quotation-marks <"> removed.
         */
        appendQuotedString(str.toByteArray())
    }

    private fun appendAddressType(address: EncodedStringValue): EncodedStringValue? {
        var temp: EncodedStringValue? = null

        try {
            val addressType = checkAddressType(address.string)
            temp = EncodedStringValue.copy(address)
            if (PDU_PHONE_NUMBER_ADDRESS_TYPE == addressType) {
                // Phone number.
                temp!!.appendTextString(STRING_PHONE_NUMBER_ADDRESS_TYPE.toByteArray())
            } else if (PDU_IPV4_ADDRESS_TYPE == addressType) {
                // Ipv4 address.
                temp!!.appendTextString(STRING_IPV4_ADDRESS_TYPE.toByteArray())
            } else if (PDU_IPV6_ADDRESS_TYPE == addressType) {
                // Ipv6 address.
                temp!!.appendTextString(STRING_IPV6_ADDRESS_TYPE.toByteArray())
            }
        } catch (e: NullPointerException) {
            return null
        }

        return temp
    }

    /**
     * Append header to mMessage.
     */
    private fun appendHeader(field: Int): Int {
        when (field) {
            PduHeaders.MMS_VERSION -> {
                appendOctet(field)

                val version = mPduHeader.getOctet(field)
                if (0 == version) {
                    appendShortInteger(PduHeaders.CURRENT_MMS_VERSION)
                } else {
                    appendShortInteger(version)
                }
            }

            PduHeaders.MESSAGE_ID, PduHeaders.TRANSACTION_ID -> {
                val textString = mPduHeader.getTextString(field)
                    ?: return PDU_COMPOSE_FIELD_NOT_SET

                appendOctet(field)
                appendTextString(textString)
            }

            PduHeaders.TO, PduHeaders.BCC, PduHeaders.CC -> {
                val addr = mPduHeader.getEncodedStringValues(field)
                    ?: return PDU_COMPOSE_FIELD_NOT_SET

                for (i in addr.indices) {
                    val temp = appendAddressType(addr[i])
                        ?: return PDU_COMPOSE_CONTENT_ERROR

                    appendOctet(field)
                    appendEncodedString(temp)
                }
            }

            PduHeaders.FROM -> {
                // Value-length (Address-present-token Encoded-string-value |
                // Insert-address-token)
                appendOctet(field)

                val from = mPduHeader.getEncodedStringValue(field)
                if ((from == null) ||
                    TextUtils.isEmpty(from.string) ||
                    String(from.getTextString()) == PduHeaders.FROM_INSERT_ADDRESS_TOKEN_STR
                ) {
                    // Length of from = 1
                    append(1)
                    // Insert-address-token = <Octet 129>
                    append(PduHeaders.FROM_INSERT_ADDRESS_TOKEN)
                } else {
                    mStack.newbuf()
                    val fstart = mStack.mark()

                    // Address-present-token = <Octet 128>
                    append(PduHeaders.FROM_ADDRESS_PRESENT_TOKEN)

                    val temp = appendAddressType(from)
                        ?: return PDU_COMPOSE_CONTENT_ERROR

                    appendEncodedString(temp)

                    val flen = fstart.getLength()
                    mStack.pop()
                    appendValueLength(flen.toLong())
                    mStack.copy()
                }
            }

            PduHeaders.READ_STATUS, PduHeaders.STATUS, PduHeaders.REPORT_ALLOWED,
            PduHeaders.PRIORITY, PduHeaders.DELIVERY_REPORT, PduHeaders.READ_REPORT,
            -> {
                val octet = mPduHeader.getOctet(field)
                if (0 == octet) {
                    return PDU_COMPOSE_FIELD_NOT_SET
                }

                appendOctet(field)
                appendOctet(octet)
            }

            PduHeaders.DATE -> {
                val date = mPduHeader.getLongInteger(field)
                if (-1L == date) {
                    return PDU_COMPOSE_FIELD_NOT_SET
                }

                appendOctet(field)
                appendDateValue(date)
            }

            PduHeaders.SUBJECT -> {
                val enString = mPduHeader.getEncodedStringValue(field)
                    ?: return PDU_COMPOSE_FIELD_NOT_SET

                appendOctet(field)
                appendEncodedString(enString)
            }

            PduHeaders.MESSAGE_CLASS -> {
                val messageClass = mPduHeader.getTextString(field)
                    ?: return PDU_COMPOSE_FIELD_NOT_SET

                appendOctet(field)
                if (messageClass.contentEquals(PduHeaders.MESSAGE_CLASS_ADVERTISEMENT_STR.toByteArray())) {
                    appendOctet(PduHeaders.MESSAGE_CLASS_ADVERTISEMENT)
                } else if (messageClass.contentEquals(PduHeaders.MESSAGE_CLASS_AUTO_STR.toByteArray())) {
                    appendOctet(PduHeaders.MESSAGE_CLASS_AUTO)
                } else if (messageClass.contentEquals(PduHeaders.MESSAGE_CLASS_PERSONAL_STR.toByteArray())) {
                    appendOctet(PduHeaders.MESSAGE_CLASS_PERSONAL)
                } else if (messageClass.contentEquals(PduHeaders.MESSAGE_CLASS_INFORMATIONAL_STR.toByteArray())) {
                    appendOctet(PduHeaders.MESSAGE_CLASS_INFORMATIONAL)
                } else {
                    appendTextString(messageClass)
                }
            }

            PduHeaders.EXPIRY -> {
                val expiry = mPduHeader.getLongInteger(field)
                if (-1L == expiry) {
                    return PDU_COMPOSE_FIELD_NOT_SET
                }

                appendOctet(field)

                mStack.newbuf()
                val expiryStart = mStack.mark()

                append(PduHeaders.VALUE_RELATIVE_TOKEN)
                appendLongInteger(expiry)

                val expiryLength = expiryStart.getLength()
                mStack.pop()
                appendValueLength(expiryLength.toLong())
                mStack.copy()
            }

            else -> return PDU_COMPOSE_FIELD_NOT_SUPPORTED
        }

        return PDU_COMPOSE_SUCCESS
    }

    /**
     * Make ReadRec.Ind.
     */
    private fun makeReadRecInd(): Int {
        // X-Mms-Message-Type
        appendOctet(PduHeaders.MESSAGE_TYPE)
        appendOctet(PduHeaders.MESSAGE_TYPE_READ_REC_IND)

        // X-Mms-MMS-Version
        if (appendHeader(PduHeaders.MMS_VERSION) != PDU_COMPOSE_SUCCESS) {
            return PDU_COMPOSE_CONTENT_ERROR
        }

        // Message-ID
        if (appendHeader(PduHeaders.MESSAGE_ID) != PDU_COMPOSE_SUCCESS) {
            return PDU_COMPOSE_CONTENT_ERROR
        }

        // To
        if (appendHeader(PduHeaders.TO) != PDU_COMPOSE_SUCCESS) {
            return PDU_COMPOSE_CONTENT_ERROR
        }

        // From
        if (appendHeader(PduHeaders.FROM) != PDU_COMPOSE_SUCCESS) {
            return PDU_COMPOSE_CONTENT_ERROR
        }

        // Date Optional
        appendHeader(PduHeaders.DATE)

        // X-Mms-Read-Status
        if (appendHeader(PduHeaders.READ_STATUS) != PDU_COMPOSE_SUCCESS) {
            return PDU_COMPOSE_CONTENT_ERROR
        }

        // X-Mms-Applic-ID Optional(not support)
        // X-Mms-Reply-Applic-ID Optional(not support)
        // X-Mms-Aux-Applic-Info Optional(not support)

        return PDU_COMPOSE_SUCCESS
    }

    /**
     * Make NotifyResp.Ind.
     */
    private fun makeNotifyResp(): Int {
        // X-Mms-Message-Type
        appendOctet(PduHeaders.MESSAGE_TYPE)
        appendOctet(PduHeaders.MESSAGE_TYPE_NOTIFYRESP_IND)

        // X-Mms-Transaction-ID
        if (appendHeader(PduHeaders.TRANSACTION_ID) != PDU_COMPOSE_SUCCESS) {
            return PDU_COMPOSE_CONTENT_ERROR
        }

        // X-Mms-MMS-Version
        if (appendHeader(PduHeaders.MMS_VERSION) != PDU_COMPOSE_SUCCESS) {
            return PDU_COMPOSE_CONTENT_ERROR
        }

        // X-Mms-Status
        if (appendHeader(PduHeaders.STATUS) != PDU_COMPOSE_SUCCESS) {
            return PDU_COMPOSE_CONTENT_ERROR
        }

        // X-Mms-Report-Allowed Optional (not support)
        return PDU_COMPOSE_SUCCESS
    }

    /**
     * Make Acknowledge.Ind.
     */
    private fun makeAckInd(): Int {
        // X-Mms-Message-Type
        appendOctet(PduHeaders.MESSAGE_TYPE)
        appendOctet(PduHeaders.MESSAGE_TYPE_ACKNOWLEDGE_IND)

        // X-Mms-Transaction-ID
        if (appendHeader(PduHeaders.TRANSACTION_ID) != PDU_COMPOSE_SUCCESS) {
            return PDU_COMPOSE_CONTENT_ERROR
        }

        // X-Mms-MMS-Version
        if (appendHeader(PduHeaders.MMS_VERSION) != PDU_COMPOSE_SUCCESS) {
            return PDU_COMPOSE_CONTENT_ERROR
        }

        // X-Mms-Report-Allowed Optional
        appendHeader(PduHeaders.REPORT_ALLOWED)

        return PDU_COMPOSE_SUCCESS
    }

    /**
     * Make Send.req.
     */
    private fun makeSendReqPdu(): Int {
        // X-Mms-Message-Type
        appendOctet(PduHeaders.MESSAGE_TYPE)
        appendOctet(PduHeaders.MESSAGE_TYPE_SEND_REQ)

        // X-Mms-Transaction-ID
        appendOctet(PduHeaders.TRANSACTION_ID)

        val trid = mPduHeader.getTextString(PduHeaders.TRANSACTION_ID)
            // Transaction-ID should be set(by Transaction) before make().
            ?: throw IllegalArgumentException("Transaction-ID is null.")
        appendTextString(trid)

        // X-Mms-MMS-Version
        if (appendHeader(PduHeaders.MMS_VERSION) != PDU_COMPOSE_SUCCESS) {
            return PDU_COMPOSE_CONTENT_ERROR
        }

        // Date Date-value Optional.
        appendHeader(PduHeaders.DATE)

        // From
        if (appendHeader(PduHeaders.FROM) != PDU_COMPOSE_SUCCESS) {
            return PDU_COMPOSE_CONTENT_ERROR
        }

        var recipient = false

        // To
        if (appendHeader(PduHeaders.TO) != PDU_COMPOSE_CONTENT_ERROR) {
            recipient = true
        }

        // Cc
        if (appendHeader(PduHeaders.CC) != PDU_COMPOSE_CONTENT_ERROR) {
            recipient = true
        }

        // Bcc
        if (appendHeader(PduHeaders.BCC) != PDU_COMPOSE_CONTENT_ERROR) {
            recipient = true
        }

        // Need at least one of "cc", "bcc" and "to".
        if (!recipient) {
            return PDU_COMPOSE_CONTENT_ERROR
        }

        // Subject Optional
        appendHeader(PduHeaders.SUBJECT)

        // X-Mms-Message-Class Optional
        // Message-class-value = Class-identifier | Token-text
        appendHeader(PduHeaders.MESSAGE_CLASS)

        // X-Mms-Expiry Optional
        appendHeader(PduHeaders.EXPIRY)

        // X-Mms-Priority Optional
        appendHeader(PduHeaders.PRIORITY)

        // X-Mms-Delivery-Report Optional
        appendHeader(PduHeaders.DELIVERY_REPORT)

        // X-Mms-Read-Report Optional
        appendHeader(PduHeaders.READ_REPORT)

        // Content-Type
        appendOctet(PduHeaders.CONTENT_TYPE)

        // Message body
        return makeMessageBody()
    }

    /**
     * Make message body.
     */
    private fun makeMessageBody(): Int {
        // 1. add body informations
        mStack.newbuf() // Switching buffer because we need to

        val ctStart = mStack.mark()

        // This contentTypeIdentifier should be used for type of attachment...
        val contentType = String(mPduHeader.getTextString(PduHeaders.CONTENT_TYPE)!!)
        val contentTypeIdentifier = mContentTypeMap[contentType]
            // content type is mandatory
            ?: return PDU_COMPOSE_CONTENT_ERROR

        appendShortInteger(contentTypeIdentifier)

        // content-type parameter: start
        val body = (mPdu as SendReq).body
        if (null == body || body.partsNum == 0) {
            // empty message
            appendUintvarInteger(0)
            mStack.pop()
            mStack.copy()
            return PDU_COMPOSE_SUCCESS
        }

        var part: PduPart
        try {
            part = body.getPart(0)

            val start = part.contentId
            if (start != null) {
                appendOctet(PduPart.P_DEP_START)
                if (('<'.code.toByte() == start[0]) && ('>'.code.toByte() == start[start.size - 1])) {
                    appendTextString(start)
                } else {
                    appendTextString("<" + String(start) + ">")
                }
            }

            // content-type parameter: type
            appendOctet(PduPart.P_CT_MR_TYPE)
            appendTextString(part.contentType!!)
        } catch (e: ArrayIndexOutOfBoundsException) {
            Log.e(TAG, "logging error", e)
            e.printStackTrace()
        }

        val ctLength = ctStart.getLength()
        mStack.pop()
        appendValueLength(ctLength.toLong())
        mStack.copy()

        // 3. add content
        val partNum = body.partsNum
        appendUintvarInteger(partNum.toLong())
        for (i in 0 until partNum) {
            part = body.getPart(i)
            mStack.newbuf() // Leaving space for header lengh and data length
            val attachment = mStack.mark()

            mStack.newbuf() // Leaving space for Content-Type length
            val contentTypeBegin = mStack.mark()

            val partContentType = part.contentType
                // content type is mandatory
                ?: return PDU_COMPOSE_CONTENT_ERROR

            // content-type value
            val partContentTypeIdentifier = mContentTypeMap[String(partContentType)]
            if (partContentTypeIdentifier == null) {
                appendTextString(partContentType)
            } else {
                appendShortInteger(partContentTypeIdentifier)
            }

            /*
             * Content-type parameter : name.
             * The value of name, filename, content-location is the same.
             * Just one of them is enough for this PDU.
             */
            var name = part.name

            if (null == name) {
                name = part.filename

                if (null == name) {
                    name = part.contentLocation

                    if (null == name) {
                        /*
                         * at lease one of name, filename, Content-location
                         * should be available.
                         */
                        return PDU_COMPOSE_CONTENT_ERROR
                    }
                }
            }
            appendOctet(PduPart.P_DEP_NAME)
            appendTextString(name)

            // content-type parameter : charset
            val charset = part.charset
            if (charset != 0) {
                appendOctet(PduPart.P_CHARSET)
                appendShortInteger(charset)
            }

            val contentTypeLength = contentTypeBegin.getLength()
            mStack.pop()
            appendValueLength(contentTypeLength.toLong())
            mStack.copy()

            // content id
            val contentId = part.contentId

            if (null != contentId) {
                appendOctet(PduPart.P_CONTENT_ID)
                if (('<'.code.toByte() == contentId[0]) && ('>'.code.toByte() == contentId[contentId.size - 1])) {
                    appendQuotedString(contentId)
                } else {
                    appendQuotedString("<" + String(contentId) + ">")
                }
            }

            // content-location
            val contentLocation = part.contentLocation
            if (null != contentLocation) {
                appendOctet(PduPart.P_CONTENT_LOCATION)
                appendTextString(contentLocation)
            }

            // content
            val headerLength = attachment.getLength()

            var dataLength = 0 // Just for safety...
            val partData = part.data

            if (partData != null) {
                arraycopy(partData, 0, partData.size)
                dataLength = partData.size
            } else {
                var cr: InputStream? = null
                try {
                    val buffer = ByteArray(PDU_COMPOSER_BLOCK_SIZE)
                    cr = mResolver.openInputStream(part.dataUri!!)
                    var len = 0
                    while ((cr!!.read(buffer).also { len = it }) != -1) {
                        mMessage.write(buffer, 0, len)
                        mPosition += len
                        dataLength += len
                    }
                } catch (e: FileNotFoundException) {
                    return PDU_COMPOSE_CONTENT_ERROR
                } catch (e: IOException) {
                    return PDU_COMPOSE_CONTENT_ERROR
                } catch (e: RuntimeException) {
                    return PDU_COMPOSE_CONTENT_ERROR
                } finally {
                    if (cr != null) {
                        try {
                            cr.close()
                        } catch (e: IOException) {
                        }
                    }
                }
            }

            if (dataLength != (attachment.getLength() - headerLength)) {
                throw RuntimeException("BUG: Length sanity check failed")
            }

            mStack.pop()
            appendUintvarInteger(headerLength.toLong())
            appendUintvarInteger(dataLength.toLong())
            mStack.copy()
        }

        return PDU_COMPOSE_SUCCESS
    }

    /**
     * Record current message informations.
     */
    private class LengthRecordNode {
        var currentMessage: ByteArrayOutputStream? = null
        var currentPosition = 0

        var next: LengthRecordNode? = null
    }

    /**
     * Mark current message position and stact size.
     */
    private inner class PositionMarker {
        var c_pos = 0 // Current position
        var currentStackSize = 0 // Current stack size

        fun getLength(): Int {
            // If these assert fails, likely that you are finding the
            // size of buffer that is deep in BufferStack you can only
            // find the length of the buffer that is on top
            if (currentStackSize != mStack.stackSize) {
                throw RuntimeException("BUG: Invalid call to getLength()")
            }

            return mPosition - c_pos
        }
    }

    /**
     * This implementation can be OPTIMIZED to use only
     * 2 buffers. This optimization involves changing BufferStack
     * only... Its usage (interface) will not change.
     */
    private inner class BufferStack {
        private var stack: LengthRecordNode? = null
        private var toCopy: LengthRecordNode? = null

        var stackSize = 0

        /**
         * Create a new message buffer and push it into the stack.
         */
        fun newbuf() {
            // You can't create a new buff when toCopy != null
            // That is after calling pop() and before calling copy()
            // If you do, it is a bug
            if (toCopy != null) {
                throw RuntimeException("BUG: Invalid newbuf() before copy()")
            }

            val temp = LengthRecordNode()

            temp.currentMessage = mMessage
            temp.currentPosition = mPosition

            temp.next = stack
            stack = temp

            stackSize += 1

            mMessage = ByteArrayOutputStream()
            mPosition = 0
        }

        /**
         * Pop the message before and record current message in the stack.
         */
        fun pop() {
            val currentMessage = mMessage
            val currentPosition = mPosition

            mMessage = stack!!.currentMessage!!
            mPosition = stack!!.currentPosition

            toCopy = stack
            // Re using the top element of the stack to avoid memory allocation

            stack = stack!!.next
            stackSize -= 1

            toCopy!!.currentMessage = currentMessage
            toCopy!!.currentPosition = currentPosition
        }

        /**
         * Append current message to the message before.
         */
        fun copy() {
            arraycopy(
                toCopy!!.currentMessage!!.toByteArray(), 0,
                toCopy!!.currentPosition,
            )

            toCopy = null
        }

        /**
         * Mark current message position
         */
        fun mark(): PositionMarker {
            val m = PositionMarker()

            m.c_pos = mPosition
            m.currentStackSize = stackSize

            return m
        }
    }

    companion object {
        /**
         * Address type.
         */
        private const val PDU_PHONE_NUMBER_ADDRESS_TYPE = 1
        private const val PDU_EMAIL_ADDRESS_TYPE = 2
        private const val PDU_IPV4_ADDRESS_TYPE = 3
        private const val PDU_IPV6_ADDRESS_TYPE = 4
        private const val PDU_UNKNOWN_ADDRESS_TYPE = 5

        /**
         * Address regular expression string.
         */
        const val REGEXP_PHONE_NUMBER_ADDRESS_TYPE = "\\+?[0-9|\\.|\\-]+"
        const val REGEXP_EMAIL_ADDRESS_TYPE = "[a-zA-Z| ]*\\<{0,1}[a-zA-Z| ]+@{1}" +
            "[a-zA-Z| ]+\\.{1}[a-zA-Z| ]+\\>{0,1}"
        const val REGEXP_IPV6_ADDRESS_TYPE = "[a-fA-F]{4}\\:{1}[a-fA-F0-9]{4}\\:{1}[a-fA-F0-9]{4}\\:{1}" +
            "[a-fA-F0-9]{4}\\:{1}[a-fA-F0-9]{4}\\:{1}[a-fA-F0-9]{4}\\:{1}" +
            "[a-fA-F0-9]{4}\\:{1}[a-fA-F0-9]{4}"
        const val REGEXP_IPV4_ADDRESS_TYPE = "[0-9]{1,3}\\.{1}[0-9]{1,3}\\.{1}" +
            "[0-9]{1,3}\\.{1}[0-9]{1,3}"

        /**
         * The postfix strings of address.
         */
        const val STRING_PHONE_NUMBER_ADDRESS_TYPE = "/TYPE=PLMN"
        const val STRING_IPV4_ADDRESS_TYPE = "/TYPE=IPV4"
        const val STRING_IPV6_ADDRESS_TYPE = "/TYPE=IPV6"

        /**
         * Error values.
         */
        private const val PDU_COMPOSE_SUCCESS = 0
        private const val PDU_COMPOSE_CONTENT_ERROR = 1
        private const val PDU_COMPOSE_FIELD_NOT_SET = 2
        private const val PDU_COMPOSE_FIELD_NOT_SUPPORTED = 3

        /**
         * WAP values defined in WSP spec.
         */
        private const val QUOTED_STRING_FLAG = 34
        private const val END_STRING_FLAG = 0
        private const val LENGTH_QUOTE = 31
        private const val TEXT_MAX = 127
        private const val SHORT_INTEGER_MAX = 127
        private const val LONG_INTEGER_LENGTH_MAX = 8

        /**
         * Block size when read data from InputStream.
         */
        private const val PDU_COMPOSER_BLOCK_SIZE = 1024
        private const val TAG = "PduComposer"

        /**
         * Map of all content type
         */
        private val mContentTypeMap: HashMap<String, Int> = HashMap()

        init {
            for (i in PduContentTypes.contentTypes.indices) {
                mContentTypeMap[PduContentTypes.contentTypes[i]] = i
            }
        }

        /**
         * Check address type.
         *
         * @param address address string without the postfix stinng type,
         *                such as "/TYPE=PLMN", "/TYPE=IPv6" and "/TYPE=IPv4"
         * @return PDU_PHONE_NUMBER_ADDRESS_TYPE if it is phone number,
         *         PDU_EMAIL_ADDRESS_TYPE if it is email address,
         *         PDU_IPV4_ADDRESS_TYPE if it is ipv4 address,
         *         PDU_IPV6_ADDRESS_TYPE if it is ipv6 address,
         *         PDU_UNKNOWN_ADDRESS_TYPE if it is unknown.
         */
        private fun checkAddressType(address: String?): Int {
            /**
             * From OMA-TS-MMS-ENC-V1_3-20050927-C.pdf, section 8.
             * address = ( e-mail / device-address / alphanum-shortcode / num-shortcode)
             * ...
             */

            if (null == address) {
                return PDU_UNKNOWN_ADDRESS_TYPE
            }

            return if (address.matches(REGEXP_IPV4_ADDRESS_TYPE.toRegex())) {
                // Ipv4 address.
                PDU_IPV4_ADDRESS_TYPE
            } else if (address.matches(REGEXP_PHONE_NUMBER_ADDRESS_TYPE.toRegex())) {
                // Phone number.
                PDU_PHONE_NUMBER_ADDRESS_TYPE
            } else if (address.matches(REGEXP_EMAIL_ADDRESS_TYPE.toRegex())) {
                // Email address.
                PDU_EMAIL_ADDRESS_TYPE
            } else if (address.matches(REGEXP_IPV6_ADDRESS_TYPE.toRegex())) {
                // Ipv6 address.
                PDU_IPV6_ADDRESS_TYPE
            } else {
                // Unknown address.
                PDU_UNKNOWN_ADDRESS_TYPE
            }
        }
    }
}
