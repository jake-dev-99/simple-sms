...
public class MmsMessageSender implements MessageSender {
    ...
    public boolean sendMessage(long token) throws MmsException {
        ...
        if (!mMessageUri.toString().startsWith(Mms.Draft.CONTENT_URI.toString())) {
            // If the message is already in the outbox (most likely because we created a
            // "primed"
            // message in the outbox when the user hit send), then we have to manually put
            // an
            // entry in the pending_msgs table which is where TransacationService looks for
            // messages to send. Normally, the entry in pending_msgs is created by the
            // trigger:
            // insert_mms_pending_on_update, when a message is moved from drafts to the
            // outbox.
