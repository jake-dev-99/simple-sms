...
public class NotificationTransaction extends Transaction implements Runnable {
    ...
    public NotificationTransaction(
        ...
        try {
            // Save the pdu. If we can start downloading the real pdu immediately, don't
            // allow
            // persist() to create a thread for the notificationInd because it causes UI
            // jank.
            boolean group;
    ...
    public static boolean allowAutoDownload(Context context) {
        ...
        boolean autoDownload = PreferenceManager.getDefaultSharedPreferences(context).getBoolean("auto_download_mms",
                true);
        boolean dataSuspended = (((TelephonyManager) context.getSystemService(Service.TELEPHONY_SERVICE))
    ...
    public void run() {
        ...
        try {
            ...
            // Don't try to download when data is suspended, as it will fail, so defer
            // download
            if (!autoDownload) {
